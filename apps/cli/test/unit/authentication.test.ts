...
import type { HandshakeRole } from "@psilink/core";
...
test("both parties derive the same rotated token over a real connection", async () => {
  ...
  const [a, b] = await Promise.all([
    authenticateConnection(connA, { pakeToken: TOKEN_A }, roleA),
    authenticateConnection(connB, { pakeToken: TOKEN_A }, roleB),
...
test("authentication throws for a token that is not 43 base64url characters", async () => {
  const conn = makeConn();
  await expect(
    authenticateConnection(conn, { pakeToken: "tooshort" }, "initiator"),
  ).rejects.toThrow(
...
test("authentication throws for a token containing non-base64url characters", async () => {
  ...
  await expect(
    authenticateConnection(conn, { pakeToken: badToken }, "initiator"),
  ).rejects.toThrow(
    "authentication.pakeToken must be a base64url-encoded 32-byte value",
  );
});
...
test("authentication throws when tokens differ", async () => {
  ...
  const [resultA, resultB] = await Promise.allSettled([
    authenticateConnection(connA, { pakeToken: TOKEN_A }, roleA),
    authenticateConnection(connB, { pakeToken: TOKEN_B }, roleB),
  ]).finally(() => teardown(connA, connB));
