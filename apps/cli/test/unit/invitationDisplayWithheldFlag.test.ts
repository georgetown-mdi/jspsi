...
describe("displayInvitation: the own-membership variant's one input", () => {
  test("the honest-helper membership sentence follows core's resolved flag against the terms rendered beside it", async () => {
    ...
    // and the whole of what the prompt reads that from is
    // inviterTableWithheld. Both documents here are rendered under the
    // OPPOSITE flag to the one their own terms derive, so a prompt
    ...
    const cascadeUnderWithheld = render(tokenFor(HELPER_CASCADE_TERMS));
    expect(cascadeUnderWithheld).toContain(
    ...
    const withholdingUnderDisclosure = render(
      tokenFor(HELPER_WITHHOLDING_TERMS),
    );
