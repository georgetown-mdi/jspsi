...
/**
 * The backend the suite runs against, from ALCOVE_SFTP_BACKEND. Unset (or
...
export function selectedBackend(): SftpBackendName {
  const raw = process.env.ALCOVE_SFTP_BACKEND;
  if (raw === undefined || raw === "" || raw === "in-process")
    return "in-process";
  if (raw === "native") return "native";
  throw new Error(
    `Unknown ALCOVE_SFTP_BACKEND "${raw}" (expected "in-process" or "native").`,
  );
}

/**
 * The hardened configuration the native backend runs, from
 * ALCOVE_SFTP_NATIVE_PROFILE. Unset selects `baseline` (the unchanged Phase-1
