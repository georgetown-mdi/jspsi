      ...
      {disclosedColumns !== undefined &&
        (disclosedColumns.length > 0 ? (
          <Paper withBorder p="md">
            ...
            {/* Informational chips, not controls: changing what is sent happens in
                Advanced, never by editing a chip. The operator's column names are
                sanitized for display here (ColumnChips renders them verbatim). */}
            <Box mt="xs">
              <ColumnChips
                columns={disclosedColumns.map((name) =>
                  sanitizeForDisplay(name),
                )}
                label="Default columns sent to your partner"
              />
            </Box>
            ...
            <Text size="xs" c="dimmed" mt="xs">
              These are the defaults; you can change these by using advanced
              options for invitation generation or reviewing an existing
              invitation.
            </Text>
