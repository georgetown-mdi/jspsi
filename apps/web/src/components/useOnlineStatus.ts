 ...
 * `online`/`offline` events. Server rendering claims nothing (`true`), and the
 * client corrects it on hydration.
 *
 * Only `false` is a reliable answer -- see `apps/web/src/utils/networkStatus.ts`
 * -- so a caller gates on the offline case and never treats `true` as a promise
 * that a partner is reachable.
 */
