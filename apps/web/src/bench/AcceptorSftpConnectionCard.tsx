 ...
 * the SFTP server the PARTNER named in the invitation, before the console can run
 * the accept. The partner-supplied locator (host/port/path) is shown read-only
 * and pre-fills the authoring request; the operator supplies the username, the
 * required host-key fingerprint, and the credential -- none of which an
 * invitation can supply or override (an {@link SFTPEndpoint} is credential- and
 * fingerprint-free by construction, and {@link sftpFormFromLocator} reads only the
 * locator).
 *
 * Required control: the fingerprint is pinned BEFORE any credentialed connect,
 ...
 * transport is fixed by the invitation): an authored connection with edit and
 * clear controls, and the authoring prompt when none is set up yet.
 */
