 ...
 * the connection and the linkage terms, and the connection's `options` block,
 * which the connection rows do not show. Each is written back to the
...
export function heldSettings(record: ManagedExchangeRecord): Array<string> {
  ...
  const connectionOptions =
    ...
    exchangeFile.connection.options !== undefined
      ? ["connection.options"]
      : [];
  return [...topLevel, ...connectionOptions].sort();
}
...
/**
 * What the export tells the operator about an SFTP configuration, or undefined
 * on any other channel. The stored connection holds no credential and no host
 * key pin -- the import refuses both -- and psilink needs both to run it, so
 * the operator adds them to the file this surface hands back.
 */
export function sftpCredentialNote(
  record: ManagedExchangeRecord,
): string | undefined {
  if (record.exchangeFile.connection.channel !== "sftp") return undefined;
  return (
    "This configuration names no SFTP credential and no host key. Before you " +
    "run it, add private_key or password under connection.server -- written " +
    "as @ and the path of the file that holds it -- and the server's " +
