 ...
 * the console's job API; `save-file` mints an exchange file the operator runs
 * with the command-line tool. The console consumes this at two call sites: the
 * Create branch, which routes `save-file` to the save surface and both live
 * kinds to the run, and the run hooks, which build the matching
 * {@link ExchangeDriver}.
 */
...
/**
 * How the SFTP transport stands on a given build, distinct from the binary
 * "configured or not":
...
/**
 * Map a chosen `channel`, the build's `profile`, the console's sftp connection
 * availability, and the operator's explicit save-a-file choice to how the
 * exchange runs. A filedrop channel runs server-side on the console. An sftp
 * channel on a console runs server-side when a connection is configured
 ...
 * chose to save a file for their own command-line tool instead
 ...
 * to a branch it was never vetted for -- the allowlist discipline
 * CONTRIBUTING.md requires for transport branching.
 */
