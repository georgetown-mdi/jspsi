 ...
 * operator-mounted work-input directory -- an opaque name the server itself
 * enumerated plus the client's profiled `sizeBytes`/`modifiedAt` snapshot for the
 * create-time freshness check -- so no content transits the browser. `intentFor`
    ...
    ...(inputSource.kind === "inline"
      ...
      : {
          inputFile: {
            ...
            modifiedAt: inputSource.modifiedAt,
          },
        }),
