 ...
 * only where a worker is spawned (imported from {@link ./exchangeLifecycle} in the
 * browser) and never evaluated under Node -- which {@link ./psiCryptoController}'s
 * unit tests import.
 *
 * The role/id seed is passed in the Worker's `name` (the browser analogue of the
