 ...
 *   and the linear-time regex dialect gate applies to `linkageTerms` transform
 *   patterns, NOT to the standardization pipeline's raw-pattern steps -- so a client
 *   can submit a large or non-conformant `standardization`. That is a resource bound,
 *   not an injection escape: it cannot become argv/path/host/credential.
 *
...
export function composeConfigDocument(
  ...
  const { metadata, standardization } = intent;
