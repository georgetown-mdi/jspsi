 ...
 * shared shape: the appliance is provisioned with exactly one SFTP server, so
 ...
 * server-side provisioned entry; the intent contributes only the `sftp`
 * discriminant.
 */
 ...
 * exclusively from the operator-provisioned SFTP server.
 */
 ...
 * and the operator-provisioned server entry.
 *
 * The connection's `server` block is EXACTLY the provisioned entry: every
 * host, port, identity, and credential-reference field is server-side data
 * validated at boot, and the intent contributes nothing to it. The entry's
