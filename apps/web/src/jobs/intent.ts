 ...
 * selects an entry the server itself enumerated (validated by the listing's own
 * {@link isAdmissibleInputName} shape rule, re-admitted at create), and the
 * `sizeBytes`/`modifiedAt` pair is the client's profiled snapshot, re-checked
 * against the open-time fstat so a drifted file is refused. `modifiedAt` is the one
 * serialized representation used everywhere -- integer epoch milliseconds, exactly
 * as the listing and profile emit it -- compared by exact equality.
 */
 ...
 *   directory: an opaque name plus the client's profiled `sizeBytes`/`modifiedAt`
 *   snapshot. The name is never trusted -- at create time the manager re-runs the
 *   listing's admission (lstat regular-file + `O_NOFOLLOW` open + `(dev, ino)`
 *   recheck) against the server's own directory enumeration, streams the opened
 *   file into the same fixed workdir name, and never lets the name reach argv or a
 *   composed path. The size/mtime pair is re-checked against the open-time fstat so
 *   a file that changed since profiling is refused rather than snapshotted.
 ...
 * name selecting an operator-provisioned resource (`inputFile.name`, re-admitted
 * against the server's own directory enumeration; and, on the sftp arm, `remote`),
 * or credential material written to a fixed key file. There is no field that
 * becomes a path, a host, a credential reference (`@path`), or an argv string.
 * Every directory the exchange uses is generated by the server inside the job
 * workdir; connection material for an sftp exchange comes exclusively from the
 * operator-provisioned remotes table.
 */
...
// The `name` is bounded and single-segment by the listing's own shape rule, but is
// never trusted from here: the manager re-runs the full lstat/O_NOFOLLOW admission
// against the server's directory enumeration at create time. `sizeBytes` is a
// positive integer (a real input is non-empty) and `modifiedAt` an integer epoch-
// millisecond value, exactly as the listing/profile emit it.
