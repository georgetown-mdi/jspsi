 ...
 * enumerates the handlers under `apps/web/src/routes/api/jobs` and fails on one
 * that calls it late or not at all, so that is a checked property rather than
 * this sentence. The API is unauthenticated loopback-local (the deployment
 * publishes to host loopback), so there is no per-request auth beyond the
 * feature gate; two complementary browser defenses run after it. The loopback
 * Host-allowlist ({@link rejectDisallowedHost}) refuses a request whose
