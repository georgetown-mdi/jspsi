...
- **One security group, owned by the root.** `DisableDefaultEC2SecurityGroup` is `true`, so the platform creates no security group of its own, and `SecurityGroups` names the one group the root owns. Its inbound list is `:443` from Cloudflare's published ranges and nothing else, held by the root rather than by an option setting; why this form and not another is in [the root's README](../../../../infra/hosted/README.md#how-the-inbound-rules-are-held). Until the root is first applied, these files record the platform's own group beside that one, with `DisableDefaultEC2SecurityGroup` `false`.
