...
The inbound rules are known to survive a configuration deployment, measured 2026-09-17, and a `rebuild-environment` on each environment, measured 2026-09-23 after [the OpenTofu root's first apply](../../../../infra/hosted/README.md#the-first-run-against-the-live-account): one group stayed attached, the root's, with no platform group created. Whether they survive a managed platform update is unverified; driving one needs the live account, so it is the maintainer's to run outside the container. Record the result here when it is run, with the date and the operation that was run.
