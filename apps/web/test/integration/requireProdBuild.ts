...
// server. The suites that drive `node .output/server/index.mjs` gate themselves on
// its presence (hasBuild), so without this guard a build-free run reports a PASS
// with the whole built-server surface unexercised. CI builds the web app before
// this project (eb_build_and_test.yaml), so the guard is inert there and fires on
// a local run against a fresh clone.
//
// The opt-out restores the skip for a deliberate dev-server-only run. The guard
// cannot live on the shared dev-server setup, which the `browser` project also
// runs and which needs no production build.
