...
/**
 * The budget for every test below: vitest's default is the wrong scale, since
 * each spawns up to two sequential real node children -- the stub, then the
 * built CLI, whose ~1MB bundle every spawn parses afresh -- where a sibling
 * unit test only calls a function. Sized above the sum both spawns of one test
 * may legitimately take, so a stalled child's own {@link CHILD_EXIT_TIMEOUT_MS}
 * always fires and names it, set on the suite rather than each test so a test
 * added later cannot silently fall back to a shorter default.
 */
