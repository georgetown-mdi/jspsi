...
- Workflow `schema` for long-form agents (reviewers, panelists): put the required list property first, `maxLength`-cap any free-text sibling, and instruct "populate every property; empty array when none". An agent that exhausts the structured-output retries anyway: re-run as a plain agent with a fixed-format text contract and parse the text.
