...
- A squash-and-merge draft goes into `scratch/squash-messages/` through the normalizer, never straight from a `Write`: draft it in /tmp and run `node .claude/scripts/format-squash-message.mjs <pr-number|unassigned> <draft> --out <path>`, which rewraps the body and reports the subject budget, markdown, or a list it cannot fix without rewriting. Enforced by `block-nonconforming-squash-message.mjs` on `Write` and `Edit`; the limits live in that script alone.
