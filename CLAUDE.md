...
- `CHANGELOG.md` is reader-facing release notes, not a commit log: add an entry only for a change an operator/deployer or a security reviewer acts on (a command, flag, config field, default, behavior, exit code, wire or on-disk format, breaking change, or security change), written as one or two observable-change lines that point to `docs/` for the why. No entry for an internal refactor, a test/CI/tooling change, a `@psilink/core` API reshape (the apps are its only consumer), or a doc-only edit -- that is the git history's job. Group under Added/Changed/Deprecated/Removed/Fixed/Security and prefix a breaking change `BREAKING:`. Full rule in `CONTRIBUTING.md`, Changelog.
