...
| `infra/`         | [Terraform](infra/aws_eb/README.md) for creating the web app's Elastic Beanstalk environment. A draft, and run by nothing here -- the deployment payload CI does use is `apps/web/deploy/aws_eb/`. Beside it, [the WebRTC relay spike's AWS guardrails and run tooling](infra/relay-spike-aws/README.md), also run by nothing here |
