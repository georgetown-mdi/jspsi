...
- The subject budget and the body wrap above are checked rather than remembered. `node .claude/scripts/format-squash-message.mjs <pr-number|unassigned> [<draft-path>] [--out <path>]` rewraps a draft's body, drops the markdown and list markers a commit message does not take, and reports the two things it cannot fix without rewriting the message -- an over-budget subject, an over-wide line inside an indented block. `squash-message.mjs` prints its own draft through it.
