...
- The subject budget and the body wrap above are checked rather than remembered. `node .claude/scripts/format-squash-message.mjs <pr-number|unassigned> [<draft-path>] [--out <path>]` rewraps a draft's body and reports what it cannot fix without rewriting the message -- an over-budget subject, markdown, a top-level list. `squash-message.mjs` prints its own draft through it.
