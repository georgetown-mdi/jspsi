...
- **Scope a defensive branch to what is reachable**: if a test can only reach a branch by lying about a dependency - deleting a method from a mock, writing a flag a real socket owns - the branch asserts compatibility with the pinned version, not behavior, and compatibility is verified once at upgrade time by the [pins checklist](docs/spec/DEPENDENCY_PINS.md), not at every review round forever. Degrade at runtime only where the degraded path is better than refusing to connect; otherwise validate the seam once at connect time and fail loudly with one actionable error, as `connect()` in `apps/cli/src/connection/ssh2SftpAdapter.ts` does when it loops over the required SFTP methods and throws. Such a branch earns a guard and one line in the pins upgrade checklist - not a graceful degraded path, a differentiated operator warning, a warn cadence, its own counter, or a delete-the-guard test, because no scenario exercises it and that test can only be pantomimed.
