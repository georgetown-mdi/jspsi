...
- **Sensitive-file parsing**: parse any operator config or credential file (`psilink.yaml`, `.psilink.key`, the signing identity) only through `apps/cli/src/sensitiveFile.ts`; the raw YAML/JSON parsers leak source bytes (an inline credential) into errors and stderr, and an ESLint rule forbids them outside that chokepoint. A genuinely non-sensitive parse opts out with a one-line justified `eslint-disable`.
