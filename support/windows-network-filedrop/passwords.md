...
**The detail below is for you or your security team if you want it. The two
points above are the whole of what you have to do.**
...
- **Your agency's monitoring probably recorded it.** Creating the volume puts
  the password on a command line once, and managed PCs record the commands run
  on them and forward them to a central log (Windows event 4688, Sysmon, or an
...
- **It is still there after you delete the volume.** Deleting frees the record
  but does not erase it, and it sits inside Docker's own large disk file --
...
at the end, deletes the volume and its settings file and no more. It does not
erase the copies listed above. Having the password changed, or the account
switched off, is the step that ends it -- and if IT issued the account, that
means going back to them. The
...
Then there may be no password to give, which is a failure with its own section:
