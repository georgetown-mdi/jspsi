...
- The web app can run as a single-party console appliance, driving the party's own `psilink` exchange (filedrop or SFTP) as a server-side subprocess so an operator runs and downloads a result without the command line. Off by default and gated behind a data root; when enabled it is loopback-only, refusing to start on a non-loopback bind. See `docs/DEPLOYMENT.md` and `docs/spec/SERVER_JOB_API.md`.
