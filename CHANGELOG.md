...
- `EncryptedMessageConnection` in `@psilink/core`: an application-layer AEAD decorator over `MessageConnection` that encrypts every outbound message and decrypts every inbound one with AES-256-GCM, keyed per direction from the SPAKE2 session key, so an SFTP/filedrop administrator sees only opaque ciphertext. Any integrity, replay, ordering, or format failure surfaces as a terminal `security`-kind `ConnectionError` rather than a silent transport drop. Not yet wired into the CLI exchange.
