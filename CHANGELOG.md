...
- Count-only (`psi-c`) exchanges are selectable and run end to end: both parties learn only the size of the intersection, subject to the agreed terms' output entitlement. The consent surfaces state three bounds beside that headline: each round also discloses how many distinct key values each party contributes, beside the record counts the terms exchange already carries; only the receiver computes the count, so the other party's copy is that party's report rather than an enforced fact; and the count bounds what psilink discloses, not what a partner that chooses its own input set can learn. See `docs/EXCHANGE_REFERENCE.md` and `docs/spec/PROTOCOL.md`.
