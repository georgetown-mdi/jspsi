...
- A catastrophic-backtracking (ReDoS) regular expression in a partner's linkage-term element transform (`replace_regex`, `extract_regex`, `filter_regex`, `split_on`) is rejected at terms validation, before any pattern runs, so a crafted pattern cannot hang the acceptor's browser tab or CLI process by backtracking over the full dataset. The screen is best-effort and fails closed, so a legitimate but unusually complex pattern may also be rejected; if one of yours is, simplify it. See `docs/SECURITY_DESIGN.md` and `docs/EXCHANGE_REFERENCE.md`.
