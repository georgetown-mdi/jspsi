...
- SFTP host-key pinning: set `connection.server.host_key_fingerprint` to the server's OpenSSH SHA256 fingerprint and every SFTP connection verifies the presented host key before authenticating, aborting on a mismatch; left unset, it warns and connects unverified. The unsupported `certificate` and `known_hosts` fields are now rejected at parse time. CLI `sftp` only. See `docs/SECURITY_DESIGN.md`.
