...
describe("require-clean-tree-for-review hook", () => {
  ...
  describe("the rounds ledger", () => {
    const light = (dispositions) => ({ kind: "light", dispositions });
    const fixed = [{ item: "a", disposition: "fixed" }];
    const unfixed = [
      { item: "a", disposition: "limit", note: "lives with it" },
      { item: "b", disposition: "deferred" },
    ];

    function writeLedger(main, branch, rows) {
      mkdirSync(join(main, "scratch", "review-rounds"), { recursive: true });
      writeFileSync(
        join(main, "scratch", "review-rounds", `${branch}.jsonl`),
        rows.map((row) => `${JSON.stringify(row)}\n`).join(""),
      );
    }

    it("refuses round 3 after a round that fixed nothing, and takes no lock", () => {
      const main = track(makeRepo());
      track(addWorktree(main, "feature"));
      writeLedger(main, "feature", [light(fixed), light(unfixed)]);
      const { status, stderr } = workflowIn(main, review("feature"));
      expect(status).toBe(2);
      expect(stderr).toContain("round 3 of 'feature' would follow round 2");
      expect(stderr).toContain("ownerCapRaise: true");
      expect(existsSync(lockFor(main, "feature"))).toBe(false);
    });

    it("sizes the refusal's advice from the diff against origin/staging", () => {
      const main = track(makeRepo());
      track(addWorktree(main, "feature"));
      execFileSync("git", [
        "-C",
      ...
      writeLedger(main, "feature", [light(fixed), light(unfixed)]);
      const { stderr } = workflowIn(main, review("feature"));
      expect(stderr).toContain("at 1 changed lines");
    });

    it("lets the owner's cap raise start that round", () => {
      const main = track(makeRepo());
      track(addWorktree(main, "feature"));
      writeLedger(main, "feature", [light(fixed), light(unfixed)]);
      const raised = review("feature", { ownerCapRaise: true });
      expect(workflowIn(main, raised).status).toBe(0);
      expect(existsSync(lockFor(main, "feature"))).toBe(true);
    });

    it("keeps a branch's first role round after a lens round that fixed nothing", () => {
      const main = track(makeRepo());
      track(addWorktree(main, "feature"));
      writeLedger(main, "feature", [light(fixed), light([])]);
      const role = review("feature", {
        role: "security-reviewer",
        claims: ["the parser refuses an oversized frame"],
      });
      expect(workflowIn(main, role).status).toBe(0);
    });

    it("admits round 3 after a round that fixed something", () => {
      const main = track(makeRepo());
      track(addWorktree(main, "feature"));
      writeLedger(main, "feature", [light(unfixed), light(fixed)]);
      expect(workflowIn(main, review("feature")).status).toBe(0);
    });

    it("refuses on a ledger it cannot read unless the owner raised the cap", () => {
      const main = track(makeRepo());
      track(addWorktree(main, "feature"));
      mkdirSync(join(main, "scratch", "review-rounds"), { recursive: true });
      writeFileSync(
        join(main, "scratch", "review-rounds", "feature.jsonl"),
        "not json\n",
      );
      const { status, stderr } = workflowIn(main, review("feature"));
      expect(status).toBe(2);
      expect(stderr).toContain("could not read the rounds ledger");
      const raised = review("feature", { ownerCapRaise: true });
      expect(workflowIn(main, raised).status).toBe(0);
    });
  });
