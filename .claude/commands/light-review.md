...
A base sync is not a fix: when the head moved by merging origin/staging into the branch
(a merge commit whose first parent is the last-reviewed head), the branch owns no verdict
at the merged head, so the delta rule does not apply -- the contract is the branch's
standing contract in full, every claim re-run at the merge head, and re-running its HOLDS
