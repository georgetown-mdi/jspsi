 ...
 * `outputFormat`. It holds every date component, so the default drops nothing.
 *
...
// `pad_left` fills a value UP to its declared length, so it fixes a minimum
// and leaves the maximum where it found it: a value already longer passes
// through at its own width. It therefore raises a bound the chain already
// holds and derives none on its own. The guards mirror `padLeftFactory`, whose
// invalid params throw before any row runs.
...
/**
 * The most characters an element's standardized value can hold once its
 * `transform` chain has run, or `undefined` when the agreed terms bound it to
 * no width at all.
 *
 * Both parties read the same terms, so both derive the same number; it is what
 * lets a fuzzy element declare a ceiling tighter than the global expansion
 * limit (`fuzzyCandidateCeiling`, fuzzyComparisons.ts).
 *
 * The chain is walked IN ORDER and the last step to fix a width governs, since
 * each step is handed the previous one's output:
 *
 * - `substring`, `phonetic`, and `parse_date` fix a width from their declared
 *   params alone, whatever they are handed, so each one REPLACES the width it
 *   received.
 * - `pad_left` fills up to its length and truncates nothing, so it raises a
 *   width already held and fixes none by itself.
 * - `null_if` and `filter_regex` return the value they were handed or drop it,
 *   so they pass a width through unchanged.
 ...
 *   A step is only classified above when the params alone fix what it emits,
 ...
 *   position, an upper-casing expands the code points that case-fold to two,
 *   and a Unicode normalization is not length-preserving either.
 *
 * Deriving no width is always safe -- the consumer falls back to the global
 * cap, which is the bound that held before any of this was read -- while
 * deriving one too small would refuse an honest row at
 * `assertDeclaredWidthMatchesStrategy` (exchange.ts), which is why the default
 * is to clear.
 *
 * A width this DERIVES is never below 1. Params that would fix 0 -- a
