...
interface KexVectors {
  ...
  wire: {
    msg1: { kexMsg: "1"; e: string };
    msg2: { kexMsg: "2"; e: string; confirm: string };
...
test("known-answer vector: computeKexKeys reproduces the session key, confirmation key, and both tags", async () => {
  ...
  const k = await computeKexKeys(psk, eInitPub, eRespPub, dh);
  expect(toHex(k.chainingKey)).toBe(vectors.derived.chainingKeyHex);
  expect(toHex(k.handshakeHash)).toBe(vectors.derived.handshakeHashHex);
  expect(toHex(k.sessionKey)).toBe(vectors.outputs.sessionKeyHex);
  expect(toHex(k.confirmKey)).toBe(vectors.outputs.confirmKeyHex);
  expect(toHex(k.initiatorConfirm)).toBe(vectors.outputs.initiatorConfirmHex);
  expect(toHex(k.responderConfirm)).toBe(vectors.outputs.responderConfirmHex);
});
...
test("known-answer vector: the wire base64url encodings match", () => {
  ...
  ).toBe(vectors.wire.msg1.e);
  expect(
    toBase64Url(fromHex(vectors.derived.responderEphemeralPublicHex)),
  ).toBe(vectors.wire.msg2.e);
  expect(toBase64Url(fromHex(vectors.outputs.responderConfirmHex))).toBe(
    vectors.wire.msg2.confirm,
  );
  expect(toBase64Url(fromHex(vectors.outputs.initiatorConfirmHex))).toBe(
    vectors.wire.msg3.confirm,
  );
});
...
async function runPair(
  ...
  return Promise.allSettled([
    runKex(connA, "initiator", pskA),
    runKex(connB, "responder", pskB),
  ]);
}
...
test("the handshake times out if the peer never responds", async () => {
  ...
  await expect(runKex(conn, "responder", PSK_A)).rejects.toThrow(
...
test("runKex rejects when the psk is not 32 bytes", async () => {
  ...
  await expect(runKex(connA, "initiator", new Uint8Array(31))).rejects.toThrow(
...
test("responder sends abort when the initiator's msg1 is malformed", async () => {
  const [connA, connB] = createMessagePipe();
  const responder = runKex(connB, "responder", PSK_A);
  responder.catch(() => {});
  await connA.send({ kexMsg: "1", e: "not-base64url!!" });
...
test("initiator sends abort when the responder's msg2 is malformed", async () => {
  const [connA, connB] = createMessagePipe();
  const initiator = runKex(connA, "initiator", PSK_A);
  initiator.catch(() => {});
  const msg1 = (await connB.receive()) as { kexMsg: string };
  expect(msg1.kexMsg).toBe("1");
  await connB.send({ kexMsg: "2", e: "not-base64url!!", confirm: "AA" });
...
// DH output, and the keys both parties would derive honestly.
async function fakeResponderUpToMsg2(
  conn: ReturnType<typeof createMessagePipe>[1],
  psk: Uint8Array<ArrayBuffer>,
) {
  const msg1 = (await conn.receive()) as { kexMsg: string; e: string };
  expect(msg1.kexMsg).toBe("1");
  ...
  const keys = await computeKexKeys(psk, eInitPub, eRespPub, dh);
  return { eInitPub, eRespPub, keys };
}
...
test("a reflected confirmation (the initiator's own role label) does not verify", async () => {
  ...
  const initiator = runKex(connA, "initiator", PSK_A);
  initiator.catch(() => {});
  const { eRespPub, keys } = await fakeResponderUpToMsg2(connB, PSK_A);
...
test("tampering the initiator's public key breaks confirmation (responder side)", async () => {
  ...
  const responder = runKex(connB, "responder", PSK_A);
  responder.catch(() => {});
  // Hand-rolled initiator.
  const eph = x25519.keygen();
  const eInitPub = toBytes(eph.publicKey);
  await connA.send({ kexMsg: "1", e: toBase64Url(eInitPub) });
  const msg2 = (await connA.receive()) as { kexMsg: string; e: string };
  ...
  // Confirm over a flipped e_i (same DH and e_r): only the initiator public key
  // bound into the transcript differs, so the responder rejects.
  const flipped = Uint8Array.from(eInitPub);
  flipped[0] ^= 0x01;
  const keys = await computeKexKeys(PSK_A, flipped, eRespPub, dh);
  await connA.send({
    kexMsg: "3",
    confirm: toBase64Url(keys.initiatorConfirm),
  });
