import { createServerFileRoute, setResponseHeader, setResponseStatus } from '@tanstack/react-start/server';
...
export const ServerRoute = createServerFileRoute('/api/psi/join').methods({
  POST: async ({ request }) => {
    ...
    if (sessionManager.has(sessionId)) {
      ...
      if (Date.now() > session.timeToLive.getTime()) {
        ...
        return 'session may have expired; try again later';
      }
