...
No personal data is collected, transmitted, or retained by the project on its own behalf. [PRIVACY.md](../PRIVACY.md) is the authority on that answer, and holds the repository check that catches drift in it; this document takes it as given and states who operates what.
...
- **A third party sits in front of it.** The hosted deployment is served through Cloudflare, which terminates the public TLS session and holds its own request log of everything it forwards, under Cloudflare's retention rather than the project's ([PRIVACY.md](../PRIVACY.md#hosted-web-application)).
...
| Identity and access | No accounts, no registration, no cookies, and no login of any kind. Exchange authentication is the same authenticated key exchange the container runs. | Who may use the device and the browser profile, and sending the invitation -- which contains the secret that authenticates the exchange -- over a trusted channel ([SECURITY_DESIGN.md](SECURITY_DESIGN.md#invitation-contents-and-confidentiality)). |
| Encryption in transit | Serves the application, and the coordination server that brokers the browser-to-browser connection, over HTTPS on both legs: the browser's TLS session terminates at Cloudflare, and the leg from there to the project's own terminator is encrypted as well. The exchange itself is end-to-end encrypted under WebRTC DTLS, which neither Cloudflare nor the coordination server terminates ([SECURITY_DESIGN.md](SECURITY_DESIGN.md#channel-security)). | The network path from its own endpoints to the coordination server and to the partner. Choosing a relay, if it uses one: the browser takes its TURN and STUN servers from the operator's own relay setting, kept in browser storage, and with none uses a fixed STUN set and no TURN entry ([COMMUNICATION.md](COMMUNICATION.md#stunturn)). A relay forwards encrypted DTLS packets without terminating the session. |
| Encryption at rest | No agency data is stored on project infrastructure, so there is nothing there to encrypt. A managed (recurring) exchange persists its record -- including the rotating shared secret -- in browser storage on the operator's own device, where the at-rest posture is weaker than the CLI's owner-only key file by design: the store has no owner-only mode, and any script running in the origin can read it ([SECURITY_DESIGN.md](SECURITY_DESIGN.md#hosted-at-rest-threat-model-for-managed-exchanges)). | Device-level protection for that storage: full-disk encryption, browser-profile hygiene, and deleting the managed exchange when the partnership ends ([MANAGED_EXCHANGE.md](MANAGED_EXCHANGE.md#deleting-a-managed-exchange)). |
...
| Data retention and disposal | Holds the web server request logs and the coordination server's connection metadata named under [Hosted web application](#hosted-web-application) above, and keeps them for 90 days. No exchange content, no input data, and no record of what was linked. The window covers what its two mechanisms reach -- the instance's own copies, rotated daily under a 90-day maximum age ([DEPLOYMENT.md](DEPLOYMENT.md#log-retention-on-the-instance)), and the copies streamed to CloudWatch Logs, expiring under that log group's 90-day retention -- and is claimed over no class on a different clock: not the health agent's log, which holds client addresses and request paths under a 7-day retention; not the deployment logs in the deployment bucket; and not Cloudflare's request logs, kept under Cloudflare's own retention. [PRIVACY.md](../PRIVACY.md#hosted-web-application) sets them out class by class. Verified 2026-09-17, by reading the CloudWatch retention setting recorded for the deployed environments and by the deployment run that installed the rotation bound. | The input file, the result file, and any managed-exchange record on the device. Deleting the managed exchange removes its stored record ([MANAGED_EXCHANGE.md](MANAGED_EXCHANGE.md#deleting-a-managed-exchange)). |
...
| TURN relay | Whoever the agency configures: in the container deployment's connection block, or in the browser's own relay setting for the web application; commonly a commercial ICE service. The project operates none | Each party's network address, whether or not traffic is relayed, and the traffic volume between them. It forwards encrypted DTLS packets without terminating the session. |
| Shared SFTP server or file drop | One of the two parties, or a third party both trust | The exchange's files. On a recurring, authenticated exchange: ciphertext and file timing, plus the rendezvous files, key-exchange handshake frames, and abort marker that sit outside the application-layer wrap and hold no exchange data ([COMPLIANCE.md](COMPLIANCE.md#nist-sp-800-53), SC-8). On a zero-setup exchange: readable content, since it relies on the transport alone. |
...
**What a relay's operator learns.** Whoever runs a relay learns the network address of each party to every exchange that uses it, whether or not any traffic is relayed: a party's software reserves an address on the relay while it gathers connection candidates, before it knows whether a direct connection will work. The relay forwards encrypted DTLS packets without terminating the session and cannot read the exchange.
