...
The names in that display are the inviter's own file's headers, so every surface of the step that shows one -- the grid's row headers and control labels, the chip list, the summary a screen reader hears, the exchange ledger's row stating what will be sent, and the check-your-answers restatement of that set -- shows it verbatim inside a bidi isolate rather than escaped: the same treatment, for the same reasons, that the acceptor's confirm-columns step gives the same names (see [Confirm your columns](#confirm-your-columns)). The seam is the consent preview and other surfaces derived from the invitation summary, which show the same names escaped, as everything on those surfaces is.
