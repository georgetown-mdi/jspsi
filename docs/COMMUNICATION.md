...
The inviter chooses a CSV and enters its name. Composing fails closed before any setup secret is minted: a file that cannot be read, or whose columns satisfy none of the linkage keys, produces no invitation (the same zero-key condition the acceptor's pre-flight blocks on). Once the invitation is generated the inviter moves to the shared exchange screen, which leads with a share block -- the link and code to send out-of-band, and the expiry -- above the proposed terms summary, and waits for the acceptor to connect. On connect the share block disappears and Status shows the run underway.

The quick path also shows, once a file is chosen, which columns it will send to the partner for matched rows, with a one-click path into Advanced options to change what is disclosed. This is awareness only, not a gate: the displayed set is derived from the same predicate the exchange transmits on (so it cannot drift from what leaves the machine), and no column the quick path sends stops being sent because of it. When it would send no columns it says so explicitly. Those disclosed columns are also declared in the invitation's payload data dictionary, so the partner's consent screen lists them as columns it will receive.

### Advanced options

"Advanced options" opens a column-aware editor seeded from the file-derived defaults (never a blank form), so it requires the CSV up front. It renders a live preview using the same component the acceptor's consent screen uses, so the inviter authors against the exact wording the partner will see. Validation runs through the linkage-terms schema the exchange enforces; generating is blocked until the terms are valid and at least one key is satisfiable by the inviter's columns. The inviter can set:

- its name and the invitation lifetime;
- an optional legal agreement (the partner must enter it identically);
- a column metadata grid mapping each column to a semantic type and one consequence-labeled disclosure choice (used to match, row identifier, sent to your partner, or ignored);
- which linkage keys are active and in what order (keyboard-operable, with a per-key indicator of whether the inviter's columns satisfy it);
- who receives the matched results -- both parties, only the inviter, or only the partner (the exchange honors one-sided output end-to-end; the forbidden "neither receives" combination is not offered);
- the linkage strategy, defaulting to `cascade`; choosing `single-pass` surfaces at the point of choice the disclosure tradeoff it carries (fewer round trips over a high-latency channel in exchange for disclosing more) and mints terms carrying that choice, which both parties must agree on or the exchange aborts. The disclosure mechanics and round-trip accounting are in [PROTOCOL.md](spec/PROTOCOL.md).

Per-field data cleaning is shown by default here (not behind expert mode), since cleaning is a normal part of preparing for the exchange: the inviter authors each field's cleaning steps -- raw regular-expression steps included, each marked "advanced" -- and chooses which column feeds it, with a whole-file coverage warning and a malformed-step gate that blocks generation, matching the acceptor's "Prepare your data" editor (below).
...
An attached legal agreement is placed last in the core as a pre-consent governance checkpoint, surfaced whole (its reference, purpose, and expiry) rather than as a bare flag, because the purpose is the compliance-load-bearing field a disclosure accounting turns on (HIPAA 45 CFR 164.528, FERPA; see [COMPLIANCE.md](COMPLIANCE.md)). Each tier is a labelled, assistive-technology-navigable group.
...
The acceptor's own outbound disclosure -- the columns it will send for matched records -- is forward-referenced always-visible here, since before a file is chosen the exact set is not yet known: a fixed-copy line states that a disclosure is coming and the columns are confirmed later, in "Prepare your data". Once a file is chosen the actual send list replaces it.

### Consent gate

On the same screen the acceptor gives affirmative consent (a checkbox plus its name) and chooses its own CSV -- or finds one it dropped on the home page carried over and pre-filled. Either way, both consent and a name are required before the file is parsed, so carrying a file forward never pre-empts the consent gate.
...
The parsed file opens in a "Prepare your data" editor. A grid maps each column to a semantic type and one consequence-labeled disclosure choice (used to match, row identifier, sent to your partner, or ignored), backed by a running summary of exactly which columns will be sent, and a live verdict of how many agreed linkage keys the current mapping satisfies. The former zero-key dead-end is now this editor's entry rather than a bounce to the file picker: the hard block remains -- an exchange that could match nothing cannot launch -- but the operator remaps columns in place. A column inferred as a row identifier is not sent by default.

Beside the grid, a per-field standardization workbench presents the recommended cleaning as an editable, ordered list of steps (typed-parameter inputs, including raw regular-expression steps marked "advanced") against a live before-and-after preview over a sample of the operator's rows. Two whole-file checks guard failures the sample cannot show:
