...
The names in that display are the inviter's own file's headers, so the grid's row headers and control labels, the chip list, the summary a screen reader hears, the exchange ledger's row stating what will be sent, and the check-your-answers restatement of that set each show one inside a bidi isolate rather than escaped, whole up to the display cut: the same treatment, for the same reasons, that the acceptor's confirm-columns step gives the same names (see [Confirm your columns](#confirm-your-columns)). Two seams: the single-identifier demotion notice on the same step composes its names without the treatment, and the consent preview and other surfaces derived from the invitation summary show the same names escaped, as everything on those surfaces is.
