...
Column names are the operator's own file's, so the step shows them verbatim everywhere it names one -- the grid row, its controls, and the notices beside it -- each inside a bidi isolate, so a header cannot reorder the copy around it and one name reads the same in every place the screen puts it. They are not escaped the way partner-controlled text is: escaping would show a legitimate accented or non-Latin header as a run of escapes on the operator's own authoring surface, and would collapse two long headers sharing a prefix into one truncated string. The boundary is that a header differing from another only by a homoglyph or a zero-width character reads alike here; the names come from the operator's own file, so what that costs is legibility of their own header rather than any disclosure.
