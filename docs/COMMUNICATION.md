...
Column names are the operator's own file's, so the step shows them verbatim everywhere it names one -- the grid row, its controls, the notices beside it, and the ledger row naming what will be sent -- each inside a bidi isolate, so a header cannot reorder the copy around it and one name reads the same in every place the screen puts it. A header too long to paint is the one exception, cut and marked as such in each of those places alike -- a bound on the display rather than a verdict on the exchange, so a header can read whole here and still be refused when the payload is sent. They are not escaped the way partner-controlled text is: that escape exists for text a partner chose, and what not escaping costs falls on the operator's own header -- its legibility, and on this screen a crafted twin pair of headers they could mark the wrong way (the tradeoff and its boundary are documented in `apps/web/src/components/ColumnName.tsx`).
