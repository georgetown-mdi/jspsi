...
The first way is a durable send: the send does not complete until the message is committed somewhere that outlives the connection. The file-based channels - SFTP and file-drop - work this way. A send writes the message as a file in the shared directory and does not resolve until that write (and its atomic rename into place) is durable, so the frame exists independently of the connection. Tearing the connection down cannot lose it; the peer reads it on its next poll whether or not the sender is still connected. These channels need no special teardown handling, and their close does not flush anything - the durability of the send is the guarantee.

The second way is a flushing close: the send only buffers locally, so a message can still be in flight when the sender finishes, and the close is what protects it. WebRTC works this way. A send hands the message to the data channel and returns before it is on the wire, so the final frame is guaranteed not by the send but by the close: a clean (non-error) close must flush the buffered frames before tearing the connection down, and the underlying reliable, ordered data channel delivers them ahead of the close signal the peer acts on. A close caused by an error does the opposite - it discards buffered data, because an errored link is already unusable.

The WebRTC case is the one that looks like a race when reviewed in isolation: the last operation a party performs is a non-blocking send immediately followed by a close, which appears to drop the frame. It does not, provided the close flushes - so a clean WebRTC close must not be shortcut into an abrupt connection teardown that bypasses the flush. The file-based channels have no such hazard, precisely because their send is already durable; the apparent race exists only for a buffering transport.

Any new channel added to the project must satisfy one of these two conditions - a durable send, or a flushing clean close. A transport that does neither will silently drop the final frame of an exchange. The same rule is recorded for implementors in the `Connection` and `TransportHooks` interface documentation in the core library.
