...
Marks that disagree with the payload set the invitation declares for this party block it too. An invitation may declare which columns the inviting party accepts, and the exchange refuses to run unless the marks name exactly that set, so the step states the disagreement beside the marks that decide it, names the columns at issue, and holds the launch until the two agree. Each direction carries the remedy that fits it, and both are stated at once when both hold:
...
- a column the declaration names that this file does not send takes a corrected invitation or a different file. Where the file does have that column, setting it to be sent is offered as a second option rather than the fix, since it widens what the operator discloses and takes the column out of matching.

A declaration that accepts no column at all is the same refusal with no local widening to offer: the remedies are re-marking those columns or an invitation that accepts them. The command line tool reports that accepts-nothing case on its own acceptance path (see [CLI.md](CLI.md)).
