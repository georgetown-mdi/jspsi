...
Both inform rather than block. Constraint violations are also surfaced but not enforced (the application warns on constraints, it does not block on them). What does block launching is a malformed or incomplete cleaning step, alongside the satisfiability and single-identifier checks -- the same gate applied on the inviter's side. A disclosure the invitation will not accept blocks it too: where the invitation declares the inviting party accepts no payload column and is entitled to the matched result, columns still marked to send are a pair the exchange refuses to run on, so the step names them and holds the launch until they are re-marked or a corrected invitation arrives. The CLI reports the same disagreement on its own acceptance path (see [CLI.md](CLI.md)).
