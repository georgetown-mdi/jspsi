...
"Reached the peer" is the requirement, and it is stronger than emptying a local buffer. A sender that stays alive can meet it by ordering alone: the reliable, ordered channel delivers everything already sent ahead of the in-band close signal the peer acts on, so the close needs only to emit that signal - which is what the web application does, and why its close returns without tearing anything down. A sender that must exit, as a CLI process must, cannot lean on the peer's own teardown and has to wait for the peer to acknowledge the data first. It must not wait on the channel's buffered-byte counter to reach zero: that counter is not a delivery signal, and closing on it loses frames (see [WEBRTC_TRANSPORT.md](spec/WEBRTC_TRANSPORT.md#the-clean-close), which specifies both halves).
