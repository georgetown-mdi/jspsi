...
- A browser inviter whose answer to the replaced offer arrives after the overlap has ended is not followed: the acceptor refuses that answer, the browser does not answer the new offer, and that run fails at its own rendezvous timeout. A CLI inviter follows the new offer.
