...
payload-disclosure commitments its fields carry and the run-time gates that
...
minted file cannot contain; the commitments it does carry, and what enforces
...
Four fields carry a party's payload-disclosure commitments, and every one of them
...
### Receive-side runtime lock-in (`reconcileReceivedPayload`)
...
top-level field is deliberately distinct from `payload.receive` so it does not
...
Two cases are deliberately **not** a mismatch: an absent expected set (the lazy
...
- **Invite/accept.** The inviter publishes its disclosed subset on the token and
  ...
  carrying no subset *removes* the field rather than leaving a set this
...
- **Recurring.** Both parties' persisted configs carry the lock-in, so each
...
The receive-side lock-in's counterpart, holding a *present* `payload.send` to
...
the inviter's `payload.receive`, so an empty one carries the partner's
...
has mirrored the partner's `expectsOutput` -- so it settles the coherence of this
...
The runtime lock-in above catches a partner that under- or over-delivers, but it
...
The error carries a **dual** remedy -- restore the column's metadata to transmit,
...
the pre-record laziness, stated as a limit: a mint over a config carrying no
...
records -- is sent no payload. The receive side fails closed as a backstop: a
