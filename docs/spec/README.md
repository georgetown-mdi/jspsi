...
- **A container image's npm/Docker freeze, base-image digest pin, OS-package install line, the FIPS variant's pin table, or a runtime posture measured on a built image (its writable set, its setuid/setgid inventory)** -> [CONTAINER_IMAGES.md](CONTAINER_IMAGES.md).
