...
- ssh2 builds its key-exchange offer once at module load and runs NO capability probe over it, unlike its host-key formats: on a host whose OpenSSL provider does not offer X25519 it still advertises `curve25519-sha256@libssh.org` and `curve25519-sha256`, wins the negotiation against a server that offers them, sends `KEXECDH_INIT`, and only then dies on `crypto.generateKeyPairSync('x25519')`. `apps/cli/src/connection/sftpKexCapability.ts` subtracts the unperformable algorithms from that offer, and rests on four measured properties of the pinned `ssh2`, all decided by reading the client's own `SSH_MSG_KEXINIT` off the wire rather than from its source. (1) `algorithms.kex` accepts a MODIFIER object as well as an explicit list -- the `append`/`prepend`/`remove` shape `@types/ssh2` declares as `AlgorithmList<T>` -- and `remove` matches a `RegExp` entry as well as a string, mid-string and honouring the `i` flag, which is what lets psilink express "ssh2's own offer minus everything built on X25519" without enumerating a key-exchange list of its own. (2) `remove` is applied AFTER `append`/`prepend`, so merging psilink's removal into an operator's modifier cannot be undone by the entry beside it. (3) An EMPTY explicit list is read as *unspecified* and falls back to ssh2's full defaults -- so an operator list filtered to nothing must be REFUSED, never forwarded, or the refusal would restore the very algorithms it rejected; a value that is neither a list nor a modifier (a bare string, `null`) is likewise inert and falls back to the defaults, as is a list that ARRIVES empty -- which drops nothing, so it is not the refusal's case but the inert one, and takes the removal modifier ssh2's own reading of it amounts to. (4) ssh2 appends `ext-info-c` and the Terrapin strict-key-exchange marker `kex-strict-c-v00@openssh.com` to whatever list is offered, and `remove` does not reach them, so constraining the offer cannot downgrade the handshake. A key-exchange negotiation that then finds nothing in common surfaces through ssh2-sftp-client as `getConnection: Handshake failed: no matching key exchange algorithm` -- ssh2's structured `level: "handshake"` does NOT survive that wrapper, so the message fragment is what there is to match on, exactly as for `Host denied`; matching it only ADDS a diagnostic naming the missing primitive, so a reworded message degrades to ssh2's bare text rather than to a wrong one. The fragment is not ssh2's own text alone: ssh2 renders a server's `SSH_MSG_DISCONNECT` description into that same message, so a server -- or an on-path attacker, since a disconnect precedes host-key verification -- writes the fragment verbatim and reaches the diagnostic at a stage where it does not apply (measured). That buys an adversary nothing, because a server reaches the identical diagnostic with NO message control by offering only algorithms built on the missing primitive; the residual is a misattributed diagnostic, bounded by the diagnostic's message being composed from constants, with the server's own text one cause link down where the display sink escapes it once. `apps/cli/test/integration/sftpKexOffer.test.ts` drives all of it against the installed ssh2 -- each offer shape (no operator `algorithms.kex`, an explicit list, a modifier whose `append`/`prepend` names an unperformable algorithm, and a list that arrives empty) decoded off the wire with the capability verdict forced, both appended markers surviving every one of them, the empty-list and non-list fallbacks measured against bare ssh2 rather than assumed, the constrained offer negotiating against the suite's real server on three of the four dial paths (a direct connect, core's host-key probe, and a recovery re-dial after a server-cut session, the last two read through a recording relay that also holds the re-dial's offer identical to the first dial's), the no-common-algorithm refusal against a real `ssh2.Server` restricted to `curve25519-sha256`, and a listener that writes the failure description itself, holding its bytes out of the diagnostic -- so a bump that changed any of the four fails it red. Its limits: the fourth dial path, the `connection_per_poll` cycle-start reconnect (`reestablishSession`), is not driven -- what covers it is the constraint sitting at `connectLocked`, the single path to ssh2's `connect`, rather than an enumeration of callers, and it re-dials from the same retained, already-constrained options; the capability verdict is forced rather than produced by a FIPS host, because the only lever this image has is `crypto.setFips(true)`, which is not a model of one -- measured on node 26.5.1 / openssl 3.5.7 it leaves OpenSSL with NO usable algorithms (`createHash('sha256')`, `createCipheriv('aes-128-gcm', ...)` and `generateKeyPairSync('ec', {namedCurve: 'prime256v1'})` all throw `error:0308010C:digital envelope routines::unsupported`), where a real provider keeps those and drops only X25519; and the no-common-algorithm refusal is driven only against an in-process `ssh2.Server`, never a native `sshd` -- the one native profile whose server would provoke it, `restricted-crypto`, accepts only `curve25519-sha256`, exactly what the forced verdict withholds, so the two dials that must COMPLETE a handshake skip that leg while every other native leg runs them.
...
- `node_modules/ssh2/lib/protocol/constants.js` / `protocol/kex.js`: the key-exchange offer's premises are DRIVEN rather than read (`apps/cli/test/integration/sftpKexOffer.test.ts` decodes the client's own `SSH_MSG_KEXINIT`), so read these only to explain a failing case -- confirm `algorithms.kex` still takes the `append`/`prepend`/`remove` modifier with `remove` applied last and matching a `RegExp`, that an empty explicit list still falls back to `DEFAULT_KEX`, that `ext-info-c` and `kex-strict-c-v00@openssh.com` are still appended outside that filtering, and that a negotiation with nothing in common still throws with a message containing `no matching key exchange algorithm`.
