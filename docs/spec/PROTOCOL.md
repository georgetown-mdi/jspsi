...
**Operation semantics.** `replace_regex` replaces every match (global), with `$n`, `$&`, and `$$` in the replacement carrying their JavaScript meanings and an unrecognized `$n` emitted literally. `extract_regex` yields the first capturing group, or the whole match when the pattern has none, or `null` on no match or an empty result. `filter_regex` tests for a match anywhere in the value. `split_on` splits on the delimiter; unlike JavaScript `String.prototype.split`, capturing groups in the delimiter are NOT emitted as output elements, and the function discards empty parts.

**Versioning.** The dialect is bound to the linkage-terms [`version`](../EXCHANGE_REFERENCE.md#linkage-terms): a change to the permitted syntax, the flag set, the Unicode mode, or the engine's match/replace/split semantics is a wire-compatibility event that MUST accompany a `version` change, so two implementations can never silently disagree on whether a pattern is valid or what it means.
