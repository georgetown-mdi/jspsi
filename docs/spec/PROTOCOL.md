...
**Engine and semantics.** Patterns MUST be evaluated under an RE2-style engine: a Thompson NFA / lazy DFA that performs leftmost-first matching in time linear in the input length, with no backtracking. The reference implementation uses `re2js` (a pure-JavaScript RE2 port), the same build in the CLI (Node) and the web (browser). Matching is over Unicode code points of the NFC-normalized value. No flags are set: matching is case-sensitive, `.` matches any code point except newline, and `^` and `$` anchor the start and end of the whole value (not lines); inline flag groups (`(?i)`, `(?s)`, `(?m)`) within a pattern are permitted. The predefined classes `\d`, `\w`, and `\s` use their ASCII definitions, as JavaScript's do without the `u` flag.
