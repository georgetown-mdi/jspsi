...
Ahead of that algorithmic ceiling, the web app applies a conservative CSV intake cap of 100 MB (`MAX_CSV_FILE_BYTES`, `apps/web/src/components/csvIntake.ts`) - roughly one to two million identifier rows. This is a per-tab memory budget for *reading, parsing, and hashing the file in the browser* before any exchange begins, not a protocol limit, and it is deliberately tighter than the comparison-step figure above because intake holds the raw file text, the parsed rows, and the derived payload at once. It is independent of the wire format. The file is parsed off the main thread and accumulated across PapaParse chunks, so a file that spans more than one `Papa.LocalChunkSize` chunk yields its complete row set rather than the silently truncated final chunk (which, on a record-linkage input, would be a wrong intersection with no error). The resulting payload then rides the WebRTC data channel under PeerJS's own message chunking, which already fragments and reassembles payloads far larger than a single frame at any cap in this range - so the cap is bounded only by browser memory and carries no transport or wire-format consequence. Tune it against that memory budget alone.
