...
| `subdirectory` | The folder the exchange will run in opened, and how many entries it holds. With no subdirectory configured this is the share root, counted the same way. A folder already holding more entries than the transport's directory-listing bound is a `warn` whose action is a folder dedicated to the exchange (see [CHANNEL_SECURITY.md](CHANNEL_SECURITY.md#directory-listing-bound)). The captured output allows each entry line room for a full-length name, but a listing can still be cut short of the bound -- by lines longer than that allowance -- so a listing too long to capture whose counted entries are within the bound is a separate `warn`: the count is reported as a lower bound, the `meaning` states that the listing could not be captured in full, and the `action` asks the operator to confirm the folder is within the bound or use a dedicated one. |
| `free_space` | The free space the server reported for the share. Below 100 MB, or none at all, is a `warn`; a server that reports no figure, or a listing too long to capture through to the figure after its last entry, makes this `skipped`. |
...
| `write_rename` | A file was written under a temporary name and renamed into place, the shape Alcove writes every message with. A write or rename that fails is a `fail`, and so is one of its two working names, `.alcove-w.tmp` and `.alcove-w2.tmp`, left in the directory where the run cannot remove it first. Either `fail` stops the run, leaving the checks after it `skipped`. |
| `exclusive_create` | The share refuses to create a file that already exists -- the `O_EXCL` refusal Alcove's rendezvous uses to decide which side goes first. Only an "already exists" error (`EEXIST`) on the second create counts as that refusal. A share that does not refuse it is a `warn`; a share where the create could not be tested at all -- the first create failed, or the second failed with any other error -- is `skipped`, with an `action` naming the same remedy. A working name left in the directory that the run cannot remove first is a `fail`: the check never ran, and the name has to be cleared by hand. |
...
The `fail`s of `exclusive_create` and `rename_onto_existing` are the occupied-working-name case, and it is the only `fail` either of those checks emits. Their three names are fixed -- `.alcove-x.tmp` for `exclusive_create`, `.alcove-a.tmp` and `.alcove-b.tmp` for `rename_onto_existing` -- and each is removed before its check runs; a name still present after that attempt puts `overall` at `fix_and_retry`, never `fatal`, and unlike the `write_rename` case the checks after it still run.
...
A local fault that stops `doctor probe` before it can compose a verdict -- the temporary directory or the credentials file in it cannot be created or restricted to its owner, or a working file cannot be written locally -- also prints no verdict on either stream, and exits 69: something the checks depend on was not available, the same class a `fatal` verdict reports. A caller that sees 69 with nothing on stdout treats it as `fatal`.
