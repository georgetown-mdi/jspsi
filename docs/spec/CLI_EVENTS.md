...
Emitted for each non-fatal warning: the terms-exchange warnings mirroring `onWarning`, the cross-party host-key divergence notice -- a security signal a supervisor that discards stderr would otherwise never see -- the pre-exchange signing-without-a-record notice below, the failed-record notice of a terminated run below, and one per **persistence loss**, the class defined below (an audit artifact the run was asked for and could not produce, a configuration or consent record an online `invite`/`accept` could not write, or the configuration and key a zero-setup `--save` could not write). A warning does not end the run, and the terminated-run notice is the one exception to that reading: it accompanies a run that is already failing, and it is the failure's own `error` event that ends it.
...
A run that disclosed and then terminated in the signed-receipt swap still writes the self-attested record of that disclosure ([PROTOCOL.md](PROTOCOL.md#self-attested-record)). A failure to write it emits a `warning` naming the destination, exactly as the completed-run write does, and a record that could not be built at all emits the other shape, so a disclosure that occurred always leaves either its record or a notice of that record's absence. Both are deliberately **not** a [persistence loss](#persistence-loss) and leave the exit code alone. That class means "the exchange succeeded, do not re-run it", and its code says so; this run failed and exits on its own fault, which is the opposite instruction and the one the operator needs. The terminal event stays `error`, and the warning precedes it like every other.
