...
| `reconnects` | integer | Connection re-establishment attempts over the run: connect-time dialing retries past the first attempt (the dialing-retry loop bounded by `max_reconnect_attempts`), plus every re-establishment that followed an UNEXPECTED session loss (SFTP only; in the default held-session mode the cumulative number of those is capped by the same `max_reconnect_attempts` value, in `connection_per_poll` mode it is not). A re-establishment that follows a DELIBERATE lifecycle transition is not counted -- neither the `connection_per_poll` idle-boundary release nor teardown (the abort-marker write and the terminal-frame drain), since nothing was lost in either case. The exemption has exactly one exception, and reaching it takes a change in the pinned `ssh2` (see [DEPENDENCY_PINS.md](DEPENDENCY_PINS.md)): when the idle-boundary release cannot read whether it ended the transport -- the client socket no longer exposing `writableEnded` -- it withholds the exemption rather than claim a release it cannot confirm. That makes this field reachable from an idle-boundary release, on one route only: an operation ISSUED in the idle gap, ahead of the poll cycle's own re-establishment, finds no session, and the re-dial that recovers it is counted here and warned as a mid-exchange drop even though the release is what ended the session. Neither the release nor the cycle-start re-establishment that follows it counts anything by itself, so a run in which nothing is issued in that gap still reports `0` on this path. A reported drop that did not happen is recoverable for whoever reads the number; a real drop silently exempted is not. The release warns on that path as well, so a count reached through it is never unexplained. `0` when none occurred, which is what a healthy run reports at any poll interval in either mode. |
