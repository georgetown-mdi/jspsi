...
| `reconnects` | integer | Connection re-establishment attempts over the run: connect-time dialing retries past the first attempt (the dialing-retry loop bounded by `max_reconnect_attempts`), plus every re-establishment that followed an UNEXPECTED session loss (SFTP only; in the default held-session mode the cumulative number of those is capped by the same `max_reconnect_attempts` value, in `connection_per_poll` mode it is not). One increment per session LOST, not per operation the loss interrupted: a drop tears every operation the transport has on the wire, so a concurrent fan of them recovers over one re-established session and moves this field by one. A re-establishment that follows a DELIBERATE lifecycle transition is not counted -- neither an idle-boundary release that was itself what ended the session in `connection_per_poll` mode nor teardown (the abort-marker write and the terminal-frame drain), since nothing was lost in either case; a release that closed over a transport a partner's drop had already ended is not one of those, so an operation that drop tore is counted here like any other unexpected loss. Where that drop tore nothing it adds nothing here: the boundary falls after it with the wire empty, so what follows is the release's own re-establishment rather than a recovery, and the forced close the release drives reports the boundary as an operator log line rather than a machine counter. `0` when none occurred, which is what a healthy run reports at any poll interval in either mode, with two exceptions, both a session `connection_per_poll` holds across an idle stretch rather than releasing at a boundary. The first is the rendezvous that precedes the loop, which holds one session across its wait for the partner, so a run whose rendezvous outlives the partner's session cap re-dials to survive it and counts those re-establishments. The second is the idle-boundary release's own precondition: a boundary reached with an operation the adapter has ISSUED and not settled is held rather than closed (see [CHANNEL_SECURITY.md](CHANNEL_SECURITY.md)), so that session spans the idle gap and every later boundary the operation is still unsettled at. The hold is not a re-establishment and reaches no field of this event: its own run total is an operator-facing log line rather than a machine counter. The session it keeps is exposed to the same caps, though: a cap that cuts it while the operation is outstanding fails that operation, which is an unexpected loss like any other and is counted and warned. One that cuts it after the operation settled is absorbed uncounted by the next cycle's re-dial, which finds the session cleared and dials as it would after a release. A healthy run of either shape can therefore report a non-zero `reconnects`. |
