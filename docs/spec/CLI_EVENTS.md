...
- The `warning` `message` is stripped of PEM/OpenSSH private-key blocks before it is escaped, so the two free-text fields of this stream are redacted alike; a persisted event stream is the same class of sink as `--log-file`. Its escape runs under a display cap sized for a whole composed warning rather than for a single value (4096 output characters), because the cross-party host-key divergence notice is the warning a supervisor that discards stderr has nothing else to read, and the per-value cap cuts it before its recovery instruction. The values interpolated INTO a warning still carry the per-value cap where they were composed.
