...
| `transportTeardown` | The run's transport did not finish closing within the ceiling for its channel, so the run stopped waiting on it. The message states how long it waited and which resource kinds were still holding the process. The exit code is the exchange's own and is not changed by it. |
