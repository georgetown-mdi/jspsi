...
- The `result` `entityClusters` object holds no free text either: every member of it and of its `shapes` entries is a count this party derived from its own table and its own rounds' blocks, floored to a non-negative whole number at construction like the metric counters. It names no record, no row index and no linkage-key value, so it states nothing beyond what the result file this party wrote already holds ([PROTOCOL.md](PROTOCOL.md#the-many-to-many-entity-closure)).
