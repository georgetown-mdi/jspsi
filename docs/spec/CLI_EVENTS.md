...
A run whose transport does not finish closing within the ceiling for its channel emits one `warning` under `source: "transportTeardown"` and stops waiting on it. The close is the last obligation a finished run bounds: every local artifact -- the result file, the exchange record, the receipt -- is already on disk when cleanup begins, each awaited with no budget of its own, and the only local wait that is bounded at all is a result streamed to stdout instead of written to a path (see [Error categories](#error-categories)).

The notice states how long the run waited and which resource kinds were still holding the process, and it leaves the exit code alone, like every warning but a [persistence loss](#persistence-loss): what overran is housekeeping after the exchange, and a status a supervisor reads as failure would send it to retry a run that already disclosed. It states that the run's local artifacts are on disk only where the run reached its output stage; cleanup also runs from an interrupt and from a failure ahead of that stage, where none of them was written and the notice claims nothing about them. It also names the one thing the abandoned close leaves behind: on `filedrop` and `sftp` in delete mode that close is what removes this party's own protocol files from the shared directory ([FILE_SYNC.md](FILE_SYNC.md), `responsibleFiles`), so the notice asks the operator to check the exchange directory and remove any the run left there, naming `--sweep-exchange-files` as what does it on the next run. A retain-mode run's close deletes nothing and the files in that directory are the transcript it was set to keep, so its notice says nothing about them. The ceiling for each channel is stated with that channel's other budgets ([WEBRTC_TRANSPORT.md](WEBRTC_TRANSPORT.md#budgets), [FILE_SYNC.md](FILE_SYNC.md)).
...
| `transportTeardown` | The run's transport did not finish closing within the ceiling for its channel, so the run stopped waiting on it. The message states how long it waited and which resource kinds were still holding the process, and on a file-drop or SFTP exchange deleting its protocol files asks the operator to remove any the run left in the exchange directory. The exit code is the exchange's own and is not changed by it. |
...
The terminal event means everything the run owed is finished, not only everything it computed. Every local artifact is written before it, and the transport is closed before it too -- or has reached the ceiling for its channel and said so in the `transportTeardown` warning on the line above. A supervisor that reads the terminal event and then kills the process group takes nothing with it.
