...
| `GET` | `/api/jobs/sftp` | `200` `{ "configured": <bool>, ... }` | The effective SFTP connection as a credential-free projection: `{ "configured": false }` when none, else `{ "configured": true, "host", "port"?, "path"? }`. See [The provisioned SFTP server](#the-provisioned-sftp-server). |
...
Browsing does not widen the credential-reference rule: the reference a `PUT` carries is validated only against the data-root and rendezvous exclusions (above), not required to lie under the secrets mount. The mount is the browse surface; it is not an allowlist that authorizes a path.
