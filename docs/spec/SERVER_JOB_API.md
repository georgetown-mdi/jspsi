...
| ------ | ---- | ------- | ----- |
| `POST` | `/api/jobs` | `201` `{ "id": "<uuid>" }` | Create and start a job from a JSON intent. `413` (empty body) when the body exceeds the size cap (see [Size caps](#size-caps)). `400` on unparseable body, intent that fails schema validation, an sftp intent naming an unknown remote, or an `inputFile` intent whose name is unknown/vanished, whose `(size, mtime)` drifted, or for which the data root lacks free space (all empty body, resolved before any workdir exists). `409` (empty body) when the named remote is held by a running job. |
...
| `inputCsv` | optional non-empty string, length-capped | Inline CSV CONTENT written to a fixed, server-chosen filename in the workdir. The client never names a file. The cap is anchored to the browser intake's own 100 MiB file-size gate, so a CSV that passed that gate is never rejected here (see [Size caps](#size-caps)). One of `inputCsv` or `inputFile` is required (see below). |
| `inputFile` | optional `{ name, sizeBytes, modifiedAt }` | A REFERENCE to a file in the operator-mounted work-input directory (see [Work-input files](#work-input-files)), the alternative to inline `inputCsv`. `name` is validated by the listing's single-segment shape rule but never trusted from the intent: at create time the server re-runs the listing's admission (lstat regular-file + `O_NOFOLLOW` open + `(dev, ino)` recheck) against its own directory enumeration, so the name only ever selects an already-enumerated entry -- it never reaches `path.join` with a non-enumerated value, argv, or a composed path. The opened file is snapshot-copied to the same fixed `input.csv` workdir name, so it too never becomes a client-named path. `sizeBytes` (a positive integer) and `modifiedAt` (integer epoch milliseconds, exactly as the listing and profile emit it) are the client's profiled snapshot, re-checked for drift below. |
...
Both arms are `.strict()`: an unknown key (a smuggled `path`, `host`, `server` block, or `@path` credential reference) fails validation, and each arm admits only its own fields; the `inputFile` sub-object is `.strict()` too, so a path smuggled alongside its opaque `name` fails. A union-level refine enforces exactly one input source: an intent naming neither `inputCsv` nor `inputFile`, or both, is rejected. The arm's strict parse runs first (so a smuggled field still fails), then the cross-field rule rejects the ambiguous or empty intent.

**Create-time work-input checks.** When an intent carries `inputFile`, the server resolves and opens it BEFORE any workdir exists -- the unknown-remote posture -- and each failure is an empty-bodied `400` that never echoes the name, with no workdir left behind:

- **Unknown or vanished name.** The name is re-admitted against the current directory listing (the same lstat/`O_NOFOLLOW`/`(dev, ino)` recipe the profile and coverage routes use). A name that is not an admitted entry, or an inode that no longer matches, is refused. This is also the response when `JOB_INPUT_DIR` is unset.
- **Freshness drift.** The open-time `fstat` `(size, mtime-ms)` must equal the `(sizeBytes, modifiedAt)` pair the client profiled (compared by exact equality of the round-tripped integer epoch-ms). A file that changed since profiling is refused rather than snapshotted; the CLI's own run-time column re-check remains the backstop against an equal-size, equal-mtime rewrite.
- **Insufficient space.** A best-effort `statfs` refuses the create when the data root's filesystem has less free space than the file's size, so a CLI-scale snapshot copy cannot wedge the data root mid-write. Mounted inputs have no total-size cap (gigabyte files are in scope), so the copy is streamed, never buffered.

The opened file is then STREAM-copied into the workdir as the fixed `input.csv` (mode `0o600`), giving the run snapshot semantics: an operator who overwrites the mounted file mid-run cannot corrupt the exchange. The CLI's argv is byte-identical whether the input arrived inline or by reference -- the CLI is always pointed at the same fixed workdir path and never learns the mounted source name or path. The `options` subset is deliberately the numeric/boolean/enum knobs only -- `pollIntervalMs`, `peerTimeoutMs`, `serverConnectTimeoutMs`, `maxReconnectAttempts` (0..604800), `timestampInFilename`, `locklessRendezvous`, `retainFiles`, and `unexpectedFiles` (`error`/`warn`/`ignore`). The path and directory fields of the CLI's file-sync options are intentionally not surfaced (the server owns every directory), and the free-text `peerId` is omitted for the same reason. On the sftp arm `pollIntervalMs` is additionally floored at 1000 ms: an sftp poll is a directory listing against the operator's provisioned remote server, not a job-local directory, so a client-chosen hot poll would flood a shared -- possibly partner-hosted -- host.
...
| `input.csv` | The job's input CSV: the intent's inline `inputCsv` content, or a snapshot stream-copy of the mounted file the intent's `inputFile` named (see [The exchange intent](#the-exchange-intent)). Either way a fixed, server-chosen name. |
