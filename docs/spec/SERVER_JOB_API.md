...
| `GET` | `/api/jobs/sftp` | `200` `{ "configured": <bool>, ... }` | The operator-provisioned SFTP server as a credential-free projection: `{ "configured": false }` when none, else `{ "configured": true, "host", "port"?, "path"? }`. See [The provisioned SFTP server](#the-provisioned-sftp-server). |
...
An sftp job's connection material never comes from the client; it comes from a single server the operator provisions at deploy time. The console conducts one exchange and is not a store of named connections, so the appliance is provisioned with exactly one outbound SFTP server. `JOB_SFTP_SERVER` names a YAML file (mounted read-only in the container case) of shape:
...
Returns the provisioned server as an explicitly mapped, credential-free projection: `{ "configured": false }` when none is provisioned, else `{ "configured": true, "host": "...", "port"?: <int>, "path"?: "..." }` and nothing else -- no username, no credential references (which would reveal the secret-mount filesystem layout), no fingerprint. An enabled API with no server provisioned serves `200 { "configured": false }`, the same shape family and gate as `GET /api/jobs/rendezvous` (a `404` there means the API is disabled). The console web build uses this to gate the run-SFTP-here behavior and to author an invitation's sftp endpoint from the provisioned locator. The static `sftp` segment cannot be captured as a job id: ids are validated as canonical v4 UUIDs before any use, which `sftp` is not.
...
| `JOB_SFTP_SERVER` | The provisioned SFTP server (see [The provisioned SFTP server](#the-provisioned-sftp-server)). Empty or unset -> every sftp intent is rejected; the API is otherwise unchanged. Non-empty -> the named YAML file is loaded and validated at startup, fail-closed. Requires `JOB_DATA_ROOT`; set only server-side, never derived from a request. The superseded `JOB_SFTP_REMOTES` (a multi-remote table), if still set, refuses startup with a migration message. |

### Startup and reachability

The API is enabled by `JOB_DATA_ROOT` alone: set, the manager is constructed and every endpoint is live; unset or empty, every endpoint answers `404` and no manager is constructed or child spawned. Because the work-input and rendezvous directories both fall back to `JOB_DATA_ROOT`, that one variable also lights up the input listing and the filedrop transport, so a single mount runs a full console. The server does not inspect its own bind interface -- there is no loopback bind check and no bind-derived startup refusal. Reaching the unauthenticated API only from the operator's own machine is a deployment concern: the console container publishes its port to the host loopback (`-p 127.0.0.1:PORT:PORT`), so exposure is governed by that publish binding and the host firewall rather than by an app-level gate (see [DEPLOYMENT.md](../DEPLOYMENT.md#server-job-api)).
