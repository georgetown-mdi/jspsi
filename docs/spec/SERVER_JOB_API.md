...
- **Boundary body cap.** `POST /api/jobs` reads its body under a hard byte cap (224 MiB), streamed off the request and counted chunk by chunk; the read aborts the moment the running total exceeds the cap, and `Content-Length` is never trusted (it can be absent or understated on a chunked request). An oversized body is a `413` before the body is fully buffered or any schema parse runs. The cap is derived from the `inputCsv` cap -- above twice its char cap plus headroom for the other fields -- so a JSON-encoded schema-valid intent, even under worst-case string escaping, is never rejected at the boundary instead of receiving a clean schema error.
