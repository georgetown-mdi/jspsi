...
| ------ | ---- | ------- | ----- |
| `POST` | `/api/jobs` | `201` `{ "id": "<uuid>" }` | Create and start a job from a JSON intent. `413` (empty body) when the body exceeds the size cap (see [Size caps](#size-caps)). `400` on unparseable body, intent that fails schema validation, or an sftp intent with no server provisioned (empty body, resolved before any workdir exists). `409` (empty body) when an exchange is already active; delete it first. |
...
| `GET` | `/api/jobs/sftp` | `200` `{ "configured": <bool>, "bootPinned": <bool>, ... }` | The effective SFTP connection as a credential-free projection: `{ "configured": false, "bootPinned": false }` when none, else `{ "configured": true, "bootPinned": <bool>, "host", "port"?, "path"? }`. `bootPinned` is true only for a boot `JOB_SFTP_SERVER` (authoring is refused). See [The provisioned SFTP server](#the-provisioned-sftp-server). |
| `PUT` | `/api/jobs/sftp` | `200` credential-free projection | Author the SFTP connection from a file-reference credential body (see [Authoring the SFTP connection](#authoring-the-sftp-connection)). `400` (body naming a field path, never a value) on a body that fails validation. `409` when a boot `JOB_SFTP_SERVER` is provisioned -- it wins, so authoring is refused. `413` when the body exceeds its size cap. |
| `DELETE` | `/api/jobs/sftp` | `204` | Forget the in-app authored connection (idempotent). A boot-provisioned server is unaffected; `GET` still reports it. |
...
## The provisioned SFTP server

An sftp job's connection material never comes from the create intent; it comes from a single server the operator provisions at deploy time, or -- when none is provisioned -- from a connection the operator authors in-console ([Authoring the SFTP connection](#authoring-the-sftp-connection)). The console conducts one exchange and is not a store of named connections, so there is exactly one effective outbound SFTP server at a time. A deploy-time `JOB_SFTP_SERVER` names a YAML file (mounted read-only in the container case) of shape:
...
The file is read and validated once at server startup, fail-closed: an invalid block refuses startup with an error naming the offending field path (never a value). Setting `JOB_SFTP_SERVER` without `JOB_DATA_ROOT` is itself a startup error. The variable is set only server-side, never derived from a request. The superseded multi-remote variable `JOB_SFTP_REMOTES`, if still set, refuses startup with a migration message naming `JOB_SFTP_SERVER` and the single-`server:` shape -- a silently ignored variable would leave an operator believing SFTP is provisioned.
...
- **Credential references only.** `password`, `private_key`, and `private_key_passphrase`, when present, must be `@path` references to an ABSOLUTE path OUTSIDE the resolved data root AND -- when it is configured distinctly -- the resolved rendezvous directory, and the referenced file must exist at validation time (checked by `stat`/`realpath` only; the bytes are never read into the server). Inline values are rejected, so no secret enters server memory or any composed job file; the CLI child resolves the reference at config load. The exclusion is checked both on the lexically resolved path AND on its realpath, so a symlink in the reference chain cannot resolve back into an excluded directory undetected. The exclusions close a laundering path: workdir contents are client-written and the rendezvous mount is partner-reachable through folder sync, so a reference under either could turn planted content -- or a past job's result -- into a transmitted credential.
- **Boot-time composition check.** The block is additionally parsed through core's connection schema as `{channel: "sftp", server: <block>}`, so the CLI's cross-field refines (one primary auth method, a passphrase requires a key, keyboard-interactive requires a password, canonical fingerprint form) hold at startup rather than first inside a job.

The same validation chain -- strict field allowlist, mandatory literal fingerprint, credential-`@path`-outside-the-exclusions with realpath re-confinement, and the core-schema compose -- validates both the boot file and a request-sourced authored connection, so the two cannot diverge (see [Authoring the SFTP connection](#authoring-the-sftp-connection)).

The server is startup-frozen: changing the host or pin requires a restart. The referenced credential FILES are live -- the CLI child re-reads them at each job's config load -- so rotating a secret in place takes effect without one.

**One exchange at a time.** The console facilitates a single exchange, so the manager holds at most one in a single slot: while it is occupied a second create of either channel is refused with an empty-bodied `409` until the current exchange is deleted. The slot is claimed before any workdir exists and freed only when the exchange was deleted and its child's exit was observed (see [Job lifetime and orphan handling](#job-lifetime-and-orphan-handling)), so a successor can never rendezvous with a still-dying child over the provisioned server or `JOB_RENDEZVOUS_DIR` directory.

### `GET /api/jobs/sftp`

Returns the EFFECTIVE connection (see [Authoring the SFTP connection](#authoring-the-sftp-connection) for boot-vs-authored precedence) as an explicitly mapped, credential-free projection: `{ "configured": false, "bootPinned": false }` when none is effective, else `{ "configured": true, "bootPinned": <bool>, "host": "...", "port"?: <int>, "path"?: "..." }` and nothing else -- no username, no credential references (which would reveal the secret-mount filesystem layout), no fingerprint. `bootPinned` is true only when a boot `JOB_SFTP_SERVER` is the effective connection, so the console shows it read-only and offers in-app authoring (and clear) only for an authored or absent connection; it is false whenever no connection is present (a contradictory `configured:false, bootPinned:true` is never emitted). An enabled API with no connection serves `200 { "configured": false, "bootPinned": false }`, the same shape family and gate as `GET /api/jobs/rendezvous` (a `404` there means the API is disabled). The console web build uses this to gate the run-SFTP-here behavior and to author an invitation's sftp endpoint from the effective locator. The static `sftp` segment cannot be captured as a job id: ids are validated as canonical v4 UUIDs before any use, which `sftp` is not.

## Authoring the SFTP connection

An sftp job's connection has two possible sources, and the effective one is fixed by precedence:

- **Boot server (deploy-time).** A `JOB_SFTP_SERVER` file, loaded and validated fail-closed at startup (above). When set, it WINS: it is the effective connection, and `PUT /api/jobs/sftp` refuses to override it with a `409`.
- **Authored (in-app).** When no boot server is set, the operator authors the connection in-console through `PUT /api/jobs/sftp`. The validated connection is held in a single in-memory slot scoped to the one exchange -- never persisted -- so a restart forgets it. It is the effective connection whenever no boot server is set.

`GET /api/jobs/sftp` projects whichever is effective, and `createJob`'s sftp arm composes from it, unchanged; the create-intent injection closure is unaffected (`POST /api/jobs` gains no connection field).
...
- **Materialize once, then rewrite.** The server writes `value` ONCE to a server-owned file under a fixed, container-internal scratch directory (`/run/psilink/sftp-credentials`), then rewrites the credential to `{ "kind": "ref", "ref": "@<that file>" }` and runs the SAME `assertCredentialRef` containment the file-reference forms run -- so the materialized path is confirmed outside the data root and rendezvous directory, on its realpath. Server memory holds `value` only between request parse and the file write; it is never logged, placed in argv or the child environment, or returned in any response, projection, or SSE event. The composed `psilink.yaml` carries the resulting `@path`, never the value.
...
- **Boot sweep and containment assertion.** At server startup, when the job API is enabled, the scratch directory is swept clean (covering a secret orphaned by a restart -- an SSH credential must not inherit the workdir's persistence) and asserted to resolve strictly OUTSIDE the data root and the rendezvous directory, on both the lexical resolve and the realpath and in both nesting directions (a sweep of an excluded directory would destroy operator or partner data). A directory that fails the assertion refuses the boot, fail-closed, matching the appliance's posture. Preferring a tmpfs mount at the scratch path keeps pasted secrets off disk entirely; the boot sweep is the backstop when the container does not arrange one.
...
Forgets the in-app authored connection and returns `204` (idempotent). A boot-provisioned server is unaffected -- there is nothing authored to clear, and `GET` still reports the boot server. The authored connection is ALSO cleared when the exchange it was authored for is deleted (`DELETE /api/jobs/:jobId` of the active exchange), so it is scoped to that single exchange and never lingers into the next.
...
Neither channel's rendezvous lives in the workdir. A filedrop job's rendezvous is the resolved `JOB_RENDEZVOUS_DIR` mount (which defaults to `JOB_DATA_ROOT` when unset; see [Composed CLI configuration](#composed-cli-configuration)), shared across jobs and held by the single-holder latch; an sftp job's rendezvous is the provisioned server's own directory. An sftp job's workdir has the same fixed-name layout, and its `psilink.yaml` carries the provisioned server's connection block -- locator, pinned fingerprint, and `@path` credential references, never a credential value.
...
| `JOB_SFTP_SERVER` | The provisioned SFTP server (see [The provisioned SFTP server](#the-provisioned-sftp-server)). Empty or unset -> no boot server; an sftp job runs against the in-app authored connection instead, or is rejected when neither is set. Non-empty -> the named YAML file is loaded and validated at startup, fail-closed, and WINS over any authored connection (`PUT /api/jobs/sftp` is refused). Requires `JOB_DATA_ROOT`; set only server-side, never derived from a request. The superseded `JOB_SFTP_REMOTES` (a multi-remote table), if still set, refuses startup with a migration message. |
| `JOB_SECRETS_DIR` | The mounted secrets directory the console browses for a connection's credential file (see [The secrets mount and browsing](#the-secrets-mount-and-browsing)). Empty or unset -> the secrets mount is unavailable; there is deliberately NO `JOB_DATA_ROOT` fallback (a fallback would default the credential-browse surface into the client-writable data root). Non-empty -> resolved to an absolute path. Read trimmed; set only server-side, never derived from a request. |
| `JOB_SFTP_CREDENTIAL_DIR` | Overrides the container-internal directory a pasted (`raw`) credential is materialized to (see [Materializing a pasted credential](#materializing-a-pasted-credential)). Empty or unset -> the fixed default (`/run/psilink/sftp-credentials`), which the root-running image can create. Non-empty -> that directory, for a non-root deployment or the test harness. Whichever it resolves to is asserted at startup to lie strictly outside the data root and rendezvous directory (else the boot refuses), created owner-only, and swept clean. Set only server-side, never derived from a request. |
...
The provisioned server keeps its own fail-closed startup validation: a configured `JOB_SFTP_SERVER` that is unreadable or invalid, or one configured without `JOB_DATA_ROOT`, refuses startup; a still-set `JOB_SFTP_REMOTES` refuses startup with a migration message.
...
- **One exchange in one slot.** The manager holds at most one exchange at a time. A second create -- of either channel -- is refused with an empty-bodied `409` while the slot is occupied, and the slot frees only when BOTH hold: the exchange was explicitly `DELETE`d, and the child's exit was observed (or no child was ever spawned). Keying the release on the child's confirmed exit -- not merely on the terminal event, which the buffer-overflow path emits while a `SIGKILL`ed child may still be running -- is what keeps a successor exchange from rendezvousing with a still-dying child over the provisioned server or rendezvous directory. A settled but undeleted exchange keeps the slot: the reject-until-`DELETE` rule means a create never silently destroys the previous exchange's undownloaded result.
