...
- `folderName` -- the shared folder's own name (see `JOB_RENDEZVOUS_NAME` under [Environment variables](#environment-variables)), present only where the console can name it. It is the only one of the two that may be shown to anyone AS the shared folder's name: the console's own confirm line and the accept kit take this field and say nothing where it is absent, so neither presents a mount point the operator did not choose as a folder the partner could recognize.
