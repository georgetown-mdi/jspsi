...
- The **image tag** the sheet's `docker run` lines carry. The web build exposes no version of its own, so the tag is the floating `latest` a release publishes alongside `X.Y.Z` (see [RELEASES.md](../RELEASES.md)) until a version-matched value is wired through.
