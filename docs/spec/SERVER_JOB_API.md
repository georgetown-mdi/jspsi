...
| `GET` | `/api/jobs/rendezvous` | `200` `{ "configured": <bool>, ... }` | Whether a filedrop exchange can run here and how an invitation minted here may name the shared folder: `{ "configured": false }` when no rendezvous mount resolves, else `{ "configured": true, "locator"?, "folderName"? }` (see [The filedrop rendezvous report](#the-filedrop-rendezvous-report)). |
...
The accept kit is the console's second graduation artifact: a plaintext, printable, ASCII instruction sheet the inviting operator downloads at invitation-mint time and sends to the partner alongside the invitation, taking a partner who has Docker Desktop (or can get it) and nothing else to the point of accepting. It is specified here beside the hand-off because the two artifacts share the invariants below and must not drift, **not** because the job API serves it: no endpoint returns it. It is composed in the browser from the values captured at mint time and the build's own release version -- the named values below and nothing else -- when the operator clicks download, and written to the operator's disk through the same client-side blob download as the exchange file. The operator-facing account is in [DEPLOYMENT.md](../DEPLOYMENT.md#server-job-api).

It exists for the two channels whose partner accepts from the command line, `sftp` and `filedrop`; a WebRTC exchange has no command-line side and no kit. Its contents are fixed text apart from two interpolated values:

- The **rendezvous locator** minted into that invitation -- the sftp host, optional port, and optional remote directory, or the filedrop shared folder's name -- printed back for the partner to check. It is taken from the same values that built the invitation's `connectionEndpoint`, so the sheet can never name a rendezvous the token does not carry.

  On the filedrop channel the sheet prints the shared folder's name only where the console can name it (`folderName` in [The filedrop rendezvous report](#the-filedrop-rendezvous-report)); where it cannot, the sheet omits the name entirely and says so, rather than printing the container mount point the folder was bound at as though it were the folder's name. The sheet therefore prints a subset of what the token carries, never a different value. What it asks for is a check and not a match: the same shared folder can carry a different name on each side -- a share root mapped to a drive letter is the ordinary case -- so the sheet asks the partner to check the name against the folder they were told to use rather than asserting their own name for it will be the same. The step that repoints the partner's own config at their copy of the folder follows the same branch: named, it says accepting wrote the inviter's name for the folder; unnamed, it calls what accepting wrote a placeholder, because what the token carried there was the console's mount point. Either way it directs the same replacement, so the sheet never tells the partner a value is a name neither side chose.
- The **release version** of the build that composed it, which decides two things together: the image tag every `docker run` line carries, and the release page the filedrop launcher branch links. A kit minted on a released console image names `docker.io/vdorie/psi-link:X.Y.Z` in every command and links that release's own page, so the partner runs the version that minted their invitation and takes the launchers from the release that built it. The image build reads the version from `apps/cli/package.json`, the canonical release version (see [RELEASES.md](../RELEASES.md)), and bakes it into the client bundle as `VITE_PSILINK_VERSION`; the kit therefore names whatever version the manifest held at image build. The published image's own tag comes independently from the pushed git tag, and their agreement is the release process's step ordering -- the manifest bump preceding the tag push -- not a property anything mechanizes or checks.

  A value is interpolated only in the release shape -- `X.Y.Z` with semver's optional prerelease and build suffixes, excluding the `0.0.0` marker held by the manifests that carry no release version. Anything else, including an absent value, names the floating `latest` tag a release publishes alongside `X.Y.Z` and links the releases index, so a build that passes no version -- the continuously deployed hosted deployment, or any web build outside the image build -- prints a working command rather than an empty, partial, or placeholder tag. That gate is also what holds the sheet's ASCII contract for this value: unlike the rendezvous locator, it is never filtered, because no value outside the shape reaches the sheet at all.

  Two limits hold as measured. The image build takes whatever version the CLI manifest holds, and between releases every tree holds the last published one: an image built from a non-release tree therefore names that release rather than the build that minted the invitation (as the pre-version sheet's floating tag also did), and a release-branch tree built after the version bump but before the tag is published names a release that does not exist yet. And the gate holds the value to release shape, not to Docker's tag grammar: a semver suffix -- which the release process never sets, publishing bare `X.Y.Z` only (see [RELEASES.md](../RELEASES.md)) -- would render into the tag verbatim.

Four invariants hold by construction -- nothing else is representable as an input -- and each is pinned by a test:

- **No secret.** The sheet carries no shared secret, key-file body, or credential value. The credential an sftp partner needs is theirs, not the inviter's: the sheet names the `REPLACE_WITH_SSH_USERNAME` placeholder acceptance seeds and the `@path` convention for pointing at a credential file, and states that the invitation carries no credential by construction.
- **No invitation token.** The sheet carries the fixed placeholder `PASTE_YOUR_INVITATION` where the invitation string goes, never the encoded token or its deep link. The invitation travels on its own confidential channel; the sheet is safe to pass around and to print.
- **No container path.** The only container paths on the sheet are the fixed mount points of the commands the *partner* will run (`/work`, `/sync`, `/run/secrets`); no path inside the inviter's appliance appears.
- **No inviter-machine path.** Nothing from the inviting operator's filesystem appears beyond the rendezvous locator the invitation already carries -- on the filedrop channel that is a single path segment naming the shared folder, never the appliance's resolved absolute path. The mount path does not cross the job API at all (see [The filedrop rendezvous report](#the-filedrop-rendezvous-report)), so no composition step in the browser has one to leak.

### The single-slot occupancy probe

`GET /api/jobs/slot` reports whether the console's single exchange slot is occupied, and by which job: `{ "occupied": false }` when free, else `{ "occupied": true, "id": "<uuid>" }` naming the occupying exchange. Occupied is exactly the state in which a create is refused with a `409` -- a starting or active exchange, including one deleted whose child is still dying (see [Job lifetime and orphan handling](#job-lifetime-and-orphan-handling)) -- so this probe and the busy create disclose the same occupant and cannot drift.

The console lobby's recovery panel reads it when the browser holds no stored attachment: a browser that never started the exchange -- a second machine, or one whose recovery record was lost -- can then still see the slot is taken and re-attach to the running exchange or discard it, closing the gap the attachment-only recovery left. It is gated identically to the rest of the job API -- the feature gate, the loopback Host-allowlist, and the browser-CSRF check all apply, and a disabled API is `404` -- so it adds no new perimeter. It discloses exactly what a busy `POST /api/jobs` `409` already does (occupied plus the occupant's id, a non-secret v4 UUID reachable only over the loopback-local origin) and nothing more: it is not a job listing (`GET /api/jobs` stays absent), carries no run detail, and adds no destructive endpoint -- clearing the slot rides the existing per-id `DELETE /api/jobs/:jobId` through the panel's Discard. The static `slot` segment cannot be captured as a job id: ids are validated as canonical v4 UUIDs before any use, which `slot` is not.

### The filedrop rendezvous report

`GET /api/jobs/rendezvous` reports whether a filedrop exchange can run on this console and what an invitation minted here may say about the shared folder. The body is `{ "configured": false }` when no rendezvous mount resolves (neither `JOB_RENDEZVOUS_DIR` nor `JOB_DATA_ROOT` is set), else `{ "configured": true, "locator"?, "folderName"? }`:

- `locator` -- the advisory locator the invitation's filedrop endpoint carries: the shared folder's name where the console has one, else the resolved mount's own last path segment. Always a single path segment, never the resolved mount path. A mint needs one whether or not the folder can be named, because core's endpoint schema requires a filedrop directory. Absent only for a mount with no last segment at all, which the browser client reads as unavailable.
- `folderName` -- the shared folder's own name (see `JOB_RENDEZVOUS_NAME` under [Environment variables](#environment-variables)), present only where the console can name it. It is the only one of the two that may be shown to anyone AS the shared folder's name: the inviting console's confirm line and the accept kit take this field and say nothing where it is absent, so neither presents a mount point the operator did not choose as a folder the partner could recognize. One surface holds a stated limit: a console ACCEPTING an invitation still renders the invitation's locator on its own confirm line, so a partner accepting on a console sees the locator -- which is the inviting launcher's mount segment exactly where the inviter could not name the folder. Presence: the launchers emit `JOB_RENDEZVOUS_NAME` unconditionally, an empty value where no name is derivable, and an empty value resolves to no name rather than the mount segment. That emptiness path rests on the engine passing `--env NAME=` through as set-but-empty; it is verified against the launcher tests' stub engine only, and were a real engine to drop the empty value the console would fall back to the mount segment -- the pre-name behavior, not a new state. One `docker run --rm --env FOO= alpine env` on an engine host settles it.

The resolved mount path is deliberately not in the body. Nothing in the browser needs it, and leaving it server-side is what makes the "No inviter-machine path" invariant above a property of the API boundary rather than of every consumer remembering to reduce a path before minting.
...
| `VITE_PSILINK_VERSION` | The release version baked into the client bundle, and no part of the feature gate: it configures nothing server-side and every job route ignores it. The image build sets it for the web build alone from `apps/cli/package.json` (see [The partner accept kit](#the-partner-accept-kit)), which is the only consumer; unset or in any shape but a release version, the kit names the floating image tag. Set at build time, never derived from a request. |
...
| `JOB_RENDEZVOUS_NAME` | The shared folder's own name -- what the operator and their partner call the folder the rendezvous mount stands for -- which the filedrop invitation carries as its advisory locator and the accept kit prints. Unset -> the name is the resolved rendezvous mount's last path segment, the operator-authored mount whose mount point is their own naming. Set -> that value, trimmed, when it is a bare folder name; a value that is not (empty, `.`, `..`, carrying `/` or `\`, carrying a byte below `0x20` or `0x7f`, or longer than 255 characters) leaves the console with NO folder name rather than falling back to the mount point, because a caller that set the variable has already said the mount point does not name the folder. Set only server-side, never derived from a request; it reaches the partner in the invitation. |
