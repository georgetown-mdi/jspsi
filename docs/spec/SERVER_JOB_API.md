...
- The **rendezvous locator** minted into that invitation -- the sftp host, optional port, and optional remote directory, or the filedrop shared folder's name -- printed back for the partner to check. It is taken from the same expression that built the invitation's `connectionEndpoint`, so the sheet and the token cannot name different rendezvous.
...
- **No inviter-machine path.** Nothing from the inviting operator's filesystem appears beyond the rendezvous locator the invitation already carries -- on the filedrop channel that is the shared folder's name, never the appliance's resolved absolute path (the same basename-only rule the invitation endpoint follows).
