...
`restored` is true when the view was reconstructed from disk artifacts rather than an in-memory record -- after a restart, or after the in-memory record's TTL eviction on a still-running server (see [Job lifetime and orphan handling](#job-lifetime-and-orphan-handling)). A restored job carries no event history, so `terminal` is `null`, `terminalEmitted` is `true`, and `eventCount` is `0`; its `status` is derived purely from artifacts (`succeeded` when the result file is present, else `failed`).
