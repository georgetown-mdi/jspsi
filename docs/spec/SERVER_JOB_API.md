...
The same posture covers the provisioned server: a configured `JOB_SFTP_SERVER` that is unreadable or invalid, or one configured without `JOB_DATA_ROOT`, refuses startup; a still-set `JOB_SFTP_REMOTES` refuses startup with a migration message.
