...
Recurring exchanges derive their session key from an ephemeral X25519 key exchange, keyed together with the pre-shared secret and an explicit mutual confirmation. A password-authenticated key exchange (PAKE) is deliberately not used: the secret is high-entropy (a 256-bit token, not a human password), so a PAKE's one distinctive property, resistance to offline guessing, would be unused, while the properties that matter here -- forward secrecy and mutual authentication -- are preserved by ephemeral X25519 plus a secret-keyed confirmation, with no hand-rolled PAKE surface to carry. The limit is the same as for any secret-authenticated design: a leaked secret permits active impersonation until it is rotated out, and forward secrecy buys only that a recorded transcript stays confidential if the secret later leaks.

The construction follows the Noise NNpsk0 pattern over X25519 with an added explicit, role-asymmetric key confirmation whose construction is modelled on NIST SP 800-56A Rev. 3 without being conformant to it (see [PROTOCOL.md](spec/PROTOCOL.md#x25519-authenticated-key-exchange) for why); its primitive comes from a well-respected, ideally audited library, and the full Noise framework is never hand-rolled. The wire-level specification is in [PROTOCOL.md](spec/PROTOCOL.md#x25519-authenticated-key-exchange), which also records the migration from the earlier SPAKE2 design. The CLI and the web application run the same handshake, and its version discriminant keeps future interop open.

The pre-shared secret remains the baseline authentication anchor: low setup friction, and no organizational PKI for the bilateral-agency case. A certificate-chain, authority-backed mode is left open by that same version discriminant (see [Pinned self-signed trust model](#pinned-self-signed-trust-model)). The design also carries a compliance upside, stated here as a distinction because two kinds of approval are easy to conflate. Ed25519 signatures are approved as an algorithm standard (FIPS 186-5) and the curves are NIST-specified (SP 800-186), which is worth having in an agency ATO review. It is not module-certificate approval: neither X25519 nor Ed25519 is on the approved-algorithm list of any OpenSSL Project certificate, and under the FIPS 140-3 certificate this project targets both are Non-Approved and Not Allowed. Reaching inside a validated module boundary takes a different change -- migrating key establishment to P-256 ECDH through `crypto.subtle`, keeping this schedule and confirmation round -- decided and recorded, with what a scoped FIPS claim may and may not say about key establishment, in [key-establishment-fips-boundary.md](notes/key-establishment-fips-boundary.md). The Ed25519 half is a separate decision on the same reasoning: receipt signing migrates to ECDSA over P-256 through `crypto.subtle`, recorded in [receipt-signing-fips-boundary.md](notes/receipt-signing-fips-boundary.md).
