...
| The SFTP server operator or shared-folder administrator | Every file the exchange writes into the rendezvous directory | A recurring exchange wraps its frames in application-layer AES-256-GCM keyed from the key exchange, so the administrator sees ciphertext. A zero-setup exchange has no session key and rests on that administrator's access controls instead ([Channel security](#channel-security)) |
