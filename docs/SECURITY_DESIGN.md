...
Two channel-hardening follow-ups remain deferred. Both were dormant while the decorator was unwired; wiring it into live recurring CLI exchanges turns each into live residual risk against a transport-level attacker (for SFTP, the server admin). Each is an accepted deferral for the wired-but-pre-release state only, and each is a release blocker: the first release will not ship until both are resolved (implemented and security-reviewed) or explicitly re-affirmed at release time.

The first is gap and truncation detection. The sequence-number check rejects any number that is not strictly greater than the last accepted, which stops replay and reordering but not omission: a gap (a sequence number that skips ahead) is accepted, and a truncated tail is indistinguishable from a clean end. Completeness is therefore delegated to the inner transport and the lockstep protocol above, where a dropped frame surfaces as a stalled or schema-invalid exchange. The live residual risk is that the attacker can selectively drop, withhold, or truncate frames - including the final frame(s) - without tripping the AEAD tag. Strict gap detection - rejecting anything that is not exactly the last accepted plus one - together with an integrity-protected end-of-stream marker for truncation is tracked on the project board (product item 196808078). It is blocked on the send path advancing its counter only on a fully successful send (otherwise a legitimate sender-side gap, from a failed transport send or a serialization error, would raise a false tamper alarm); that change must in turn be reconciled with the nonce-safety invariant that advances the send counter synchronously before the encrypt (so two concurrent sends cannot reuse a nonce), and as net-new security behavior it requires explicit security review. The end-of-stream-marker design will be recorded here when it is built.

The second is an inbound frame-size bound. The inbound read path loads an entire ciphertext frame into memory and JSON-parses it before any integrity check runs, so a hostile filedrop/SFTP server admin - an adversary under the threat model above - could write an arbitrarily large file and exhaust memory: a denial-of-service. Bounding the inbound frame size at the transport read, so an oversized file is refused before it is ingested, is tracked on the project board (product item 196853013). The bound must be enforced at the transport read layer - a `.max()` on the decorator's `{ enc }` envelope schema is at best partial defense-in-depth, since by then the dominant allocation has already happened - and choosing the value requires accommodating the realistic worst-case PSI protobuf frame (post-base64), so it warrants deliberate justification; as net-new security behavior it too requires explicit security review.
