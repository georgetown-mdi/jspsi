...
Running SFTP from the appliance adds an outbound surface the file-drop path lacks. The operator-authored connection pins the server's host key -- supplied by pasting the published fingerprint or by probing the server to read and compare it (a comparison aid, never a trust judgement; see [Transport-layer authentication](#transport-layer-authentication)) -- verified before any credential is presented, so redirected egress or DNS tampering yields a refused connection rather than a disclosed credential. Credentials are held as file references, resolved only by the CLI subprocess; the authoring path rejects inline secret values, so no secret enters server memory or a composed job file. The console is a single-owner prototyping tool and does not hard-block its trusted operator for defense-in-depth: referencing a credential file inside the one mounted data root (or the partner-synced rendezvous) is allowed and raises a NON-BLOCKING warning that nudges toward a separate read-only secrets volume (`JOB_SECRETS_DIR`), rather than a refusal -- the containment stays a guide within the single-operator, on-host trust model, not a barrier. The de-emphasized paste fallback for a connection authored in the console is the one credential that crosses the appliance's own loopback API: the value is materialized to a server-owned, owner-only file at a container-internal location wiped at every startup -- never to a job directory, the partner-synced rendezvous, or the composed config, which still carry only an `@path`. Keeping the secret bytes out of the web server and its job files where possible is accidental-exposure hygiene, not privilege separation: the subprocess runs as the same user, a pasted value already crosses the same on-host boundary, and a compromised server could read the mounted files directly.

What violates the invariant is any deployment that lets a second party reach the API. Publishing the port beyond host loopback, or fronting it with a reverse proxy that routes a second party to it, does exactly that, and because the API is unauthenticated any such exposure re-exposes it unauthenticated. The wire contract, the intent schema, the authored-connection validation, the workdir modes, and the gate and startup rules are in [SERVER_JOB_API.md](spec/SERVER_JOB_API.md).
