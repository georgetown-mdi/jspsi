...
- **Dark unless deliberately turned on.** The job API does nothing unless the deployment is a console build with a data root configured; the hosted `apps/web` deployment serves every job route disabled (`404`), so a misconfigured hosted deployment cannot expose it. Outbound SFTP is separately gated: it exists only when the operator authors an SFTP connection for the single exchange, and that connection is the appliance's only network egress (holding the container to that one endpoint at the host is in [DEPLOYMENT.md](DEPLOYMENT.md#restricting-the-containers-outbound-network-access)).
- **One operator, no tenants.** The API carries no per-request authentication and no per-tenant isolation, because there is no second party to isolate from. Its reach is bounded by the deployment -- container isolation, the publish binding, and the host and agency firewall -- not by an app-level check.
...
Recurring exchanges derive their session key from an ephemeral P-256 key exchange, keyed together with the pre-shared secret and an explicit mutual confirmation. A password-authenticated key exchange (PAKE) is deliberately not used: the secret is high-entropy (a 256-bit token, not a human password), so a PAKE's one distinctive property, resistance to offline guessing, would be unused, while the properties that matter here -- forward secrecy and mutual authentication -- are preserved by an ephemeral exchange plus a secret-keyed confirmation, with no hand-rolled PAKE surface to carry. The limit is the same as for any secret-authenticated design: a leaked secret permits active impersonation until it is rotated out, and forward secrecy buys only that a recorded transcript stays confidential if the secret later leaks.
...
That secret is protected only by the transport layer (SSH, or operator access controls for a file-drop -- the two channels a zero-setup exchange can run on), because a zero-setup exchange has no key-exchange session key and so no application-layer AEAD to wrap it. This is an accepted trust model: bootstrapping with `--save` rests on the same transport trust as the zero-setup exchange that carries it, for that one initial exchange. A party unwilling to trust the transport with the secret should use `psilink invite` instead, which never sends the secret through the exchange server. After this first exchange the secret rotates on every subsequent exchange and is never reused.
