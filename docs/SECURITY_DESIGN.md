...
- **File-sync transports (SFTP, file-drop)** request it unconditionally, because the server admin can snoop the transport. The CLI requests it on every file-sync exchange, so the SFTP or file-drop admin sees ciphertext rather than the exchange's contents, the key-exchange handshake frames excepted (below).
