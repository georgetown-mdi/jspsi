...
- `JOB_SECRETS_DIR` -- the mounted directory the console browses for a connection's credential file (a password file or an SSH private key) when the operator authors an SFTP connection in-app. Unlike the input and rendezvous directories it has NO `JOB_DATA_ROOT` fallback: leave it unset and the operator references credentials by typing a file path instead of browsing. It is deliberately never defaulted into the client-writable data root -- the one place a credential directory must never be. Mount it read-only alongside your credential files.
