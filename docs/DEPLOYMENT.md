...
A directory holding a **retain-mode transcript** -- yours, or your partner's -- refuses the sweep, and the console tells you so rather than offering a way around it: those files may be somebody's audit record. The escalation exists on the command line (`--sweep-exchange-files --force-retain-sweep`, which deletes the transcript permanently), and the console names it instead of providing a one-click version.
