...
`JOB_CLI_BINARY` is pre-set in the image and needs no operator value. With `--network host` the container shares the host's network namespace, so the app's own `HOST=127.0.0.1` bind is the host's real loopback interface and nothing outside the host can reach `/api/jobs`. Bridge port-publishing (`-p 3000:3000`, even `-p 127.0.0.1:3000:3000`) does not achieve this: Docker forwards the published port into the container, where the app still binds all interfaces internally, so the app never sees itself as loopback-bound and refuses to start. `HOST=127.0.0.1` is required, not optional -- left unset under `--network host` the app would bind all of the host's real interfaces. The loopback-only rule and why the API cannot sit behind a reverse proxy are in [Server job API](#server-job-api).
