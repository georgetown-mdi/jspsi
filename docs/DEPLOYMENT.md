...
Three things come with that choice, all of them because the container then runs as an account the image knows nothing about.

- **The per-user signing identity has nowhere it can be written.** What `psilink fingerprint` creates lives under the home directory rather than the working directory; add `--env HOME=/work` to put it in the mounted directory, where it also outlives `--rm`. Not when `/work` is the partner-synced folder, though: in a single-folder file-drop exchange that directory is one the partner writes into, and certificate-mode signing would be putting this party's long-lived P-256 private key there. Mount the synced folder separately from the working directory in that case, or point `HOME` at a mount only you can reach.
- **The account has no user name, so supply the identity yourself.** The identity psilink puts in the linkage terms -- the label your partner reads as your name, there and in the invitation and the exchange record -- defaults to the user name of the account psilink runs as, and an account the image does not define has none. That holds for the unsuffixed and `-fips` images alike. Pass `--identity "name, org, contact"` to `psilink invite`, `psilink accept`, and the zero-setup form; each stops with a usage error (exit 64) naming the flag rather than sending a label you did not choose. `psilink exchange` reads the label from `linkage_terms.identity` in the configuration and needs no flag.
- **`serve` wants the `chown` route rather than this one.** The console appliance writes container-internal state belonging to uid 1000.
