...
- A client that asks for a response and then stops reading it is closed once the connection has stopped draining, so an unread response cannot pin a connection open. A client that is reading, however slowly, is not cut, and neither is a slow handler or a quiet event stream: the server's own pace is not measured against the client's clock.
