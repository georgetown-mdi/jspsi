...
**Direct exchange: no invitation, terms inferred from the file.** Alongside the invitation flow above, the console lobby offers a third path, "Direct exchange," for when both parties already agreed on a server out of band: there is no invitation to create or accept. Each operator picks their own input file, the console previews the linkage terms and disclosed columns the CLI expects to infer from it and asks the operator to confirm them (and affirm the trust model below) before running, and the run itself drives the CLI's zero-setup command instead of a saved config -- see [Zero-setup exchange](CLI.md#zero-setup-exchange). It supports the SFTP and shared-directory (`filedrop`) channels only, exactly like the invitation flow above. It carries no shared secret and no application-layer encryption, so trust rests entirely on the transport (SSH/SFTP, or the shared directory's own access controls) and on whoever administers it -- the same posture the CLI's zero-setup mode carries on the command line, in contrast to the shared-secret model an invitation establishes (see [Bootstrapping a shared secret](SECURITY_DESIGN.md#bootstrapping-a-shared-secret)). Each party still writes its own self-attested exchange record of what it disclosed, exactly as an invitation-driven run does. The wire-level intent and the argv the server drives the CLI with are specified in [SERVER_JOB_API.md](spec/SERVER_JOB_API.md#the-zero-setup-intent).
...
- `JOB_SFTP_SERVER` -- the path to a mounted file naming the one SFTP server the appliance may connect out to, pinned at deploy time. Set it to fix the connection deployment-side; leave it unset and the operator authors the connection in the console instead (see the SFTP note below). (The superseded `JOB_SFTP_REMOTES`, which named a multi-server table, is no longer supported: a deployment that still sets it refuses to start with a migration message.)
- `JOB_SECRETS_DIR` -- the mounted directory the console browses for a connection's credential file (a password file or an SSH private key) when the operator authors an SFTP connection in-app. Unlike the input and rendezvous directories it has NO `JOB_DATA_ROOT` fallback: it is deliberately never defaulted into the client-writable data root -- the one place a credential directory must never be. In a single-mount console (only `JOB_DATA_ROOT` set) a credential file cannot be referenced at all: the secrets browser is unavailable and a typed `@path` under the data root is rejected, because that folder is writable during the exchange. To author a credential by file reference, mount a separate secrets directory read-only and name it -- add `--env JOB_SECRETS_DIR=/secrets -v /host/secrets:/secrets:ro` to the `serve` command (see [Docker deployment](#docker-deployment)) and restart. Without that mount the only in-console alternative is the paste fallback, which covers a password or private key only: `private_key_passphrase` has no paste form, so an encrypted private key cannot be authored at all in a strict single-mount console.
...
There is no token, so anything that widens that publish binding widens the unauthenticated API with it. Publishing without the `127.0.0.1:` prefix exposes the API on the host's other interfaces, and fronting the loopback port with a reverse proxy re-exposes it to everyone the proxy admits -- each is a deliberate choice to widen the reachable audience, not a supported appliance default. Keep the publish binding on host loopback and reach the API only from the host itself. The bundled Elastic Beanstalk reference also returns 404 for `/api/jobs` at the proxy, redundant defense-in-depth on top of the app's own hosted-build refusal; leave that denial in place -- the appliance is not deployed behind that load-balanced front.

**SFTP runs against one server -- provisioned or authored in the console.** Exactly one SFTP connection is effective at a time, set one of two ways. Provision it at deploy time with a `JOB_SFTP_SERVER` file: the browser then supplies nothing, and everything about the connection -- host, port, account, credential file references, and the pinned host-key fingerprint -- comes from that file, read once at startup and fail-closed (an invalid file refuses to start; changing the host or fingerprint takes a restart). Or, when no `JOB_SFTP_SERVER` is set, the operator authors the connection in the console: they enter the host, username, remote directory, and the required host-key fingerprint, and point the credential at a file -- one browsed from the mounted secrets directory (`JOB_SECRETS_DIR`) or given as a file-path reference. A de-emphasized fallback lets them paste the credential value directly when it lives nowhere as a file.

A file reference is the preferred path and never puts a secret through the web server: it is resolved only by the CLI child at exchange time. A pasted value does cross the appliance's loopback API and is written to a file so the exchange can use it -- but never to the data root or the partner-synced rendezvous directory; it goes to an internal, owner-only location that is wiped at every startup, and it is deleted when the connection is cleared or the exchange is deleted. This stays within the single-operator, on-host trust boundary the appliance already assumes. The host-key fingerprint is always a literal pin -- the appliance never prompts to trust a host key (stage a rotation by listing the old and new fingerprints together). A boot `JOB_SFTP_SERVER` wins: while one is set the console shows it read-only and does not offer authoring. An authored connection is held in memory, scoped to the single exchange, and forgotten on restart or when the exchange is deleted. The appliance runs one exchange at a time, and the container needs network egress and DNS to the server's host and port -- shared-directory exchanges needed none. The connection file format, the in-app authoring request, and each validation rule are in [SERVER_JOB_API.md](spec/SERVER_JOB_API.md#authoring-the-sftp-connection); the trust posture is in [SECURITY_DESIGN.md](SECURITY_DESIGN.md#single-party-appliance-trust-boundary).
...
With only `JOB_DATA_ROOT` set, the input listing and the filedrop rendezvous both default to that directory, so one mounted directory runs a full console: the operator drops their input CSVs into `/host/work`, and a shared-directory exchange rendezvouses there too. One thing the data root does not cover is an in-app SFTP credential authored by file reference: that needs its own read-only `JOB_SECRETS_DIR` mount, or the paste fallback (a password or private key only) -- the writable data root is deliberately excluded as a credential source (see the `JOB_SECRETS_DIR` note under [Server job API](#server-job-api)).
