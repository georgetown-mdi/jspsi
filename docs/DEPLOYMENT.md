...
- **What revoking stops.** New allocations under the key. An allocation already open is not cut; once its client stops refreshing it ends within the template's `max-allocate-lifetime` of 600 s. Whether an allocation that keeps refreshing ends at its credential's expiry is not yet measured.
