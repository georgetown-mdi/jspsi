...
- **Keep every exchange file** (retain mode) leaves the exchange's files in place as a permanent transcript instead of deleting each one once it has been read. Turning it on also turns on timestamped filenames and the lockless rendezvous, which it requires -- the same implication `--retain-files` carries at the command line.
