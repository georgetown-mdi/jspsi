...
- The refusal is narrow. It applies to a shared-directory exchange only, and only while the identity file is in a folder the partner writes into: an SFTP or WebRTC exchange out of the same one mount runs, a console that has never created an identity runs, and a rendezvous folder beside or below the mounted working directory runs.
