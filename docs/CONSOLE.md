...
**A shared-directory exchange out of a single mount is refused where your signing key is, or would be, in the shared folder.** The rendezvous directory falls back to `JOB_DATA_ROOT` when `JOB_RENDEZVOUS_DIR` is unset (see [Mounted work-input directory](#mounted-work-input-directory)), so on a one-mount console a shared-directory exchange syncs the same folder the signing identity is written into -- and that run would hand this party's long-lived P-256 private key to the partner.

- Its disclosure lets whoever holds it forge receipts under this party's identity for every exchange, with every partner, not only the one the folder is shared for. That is why the console stops the run rather than warning and proceeding: no later choice recovers a key the partner already has.
- The refusal is narrow. It applies to a shared-directory exchange only, and only where the run would put a key in a folder the partner writes into -- your identity file is already there, or the run asks for a signed receipt and would create it there. An SFTP or WebRTC exchange out of the same one mount runs, an unsigned shared-directory exchange on a console with no identity file runs, and a rendezvous folder beside or below the mounted working directory runs.
- Creating the identity is refused in that layout too, on the same terms: pressing **your fingerprint** with nothing on file yet would write the key into the shared folder, so the console says so instead of creating it. Reading a fingerprint you already have is not refused -- the file is where it is, and the run that would publish it is stopped on its own.
