...
- The console decides it from the mount paths, the real paths symlinks resolve them to, and the folder identity that catches one host folder bound in twice -- so a layout it cannot see through, such as the working directory bound in again under another name outside that chain, looks separate to it and the run goes ahead. It says so beside the signing control, on every layout but the one it has established as shared, where the refusal is the message.
