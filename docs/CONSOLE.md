...
**A shared-directory exchange out of a single mount is refused while your signing key is in the shared folder.** The rendezvous directory falls back to `JOB_DATA_ROOT` when `JOB_RENDEZVOUS_DIR` is unset (see [Mounted work-input directory](#mounted-work-input-directory)), so on a one-mount console a shared-directory exchange syncs the same folder the signing identity is written into -- and that run would hand this party's long-lived P-256 private key to the partner.

- Its disclosure lets whoever holds it forge receipts under this party's identity for every exchange, with every partner, not only the one the folder is shared for. That is why the console stops the run rather than warning and proceeding: no later choice recovers a key the partner already has. The shared folder is synced by your own tool the whole time, not only while an exchange runs, so a key in it is already your partner's before any run starts -- the refusal tells you the layout and leaves the fix to the mount, it does not take the key back.
- The refusal is narrow. It applies to a shared-directory exchange only, and only where your identity file is already in a folder the partner writes into. An SFTP or WebRTC exchange out of the same one mount runs, a shared-directory exchange on a console with no identity file yet runs, and a rendezvous folder beside or below the mounted working directory runs.
- Creating the identity is refused on no layout: pressing **your fingerprint** writes the key into the mounted folder wherever the rendezvous is, which is what makes a signed SFTP or WebRTC exchange possible with one mount. The one moment it is refused is while a shared-directory exchange is running out of that same folder -- creating the key then would put it in a folder that run is syncing as it goes, so the console asks you to create it once the run finishes, or to give the synced folder a mount of its own. A signed run with no identity file is not refused here either -- it fails at the exchange, which signs with the identity file you have and creates none for a run.
- Give the rendezvous a mount of its own to run one: set `JOB_RENDEZVOUS_DIR` to a dedicated directory, separate from and not nested with the working directory that holds your key, input, and results, and start the exchange again. That is the same split that section recommends for the partner-writable mount generally; signing is where skipping it costs the most.
- The console decides it from the mount paths, the real paths symlinks resolve them to, and the folder identity that catches one host folder bound in twice -- so a layout it cannot see through, such as the working directory bound in again under another name outside that chain, looks separate to it and the run goes ahead. What it reads for the key itself is one fixed file name in the mounted folder and nothing about the file, so a copy of your key under any other name is not seen either. It says so beside the signing control wherever a partner may be syncing the folder your key is in: what the refusal covers where it has established that layout, and what no refusal covers where it could not tell. Where the rendezvous has a mount of its own it says neither, since nothing your partner syncs holds your key.
- The separate mount is what removes the collision, not the refusal.
