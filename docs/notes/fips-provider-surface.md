...
The OpenSSL Project holds three active certificates under the module name "OpenSSL FIPS Provider": **4282** and **4811** (FIPS 140-2, module versions 3.0.8 and 3.0.9), and **4985** (FIPS 140-3, module version 3.1.2). The certified surface is therefore not one series. The variant image pairs with a vendor certificate rather than any of them -- AWS's **5438**, FIPS 140-3, module version `3.2.2-799901ad7ab41d45` -- whose tables are read in [CONTAINER_IMAGES.md](../spec/CONTAINER_IMAGES.md#what-certificate-5438-attests) and shown in the last column below for comparison.

The two algorithms this spike measured land differently on each certificate, and that difference is the whole answer:

| Algorithm | 4282 / 4811 (140-2) | 4985 (140-3) | 5438 (140-3) |
|---|---|---|---|
| X25519 | Table 7, **Allowed** | Table 8, **Non-Approved, Not Allowed** | In **no table** |
| Ed25519 | Table 8, **Non-Approved** | Table 8, **Non-Approved, Not Allowed** | In **no table** |
...
Certificate 4985's Table 13 (Non-Approved Services) has a **Key Derivation** row, "Derive keys (key derivation key passed in by the calling process)", over `X942KDF-CONCAT`, `X963KDF`, `HKDF` and `OneStep KDF`. Read on its own it says that HKDF with a caller-supplied key is a non-approved service -- which would put Alcove's whole key schedule, and the collapsed `deriveBits` call that
...
Alcove derives with HKDF-SHA-256 from 256-bit key-derivation keys to 256-bit outputs, so neither end of the derivation approaches the 112-bit threshold that row states and the row does not reach it.
...
The reading is 4985's alone, and certificate 5438 reaches the same answer through a differently arranged policy. Its non-approved *services* table is **Table 15**, whose "Key derivation" row contains KBKDF, `KDA OneStep`, `KDA TwoStep`, HKDF, ANS X9.42 KDF and ANS X9.63 KDF at "< 112-bit keys", with further entries conditioning `KDA OneStep`, `KDA TwoStep` and ANS X9.42 KDF on the SHAKE PRFs, ANS X9.63 KDF on those plus SHA-1, and the SSH, TLS 1.2 and TLS 1.3 KDFs each on a named hash list. Two things separate it from 4985's row. Its condition is a parameter condition stated inline rather than 4985's "key derivation key passed in by the calling process" description with the qualification deferred to another table, so the misreading above does not arise on that certificate at all; and its approved-services table has a key-derivation service for HKDF from a shared secret, which is the same corroboration 4985's Table 12 gives. Alcove's HKDF-SHA-256 from a 256-bit key-derivation key sits outside every condition either row states.

The table numbers do not travel with the reading, in either direction. 5438's non-approved *algorithms* table -- where `AES GCM (external IV)` sits -- is **Table 8**, which is 4985's number for it, while its approved-algorithm table is Table 6 against 4985's Table 5, its approved-services table Table 14 against Table 12, and its non-approved-services table Table 15 against Table 13. Agreement at one row is why a row cited by table number alone is unsafe, and every citation here names the certificate first. Part of the offset is 5438 stating both of its non-approved-but-allowed categories empty in prose ("N/A for this module") instead of carrying a table for each, where 4985 carries the Table 7 that X25519 sits in. What 5438 states about the module's HKDF beyond these placements is nothing at all: it attaches no use-context restriction, which [fips-variant-image.md](fips-variant-image.md) records.
...
That route was taken, and it moves the environment question rather than closing it. Certificate 5438's four tested environments are Amazon Linux 2023 on two bare-metal EC2 platforms, and the three it vendor-affirms are Amazon Linux 2023 on Snowball and Snowcone appliances; none of the seven is a container or a virtual machine, so the container is outside both sets on that certificate as much as it is outside 4985's tested set ([CONTAINER_IMAGES.md](../spec/CONTAINER_IMAGES.md#what-certificate-5438-attests)). 4985's flat denial above is still not a sentence to quote against it, for the opposite reason: 5438 affirms three environments, and the policy's own line beneath them is that CMVP makes no statement about a module so ported. What that supports a claim of is reasoned about in [fips-variant-image.md](fips-variant-image.md).
