---
title: "Fan-Out Matching: Local Resolution, and the Strategy It Ran Under First"
---

# Fan-out matching: why resolution is local, single-pass first, and greedy

*Status: built under both linkage strategies. The construction is normatively specified in [PROTOCOL.md](../spec/PROTOCOL.md#fan-out-matching-multi-value-key-candidates), with the operator-facing description in [EXCHANGE_REFERENCE.md](../EXCHANGE_REFERENCE.md#fan-out-multi-value-fields); the cascade's own realization landed after this note was written, and [cascade-fan-out.md](cascade-fan-out.md) records it. This note records why the rules have the shape they do, the alternatives weighed and set aside, and what remains open. See [docs/notes/README.md](README.md).*
...
2. **No additional protocol messages.** Pruning is local, computed from what the existing message flow already contains. Extending the content of a frame the exchange already sends is in scope; adding a frame or a round is not.
...
The cascade could not reach the same place on the frames it had, and the reason is a timing defect rather than a disclosure one. The cascade's per-round exchange sends positions in that round's candidate set; the translation of matched positions into the partner's original row indices happens AFTER the per-key loop has closed, in the final mapped-element passes (`exchangeMappedElements` in `packages/core/src/psi/link.ts`). Fan-out resolution is exactly what decides which records leave the candidate set for the NEXT round. Grouping delivered on the final frames therefore arrives after every candidate set it was supposed to inform was already committed: the two sides can remove different records at round `j` and diverge, which is the hazard above with the clock added.

So the alternative that looked cheapest -- send the matched-value record grouping on the cascade's existing final table exchange -- is not merely more disclosive, it is incorrect for the cascade removal semantics. A correct cascade fan-out has to extend the PER-ROUND frames instead, so that round `j`'s resolution is known before round `j+1`'s candidate set is formed. That is a larger change, and it lands in the innermost security-reviewed loop of the protocol, so it was taken as work of its own; the resolution rule is fixed normatively here precisely so that work reproduces this table rather than inventing a second one, and [cascade-fan-out.md](cascade-fan-out.md) records what it found.

Shipping the feature on one strategy first also reused machinery that already exists and is already symmetric. `linkage_strategy` is a mandatory-consistency agreed term, so a strategy-conditional refusal narrowed the standing `split_on` refusal rather than inventing a control: both parties refused in lockstep from the agreed pair.

**The cost that ordering carried.** `cascade` is the schema default and the web application's ordinary path, so while the feature ran under single-pass alone an operator reached fan-out by opting into a *different strategy*, inheriting that strategy's larger disclosure (the receiver sees the sender's whole per-key value structure and the weaker-key matches the lockstep cascade filters out) and its tighter dataset ceiling. That coupling was a real reduction in reach, which is why it was stated in the disclosure rows of the spec rather than left for an operator to discover, and it is what the cascade realization removes.

One asymmetry in a refusal of this kind deserves its own line, because it is the one place such a refusal is not symmetric: a field-level fan-out lives in a party's own data standardization, which is per-party and local and which no invitation states. The partner cannot derive that refusal from the agreed terms, so it has to land in front of the operator at preparation time -- before a connection exists to leave hanging -- and, if it somehow reaches the run boundary, as an abort stating its reason rather than a silent decline. An element-transform fan-out rides the agreed terms and needs no such handling.
...
- **Role-derived components.** Sender and receiver are fixed for the exchange by role resolution, from counts both parties hold. An order phrased in terms of "local" and "partner" rows would name different things on the two sides; one phrased in sender/receiver names the same thing on both. That is what lets a cascade round, where each party resolves its own, compute the identical order.
- **It is an order the frames already hold.** Message 3's local half is the sender's own rows in ascending order, and that ascending order is enforced at the single-pass boundary. Ordering the sweep sender-major means the resolution order and the frame's canonical order are one order rather than two.
...
A maximum-cardinality matching would pair more records in an ambiguous round than a greedy sweep does. It was set aside: it needs a canonical tiebreak anyway (so it does not avoid this decision, only adds to it), it is more work in the innermost replay, and "more pairs" is not obviously the better answer when the ambiguity means the evidence was contradictory. The greedy's bias is clear -- the lowest-numbered sender record wins its lowest-numbered candidate -- and clarity is worth more here than cardinality.
...
Constraint 1 is implemented as: every record appearing in ANY of round `j`'s record-level candidate pairs leaves candidacy for rounds after `j`, accepted or not. The consequence: a record whose candidates matched but which lost the sweep ends the exchange unmatched -- it does not fall through to a less precise key.

That is the intended reading of the cascade, not a compromise. The keys are ordered most precise first; a record whose candidates produced contradictory evidence on a precise key has not earned a match on a weaker one. The alternative -- returning a contradicted record to candidacy -- would let an individual match on the weakest key in the set precisely because the strong keys disagreed about them, which inverts the cascade's whole ordering assumption.
...
The consent surface follows the same decision, and it states both cases rather than one: an element declaring a fan-out earns a breadth marker where the strategy matches its candidates, since the element fans out and the marker must say so, and keeps the "not supported" marker where the exchange refuses the terms outright, since a run that does not happen has no breadth to name.

## The width bound: value, and what happens above it

The bound exists because the single-pass cap arithmetic rests on an assumption fan-out breaks: that `keyCount * recordCount` upper-bounds a party's distinct-value count. With several values per cell that assumption is false, and every derived frame and element bound loses its footing. So a per-(record, key) candidate cap is not hygiene here; it is what restores the derivation from authenticated state.

**Why 20.** It is an arbitrary working value with no privacy, protocol, or disclosure meaning. It was set where it covers the honest shapes seen so far with margin -- a hyphenated name plus a compound field is sixteen candidates at four parts each -- and nothing else picks it. Raise it whenever a use case needs more; the only constraint is arithmetic, since the factor multiplies into the template-wide width total that keeps the slot product exact in a double, which the spec's figures and the tests pin. The operator advisory raised for a wide per-record expansion takes this figure as its own threshold, whatever width the key it fires on declares; it shares the number rather than explaining it.
...
An earlier design included each party's effective key count as an advertised integer on the terms-exchange envelope, held to a range, a divisibility rule, and a floor the agreed terms imply, with a run-boundary notice for a party running wider than that floor. The maintainer ruled that mechanism out and replaced it with the derivation the spec now states: every per-key width is a function of the agreed terms alone, so both parties compute the same vector with no field, no round-trip, and no validation surface. What the advertisement bought was the one case the terms cannot show -- a party's own standardization fanning out -- and it bought it at the price of a role-blind over-acceptance: each party had to accept any width up to twenty times the agreed floor, because it could not tell a legitimate local fan-out from an inflated claim.

That case is covered instead by the party's DECLARED RECORD COUNT, which multiplies by the fan-out factor its own cleaning declares. Data cleaning is the party's own business, and a party that pre-fanned its file outside Alcove would present identical wire behavior, so nothing there is enforceable or newly disclosed. It removes a wire field rather than adding one, and it leaves the width a checkable function of terms both parties consented to rather than a number one party asserts about itself.

The consequence to state is that role resolution reads the fanned count, so a fanning party is compared at the larger figure and trends toward *sender*. That is proportionate -- the fanned count is closer to the work that party's rows actually cost than its raw row count is -- and it is the direction that makes the declaration harmless. Single-pass has exactly one resolver, and it is the receiver: it holds the sender's full per-key structure (above). So a count only the declaring party can set moves that party onto the side whose structure is read, never onto the side that reads it; the fan-out declaration buys no seat, which is why nothing has to check it. The direction is measured rather than asserted: the fan-out role case in `packages/core/test/config/derivedKeyWidth.test.ts` runs a fanning party against a larger partner and pins its resolved role at `sender`.

**A derivation that tightens.** Bounding the effective key count keeps the single-pass gate's product exact in a double, but leaves less headroom than the raw key count does. The invariant that pins that headroom is therefore pinned against the effective key count, and it is a live constraint on any future raise of the ceiling. The arithmetic and its figures are the spec's, under **The exact-integer premise holds where it was** in [the width bound](../spec/PROTOCOL.md#the-width-bound-a-per-key-candidate-cap-the-terms-declare).
...
A related question is whether the width should be included in `linkage_terms`, refused by both parties symmetrically on a mismatch before anything runs, and stated on both consent surfaces. The three-panelist design panel that converged 3-0 on PR #969's notice mechanism routed that question to the maintainer rather than deciding it themselves. The maintainer decided against consent, and the derivation above decides most of what the question was about.
