...
_Status: decided on the maintainer's ruling; the refusal and the identity-location option are both built. The behaviour is specified in [SERVER_JOB_API.md](../spec/SERVER_JOB_API.md#refusing-a-run-that-would-publish-the-signing-identity), the option in [Where the identity is: the console's option](../spec/SERVER_JOB_API.md#where-the-identity-is-the-consoles-option), with the operator-facing account in [CONSOLE.md](../CONSOLE.md#signing-a-receipt-and-noting-where-the-result-is-filed); this note records the posture and why it was taken rather than restating those rows. The CLI's own posture is [signing-identity-custody.md](signing-identity-custody.md), the decision this one was left out of. See [docs/notes/README.md](README.md)._

The console creates and reads this party's signing identity in the one working directory the operator mounts, unless they point it somewhere else. On a single-mount console that directory is also the folder a shared-directory exchange rendezvouses out of, because `JOB_RENDEZVOUS_DIR` falls back to `JOB_DATA_ROOT` -- so the long-lived private key sits in the folder the partner writes into, and the operator's sync tool hands it over. Advisory copy on the receipts card is the console's whole answer to that layout without this decision; the decision adds a refusal for the one run where copy is not enough, and holds the copy to what the refusal does and does not see.
...
The other half of the refusal is a presence probe: one file name at each location the console knows -- the fixed name under the mounted working directory, and the file the operator picked where they picked one. Those names, and whether something is at them, are the whole of what the console knows about the key. The receipts card says so on the layouts where a partner syncs the key's folder or the console could not rule that out -- what the refusal covers where the shared layout was established, and what no refusal covers where it was not. Where the report positively says the rendezvous has a mount of its own the card raises neither: nothing about the key is in a folder the partner reads, and a warning shown on the recommended layout as well as the hazardous one tells the operator nothing about which they are in.

### Limits accepted, not closed

These are accepted under the deployment the console is for: the partner is an authenticated party under a signed agreement, and one writing into the synced folder to attack this party is not the case being designed against. They are recorded here, and in the spec's refusal section, so the refusal is not read as a control against a hostile partner. The last two are not about the partner at all: they are windows the console's own checks leave open, recorded rather than gated.
...
- **A picked location inside a synced leg takes the write the read leaves open.** The syncing refusal applies at the console's own default location only: it fires where nothing is at the path, and a picked location is read only where something is, so applying it there would refuse nothing. Where the operator has mounted the secrets directory inside a rendezvous leg, the file removed between the presence check and the child's load is created into a folder a shared-directory run is syncing as it goes -- the disclosure that refusal exists to prevent at the default location. Mounting the secrets directory read-only is what closes it; the refusal cannot.

The remedy for all of them is the one the refusal already names: keep the identity outside every folder a partner syncs. The identity-location option makes that a pick in the console rather than a mount change.

## Why the location is an option rather than a second default

The maintainer's ruling also has the key's location become a console option: the CLI's own `signing.identity_file`, prefilled with the default and changeable by browsing a mount, mount-relative and resolved server-side, exactly as the connection's credential file is browsed today.

Keeping it an option rather than moving the default is the same judgement the default's own section records. A console with one mount and one variable still makes a first signed exchange, and graduation copies the configuration unchanged either way, because the option composes the key the CLI already reads. What the option adds is a second remedy for the one layout the refusal is about -- move the key, rather than move the rendezvous -- available without restarting the console, which is what made the warning hard to act on.

Two properties follow from the console creating the identity at its default location ONLY:
...
- **The console writes nowhere but the data root, save the window above.** The certificate export lands there, and the fingerprint child's working directory is pinned there whatever the identity's location, so an `alcove.yaml` in the browsed mount never becomes the child's config.
