...
- **The console writes nowhere but the data root, save the window above.** The certificate export lands there, and the fingerprint child's working directory is pinned there whatever the identity's location, so a `psilink.yaml` in the browsed mount never becomes the child's config.
