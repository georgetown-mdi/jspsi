...
- **The unreadable-custody state**, which is this browser reading its own storage, refused before the input file and before connecting. No affordance on the surface makes the entry readable, so the sentence carries the read's own error, what the wrapper carries as its cause rather than the wrapper's own sentence, as the only diagnostic the operator has -- and no exchange reported it, so it takes the same treatment as `output`'s own-write cause above: this application's account of what it did itself, finishing the state's copy rather than standing under a label attributing it to the exchange. The same browser test measures that it lands in the copy and that the label is nowhere on the surface.

The rest withhold. The input, terms-shortfall and consent states state the cause in copy that is fixed and non-oracular by decision, so there is no second voice for a block to separate; the too-large state's live copy is the refusal's own message, composed from frame sizes and fixed constants, so a block would only repeat it; the unexplained state is withheld for the same reason the untagged `security` arm is. A state's placement moves by a decision of the same kind, and the unit test over `apps/web/test/unit/recurring/managedRunLaunchModel.test.ts` holds every state to the one recorded for it.
