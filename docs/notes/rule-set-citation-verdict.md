...
**Drop the citation.** Keep accepting, but strip the refuted citation from the derived terms, so the acceptor's record simply carries none.
...
**Three states, not two.** With a two-state flag, "not contradicted" would cover both "checked and matched" and "could not check" - so the absence of a warning would read as verification precisely where nothing was verified. `unchecked` is a distinct claim from `consistent`.
