...
## The schema rule beside it

The whole name class does hold a schema rule ([CHANNEL_SECURITY.md](../spec/CHANNEL_SECURITY.md#linkage-terms-name-class-character-rule)), decided on its own grounds rather than on the display grounds weighed here: a name reaches each party's exchange record verbatim, where a reader is somebody else's tooling and no display boundary of this implementation's stands. Taking it class-wide is what answers the first objection above; it being fail-closed for a reason outside a message is what answers the second. The two controls are not alternatives. The treatment still covers what the schema leaves length-bounded -- a transform param value and an `allowedCharacters` class -- along with every free-text value and the values a permissive reader hands back from a recorded artifact, and it is still the only one of the two that binds a clause Alcove composes rather than a document it parses.
...
Fidelity, in the direction the escape already costs it. An operator reading a value that held a line break sees a marker naming the code point rather than the character, and a value that spells the marker's own text renders as one that held it. That second reading is unclosable in printable ASCII and is the open class the truncation marker already has; what it leaves an operator is the converse -- a control character Alcove itself composed still renders as the escape's own token, and no value can produce that.
