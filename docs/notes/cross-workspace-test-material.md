...
markers are each enforced or written by core itself, and what a caller does is
send a message, decode an invitation, or read rendered text. They moved to
`./testing`, where each app's suite builds a frame or drives a bound at its
