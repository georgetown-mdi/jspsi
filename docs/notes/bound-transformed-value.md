...
Where a declared fan-out producer realized the row's multiplicity, though, the fate is the drop that producer's own width bound specifies: the transforms are the partner's while the values expanded are this party's rows, so a refusal would let a partner end an exchange by authoring a delimiter that shatters one local value. Both assembled limbs take that fate, and so does the accumulating bound at both of its charges. At the charge a listed producer's expansion makes, the step being accumulated is that producer. At the other -- the row's total across the elements and the field's own realized values, an aggregate no one step is expanding -- the binding is the row's own provenance: a listed producer expanded an element already built, and no unlisted producer expanded any, which is the binding the assembled limbs read too. Multiplicity an unlisted producer realized is recorded before either charge can fire, so that case stays fail-closed on the refusal, and so does an aggregate charged before any element has fanned out: the provenance is what the elements already built carry, and partial knowledge resolves fail-closed.
...
- **A partner can still abort a run deterministically** on the paths that refuse -- the per-value ceiling, a crossing no declared producer accounts for, and a projection no declared producer expanded -- by tuning an amplifier to cross one of them, and learns one coarse bit about the local data's magnitude per consented exchange from whether it does. That trade was taken deliberately: an abort is visible and attributable to whoever authored the step, where an unannounced narrowing -- a value clamped to fit, a candidate set silently shrunk -- is neither. Where a declared fan-out producer expanded the row the same tuning buys the warned drop instead, which is the width bound's own trade rather than a second one.
