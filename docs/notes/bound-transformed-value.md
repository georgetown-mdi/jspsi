...
Where a declared fan-out producer realized the row's multiplicity, though, the fate is the drop that producer's own width bound specifies: the transforms are the partner's while the values expanded are this party's rows, so a refusal would let a partner end an exchange by authoring a delimiter that shatters one local value. Both assembled limbs take that fate, and so does the accumulating bound at the charge a listed producer's expansion makes -- the step being accumulated is that producer, and multiplicity an unlisted producer realized earlier in the row has already been recorded, so that case stays fail-closed on the refusal. What the accumulating bound does not read a producer at is its other charge, the row's total across the elements and the field's own realized values: that is an aggregate no one step is expanding, so it refuses whatever the row's provenance, as a single-valued step's output does.
