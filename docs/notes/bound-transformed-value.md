...
The fate on exceedance is a refusal, never a clamp. Both parties must derive byte-identical keys, so a value shortened to fit would match nothing and would surface as a mismatch against the canonically hashed agreement -- the same reason every transform-parameter control refuses rather than trims.
...
- **A partner can still abort a run deterministically** on the paths that refuse -- the per-value ceiling, a crossing on a key no declared producer can expand, and a projection no declared producer expanded -- by tuning an amplifier to cross one of them, and learns one coarse bit about the local data's magnitude per consented exchange from whether it does. That trade was taken deliberately: an abort is visible and attributable to whoever authored the step, where an unannounced narrowing -- a value clamped to fit, a candidate set silently shrunk -- is neither. Where a declared fan-out producer accounts for the width the same tuning buys the warned drop instead, which is the width bound's own trade rather than a second one.
