...
One correction to the "sender learns nothing" ideal above, so this note does not overclaim what the shipped `single-pass` provides: it gates what the tool *emits*, not what crosses the wire. In `single-pass` the receiver computes the whole cascade locally and returns the sender its own half of the association table - the sender needs its matched rows to build any payload it discloses for the overlap, and to receive its result in the both-output case - and only the record and the returned result are then gated by output entitlement. So a non-receiving helper's *process* still receives, and so learns, its own membership under `single-pass` today, exactly as under the clean cascade; enforced non-output keeps the tool from emitting or persisting it, but does not blind the helper at the wire. Realizing the idealized blindness for the one case where it is achievable - a non-receiving helper that discloses no payload - means withholding that table from it at the source, and is an open task; it must suppress the table frame entirely rather than send an empty one, since an empty-versus-populated table leaks the match count by its size.
