...
Since this note was written, this pole has been implemented as the opt-in `single-pass` linkage strategy (the cascade stays the default). It batches every key into one exchange so the round-trip count is constant in the key count, and is mandatory-consistency, so both parties agree to the extra receiver-side disclosure before any data moves; paired with enforced non-output (below), the shipped tool emits only the cascade-equivalent table. The wire shape and disclosure are specified in [PROTOCOL.md](../spec/PROTOCOL.md#linkage-strategies-cascade-and-single-pass) and summarized in [EXCHANGE_REFERENCE.md](../EXCHANGE_REFERENCE.md#linkage_termslinkage_strategy).
