...
Since this note was written, the send-everything pole has been implemented as the opt-in `single-pass` linkage strategy (the cascade stays the default). It batches every key into one exchange, so the number of round trips stays the same no matter how many keys there are. Both parties must agree to use it, so they both consent to the extra receiver-side disclosure before any data moves; paired with enforced non-output (below), the shipped tool *emits* only the cascade-equivalent table. The wire shape and disclosure are specified in [PROTOCOL.md](../spec/PROTOCOL.md#linkage-strategies-cascade-and-single-pass) and summarized in [EXCHANGE_REFERENCE.md](../EXCHANGE_REFERENCE.md#linkage_termslinkage_strategy).

One refinement to the "sender learns nothing" ideal above, so this note tracks what the shipped `single-pass` provides. In `single-pass` the receiver computes the whole cascade locally and returns the sender its own half of the association table - which the sender needs to build any payload it discloses for the overlap, and to receive its result in the both-output case. Where the sender needs neither - a non-receiving helper that discloses no payload - that table is now withheld at the source: the receiver suppresses the frame entirely and the helper skips awaiting it, so the helper's *process* never receives, and so never learns, its own membership. Both parties derive that decision from symmetric authenticated session state (the sender's output entitlement and its `disclosesPayload` flag, carried on the terms exchange), so the suppression and the skip stay in lockstep; and the frame is suppressed rather than sent empty, since an empty-versus-populated table would leak the match count by its size. This realizes the idealized send-everything blindness for the one case where it is achievable; the wire shape is in [PROTOCOL.md](../spec/PROTOCOL.md#linkage-strategies-cascade-and-single-pass). The membership disclosure remains intrinsic and threat-model-accepted in the two cases that cannot be closed without changing the choreography or dropping a capability: the clean cascade (the helper must learn its matches to drop them) and a payload-disclosing helper under `single-pass` (it needs its matched rows to enrich the overlap). In those, enforced non-output still keeps the tool from *emitting* or persisting the table, but does not blind the helper at the wire - the same gate on what the tool emits, not on what crosses it.

One subtlety is a necessary condition for the sender's blindness in that idealized pole: the receiver must keep feeding its *full* set every round and never shrink it. If it dropped its own matched records to save bandwidth, the sender would watch the set shrink and read the per-round match counts straight off the size. Holding the size constant is what would buy the blindness - necessary, but not by itself sufficient in the shipped `single-pass`, per the correction above.
...
| Send-everything | dataset size only in the ideal; the shipped `single-pass` returns the sender its table unless it is a non-receiving helper disclosing no payload, now withheld (see the refinement above) | fuller match structure in the overlap | high bandwidth |
