...
There is a tempting shortcut worth heading off. A cardinality-only exchange looks as though it could hide *which* records are shared by having the sender shuffle the receiver's data before returning it, so the receiver counts matches but cannot identify them. (This is not how the count-only algorithm works: psi-c uses the PSI library's own count-only mode, and a round-consistent shuffle is deliberately no part of it - [psi-c-count-only.md](psi-c-count-only.md) has the reasoning.) The natural question is whether such a shuffle could hide which of the *sender's* records matched - keeping the clean cascade without ever telling the sender anything.
