...
An invitation carries one linkage-terms document, and `deduplicate` is per-party. So an accepted invitation has to get the acceptor's own value from somewhere, and there were three places it could come from.

**Copying the inviter's.** This is what a naive derivation does, and it is the one reading that is unsafe. The value an invitation carries is one document and the value its author presents at the terms exchange is another, so an inviter could carry `true`, have the acceptor take the "many" side and disclose how its records group, then present `false` and run the pair as many-to-one at the acceptor's expense. The acceptor would have consented to a widening of its own data's disclosure that it never asked for.
...
**Deriving it as false.** The answer taken. It rests on reading the term as what it is: a statement about the DECLARING party's own data, not a property of the exchange the two parties negotiate. An invitation is the inviting party speaking about its own side, so it carries that side and no more; the acceptor's side starts closed and stays closed unless that party authors it.

Three things follow, and they are why this is preferred to the refusal it replaces.

- The unsafe reading becomes unrepresentable rather than refused. There is no value an invitation can carry, and no value its author can present at the terms exchange, that puts the acceptor on the "many" side -- so the flip is closed by the shape of the derivation rather than by a check that has to fire.
- What the acceptor consents to is a run that happens. The disclosure a deduplicating match makes runs toward the party that receives the result, which under an accepted invitation that shares one is the acceptor: it reads the grouping, it does not pay it. Where the inviter is the sole receiver, psilink presents the acceptor no group sizes and no row positions -- a display-scoped withholding, not a wire one: the matching rounds still carry that grouping to the acceptor's own software, as [shared-consent-summary.md](shared-consent-summary.md#where-the-acceptors-non-receipt-is-held-and-where-it-is-not) records. That is a different sentence rather than the same one, so the consent surfaces carry both, selected by the invitation's output shape.
...
The cost is a real asymmetry and worth stating: from an invitation, only the inviting party can be the "many" side. An acceptor that wants ITS records grouped cannot get there by accepting, and both parties fall back to the per-party configuration path. Whether to close that by putting the choice to the acceptor at accept time is left open below, because it is a consent-surface question rather than a derivation one.

The derivation hides one combination from the acceptor's own document, and the refusal timing is set so that it is answered anyway. The acceptor derives its own side as `false`, so a crafted invitation pairing `deduplicate: true` with `linkage_strategy: single-pass` -- the one deduplicating combination no strategy matches -- is in shape in the document the acceptor holds and would otherwise reach the agreed-terms boundary before anything answered it, having first rendered a consent display for a run that cannot happen. The accept path therefore reads the INVITING party's terms for that pair, alongside the count-only shape rules it already reads there: `linkage_strategy` is a mandatory-consistency term, so the invitation's value is the agreed one, and the combination is answered before any consent display and before any connection. The agreed-terms boundary keeps the same refusal for the pair no accept path sees -- two separately authored configurations, where the combination is the partner's own document and arrives at the terms exchange -- and it stays symmetric there, each party asserting over both documents.
...
- **An accept-time choice of the acceptor's own side.** The acceptor's `deduplicate` is derived rather than asked for (above), which settles the invite-and-accept path without settling whether an acceptor should be OFFERED the choice. Offering it is a consent-surface question of its own: an accept-time control changes what the accepting party is agreeing to about its own data, and the accepted terms are already agreed by the time it would be answered.
