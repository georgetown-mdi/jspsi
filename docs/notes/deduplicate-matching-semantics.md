...
- What the acceptor consents to is a run that happens. The disclosure a deduplicating match makes runs toward the "one" side, which under an accepted invitation is the acceptor: it reads the grouping, it does not pay it. That is exactly what the consent surfaces already stated, so stating it ahead of a run that now proceeds took no new claim -- only the retirement of the note saying the run would not happen.
- The default is the narrow one. `false` is one-to-one matching, the behavior every non-deduplicating exchange already runs, so an acceptor that reads nothing and consents gets no widening of its own side.
