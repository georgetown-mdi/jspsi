...
The refusal timing moves with the derivation, and in one direction. Under the refusal that was held while the question was open, an invitation carrying the term was answered at accept, before any connection. Now the acceptor derives its own side and accepts, so a crafted invitation pairing `deduplicate: true` with `linkage_strategy: single-pass` -- the one deduplicating combination no strategy matches -- is answered at the agreed-terms boundary instead: after the connection and the terms exchange, before any PSI frame, and symmetrically, since each party asserts over both documents. What that costs is a connection and a terms exchange, both of which carry what the two parties would have exchanged anyway; what it does not cost is a round, a key, or a payload.
