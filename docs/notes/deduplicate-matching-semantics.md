...
- **The `deduplicate` term on the invite-and-accept path.** Acceptance adopts the inviting party's declaration verbatim rather than mirroring it, the known limitation recorded on `deriveAcceptedLinkageTerms` in `packages/core/src/config/linkageTerms.ts`, so an accepted deduplicating invitation reaches the run as the both-sided pair and is refused as `many-to-many`: a deduplicating exchange is reachable from two separately authored configurations rather than through invite-and-accept. Closing it is a consent-surface decision -- mirroring the term to `false`, or putting the choice to the acceptor at accept time -- about what a party agrees to on its own side of the cardinality, and it is left open rather than settled by the adoption rule.
