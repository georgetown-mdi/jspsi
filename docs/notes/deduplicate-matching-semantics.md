...
**Copying the inviter's.** This is what a naive derivation does, and it is the one reading that is unsafe. Nothing binds the value an invitation carried to the value its author presents at the terms exchange, so an inviter could carry `true`, have the acceptor take the "many" side and disclose how its records group, then present `false` and run the pair as many-to-one at the acceptor's expense. The acceptor would have consented to a widening of its own data's disclosure that it never asked for and that no check binds.
