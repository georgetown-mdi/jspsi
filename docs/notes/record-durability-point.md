...
A send the transport settles as not having happened does not open the region, so no record is owed on it -- what the operator gets there is the run's own error and its entry on the machine-readable event stream, as for every failure before the send. The linkage-key material an earlier cut discloses is likewise outside what this artifact attests.

One rejection is not settled that way, and it does open the region: a publish the file-sync transport reports as indeterminate may already be in the partner's directory ([EXCHANGE_RECORD.md](../spec/EXCHANGE_RECORD.md#when-a-record-is-owed)). Keeping it outside was the alternative, on the ground that the run's own error already tells the operator the message may or may not have reached the partner. It was not taken: that error is a run-diagnosis trace, and the artifact the accounting is kept in would hold nothing for a disclosure that may have occurred. Opening the region follows the point's own principle -- the record attests the hand-off, and this is a hand-off the transport could neither confirm nor retract. The cost is a record for a payload that may never have left, which is the over-reporting this point already accepts for a frame buffered at close and the conservative direction for an accounting.

Two smaller questions came with it, both settled the same way the section above settles them. Such a record takes no mark of its own: the terminated `outcome` and the commitments beside it already say what it attests, and distinguishing it would cost the format move weighed under "One outcome value for the whole region" for a distinction the run log already holds. And both legs take the treatment, the responder's record committing the partner payload that had arrived before its own send -- the received-payload commitment is what this party received, on whichever leg the run ended.
