...
**Bounding a `transpositions` element.** Its count is one candidate per PAIR of the value's positions, so it grows with the square of the width: 46 candidates at 10 characters, 991 at 45, and past what one key may declare at 46. An element declaring `transpositions` must therefore bound its value with a `transform` that fixes a width of at most 45 characters -- a `substring` to a length, a `phonetic` code, a `parse_date` layout -- and terms whose element bounds none are refused before the exchange runs. Two `transpositions` elements in one key compound, so each of the two must be bounded to at most 8 characters.
