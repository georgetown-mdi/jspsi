...
One string is not a label: `REPLACE_WITH_YOUR_IDENTITY`, the placeholder [`psilink init`](CLI.md#initialization) writes into a fresh template. A command that reads a label refuses that exact value -- alone in the field or on `--identity`, whitespace around it or not -- as firmly as it refuses none at all (see [Configuration](CLI.md#configuration)).
