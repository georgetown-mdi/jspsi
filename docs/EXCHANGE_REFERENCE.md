...
A declared name holding those characters is refused rather than reconciled against the header. Where it is a `linkage_fields` name, an element `field` or `swap` reference, a `payload` column name, the legal agreement's `reference`, or a rule set's `name`, the name rule above refuses the document outright; where it is a `metadata` `name`, the same shape refuses it here, under a message naming this block. Either way the configuration fails to load, naming the field by its issue path -- `linkage_fields.0.name` for a terms name, and a bare `0.name` under a prefix naming the `metadata` block -- and printing none of the value, and no exchange runs -- so no column name you declare reaches your partner with these characters in it, the invitation's disclosed payload column list included. A `standardization` `output` or `input` is outside the rule: it names a linkage field and a column of your own input, and neither is sent.
