...
Under `certificate` signing mode this otherwise-free-text field is effectively pinned: it must match the identity bound into the party's signing certificate, or the partner rejects the receipt. [`psilink fingerprint`](CLI.md#signing-identity-fingerprint) warns when it binds a certificate to a different string, so the mismatch surfaces where the certificate is created rather than at the partner's exchange. See [Signing](#signing).
