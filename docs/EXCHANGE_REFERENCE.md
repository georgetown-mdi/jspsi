...
**Zero is a valid value** and means what it says on both counters: connect once without a dialing retry, and treat the first mid-run drop as terminal. A value above the ceiling is refused with a usage error (exit 64) before anything is dialed, whether it came from `psilink.yaml` or from `--max-reconnect-attempts`.
