...
A column name that holds invisible text-direction characters loses them at every read (see [CSV header sanitation](spec/CHANNEL_SECURITY.md#csv-header-sanitation-at-ingestion)), and the seat that read the file states which column positions changed. Two consequences reach this block. A name made only of those characters comes back empty and meets the refusal above, which states the removal as the cause instead of the header-row causes. And a name written as the header was typed -- a `metadata` `name`, a `linkage_fields` `name`, or a `standardization` output -- matches no column of the file: rename it to the column without those characters, which is the name `psilink init` writes into a configuration it authors from that file. The linkage refusal states that cause where a linkage key depends on the declared name. A `metadata` `name` for a column that is only sent as payload satisfies every key, so nothing refuses it: the read leaves it as declared, and your partner receives it with those characters in it. Rename that one yourself.
