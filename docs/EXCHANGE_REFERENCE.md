...
>
> **A candidate set refuses the pair:** terms that match a record on several values -- a [`split_on`](#fan-out-multi-value-fields) transform, a [`generate_fuzzy_comparisons`](#element-fields) expansion, or a key declaring [`swap`](#swapped-keys) -- are refused under `true` on both parties whatever the strategy, when the invitation is accepted and again once both parties' terms are agreed. [The built-in rules](#the-built-in-rules) declare a swapped key, so a both-sided match over the shipped default keys meets that refusal: turn that key off, or leave one party's `deduplicate` at `false` and run the many-to-one match.
>
