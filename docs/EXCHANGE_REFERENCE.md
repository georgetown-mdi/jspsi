...
- **`lockless_rendezvous` (and therefore `retain_files`).** The run fails within a fraction of the peer timeout, naming the file and asking you to remove it once you have confirmed no other session is using the directory. It does not delete the file -- a hello it cannot prove is its own is not its to remove -- so every re-run fails the same way until you clear it. Delete the named file, or use `--sweep-exchange-files`, whose assertion is the same one you are making by hand.
