...
The rule covers those four fields and no others. The string values of a [transform step's](#transform-steps) `params` -- a regex pattern, a replacement, a delimiter, a default -- are bounded at 1000 characters each and held to no character rule, because a step's bytes are its meaning; what neutralizes a control character in one of those is the display escaping psilink applies wherever it renders a value, the same boundary that covers every other partner-supplied string on its way to a screen, a log, or an error.
