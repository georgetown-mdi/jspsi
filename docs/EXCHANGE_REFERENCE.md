...
One feature requires `single-pass`: matching on the several values a `split_on` produces runs under it only, so terms declaring a fan-out under `cascade` are refused rather than matched on one value, until the cascade realization the protocol specifies is built (see [Fan-out (multi-value fields)](#fan-out-multi-value-fields)). Once fuzzy expansion ships, a key declaring `swap` reaches the same restriction for the same reason: matching the pair in either order doubles that key's declared width, so `cascade` refuses those terms rather than matching the one order the terms name, until that same realization lands.
