...
**Regex patterns run under a linear-time engine.** The four regex functions (`replace_regex`, `extract_regex`, `filter_regex`, and `split_on`) take a `pattern` (a `delimiter` for `split_on`) that is compiled and run per row over the full dataset. Because a partner authors these patterns and they execute on your data, they run under a linear-time regular-expression engine (RE2 semantics) rather than the JavaScript engine, so no pattern can hang the application through catastrophic backtracking -- this is guaranteed by construction, not screened for. The cost is a restricted **dialect**: a pattern may use the common syntax (character classes, anchors, quantifiers, groups, alternation, and Unicode classes) but **not** backreferences (`\1`) or lookarounds (`(?=...)`, `(?<=...)`), the features that require backtracking, nor the `\uHHHH` escape (use `\x{...}`). A pattern outside the dialect is rejected at terms validation, before any row is processed; the full dialect is specified in [PROTOCOL.md](spec/PROTOCOL.md#transform-regular-expression-dialect). Two further details follow from RE2 semantics: `split_on` does not emit capturing groups in the delimiter as output parts (unlike JavaScript's split), and patterns that backtracked catastrophically on the old engine (e.g. `(a+)+`) now simply run in linear time rather than being rejected. `parse_date` runs on the same engine: its `input_format` is expanded into a regular expression internally, so a format with many adjacent variable-width tokens (e.g. `MMDDMMDD...`) is bounded by the engine rather than rejected.
