...
A `split_on` step fails closed at the same points under a combination that matches one value per record, whether it sits in a `standardization` or in a linkage key's element transform: a count-only exchange, and a `deduplicate: true` both parties declare, are refused rather than run with the narrower matching they would deliver (see [Fan-out (multi-value fields)](#fan-out-multi-value-fields)). The web app authors no `split_on` at all: its cleaning-step menu does not offer the function, and a terms document that declares one -- in a cleaning step or a linkage key's element transform -- blocks generation. A fan-out is therefore authored in a configuration file, as the example above does, and run with `psilink exchange` or minted with `psilink invite`.
...
Two combinations refuse a fan-out rather than matching on one candidate, when the terms are authored or minted into an invitation, at data preparation, and again once both parties' terms are agreed: the count-only algorithm [`psi-c`](#linkage_termsalgorithm), whose round counts matched values and would report more links than the exchange stands for, and `deduplicate: true` on both parties, which resolves to a many-to-many match no strategy pairs with a candidate set.

**What the accepting party is told**: an invitation whose linkage keys split a value marks each such key element as matching on several values, and states the consequences the acceptor consents to -- matching per candidate, the record leaving the later keys once any candidate matches, and the candidate grouping the exchange's resolver is handed. Under a combination that refuses one, that same invitation states the refusal instead. A fan-out authored in your own `standardization` is not on that surface at all: a standardization is per-party and local, and no invitation holds one.
