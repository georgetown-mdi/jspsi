...
- The two directories must differ. This is checked byte-for-byte at config validation and again, after textual normalization, when the connection opens (both channels): differences that are only redundant separators, `.` segments, a trailing slash, or -- for filedrop -- backslashes are caught. Equivalences that cannot be settled client-side are NOT caught and are the operator's responsibility to keep distinct: `..` segments (which may cross a symlink), case (Windows filesystems are case-insensitive), and -- for SFTP -- a relative path versus the absolute path it expands to under the login home.
