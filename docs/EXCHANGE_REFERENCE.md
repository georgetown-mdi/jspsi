...
|-------|------|---------|-------------|
| `peer_timeout_ms` | integer | 3600000 | Milliseconds to wait for the partner at any single step before giving up, applied both to the initial rendezvous and to each message exchanged during the protocol; if the partner goes silent past this window the exchange fails with a transport error. The effective limit is the minimum of this and the remaining shared-secret lifetime. Must be a positive integer. Connection teardown after a completed exchange is bounded separately and short, not by this value. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `connection_per_poll` | boolean | false | When `true`, a fresh SFTP session is opened at the start of each poll cycle and released before the loop goes idle again, instead of holding one session for the whole exchange. Because each session then needs only survive a single cycle's seconds, it never reaches a server's maximum-session-duration or idle cap. Use it when the partner's SFTP server enforces a session-lifetime limit you cannot change and a single exchange spans many idle poll gaps (a slow peer that reconciles the directory only occasionally, so an exchange lasts hours). **Pair it with a long `poll_interval_ms`** (minutes-scale): the mode pays a full SSH handshake every cycle, negligible at a long interval but wasteful at a short one, so combining it with a sub-minute interval draws a CLI warning (the `--connection-per-poll` flag with no long `--polling-frequency` is flagged the same way). **Local, not bilateral**: how one party dials is invisible to the peer and to the shared directory, so it is not advertised in the hello and cannot cause a rendezvous mismatch -- one party may cycle its session while the other holds one, and each sets this independently. A within-cycle drop is still transparently re-dialed underneath the mode (see `max_reconnect_attempts`). The mode's own per-cycle dialing is not reported as a reconnection: the reconnect count and its warning are reserved for unexpected session losses, so a healthy exchange in this mode reports none at any poll interval, and a warning about a re-dial names a drop that really happened within a cycle. The one exception announces itself: if the release cannot determine whether it closed the session itself -- which takes a change in the underlying SSH library, and draws its own warning saying so -- it stops vouching for the re-dials that follow, rather than risk hiding a real drop. The cycling itself still reports nothing -- releasing at one boundary and dialing at the next adds no drop either way. What it can misreport is work that lands in the gap between two cycles, sending a message being the one that does, so a run whose log carries that warning may report a drop it did not have. A *separate* warning -- that the partner's server did not close the connection, so this side closed it -- names no drop either, and against some servers a healthy exchange draws it routinely: a server that accepts the disconnect and then leaves the connection half-open never completes the release, so the mode closes it from this side and dials a fresh session next cycle. It is paced (the first, then every tenth) and totalled once in the end-of-run summary. |
