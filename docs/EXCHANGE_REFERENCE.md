...
>
> **The run names what the pair resolved to:** nothing before the terms exchange can, since each party's value comes from its own file and the cardinality follows from the two together. So a run resolving to any deduplicating cardinality names it once the terms are agreed and before the first round -- on the CLI's log and its machine-interface warning stream, and among the run notices a browser seat shows. A both-sided run whose two declared record counts project more than ten million matched pairs adds an advisory naming that projection and what each side contributes to it, so you can tell which side drives it. Neither notice refuses anything and the run continues; the projection is the largest table those counts admit rather than a prediction of the match.
>
