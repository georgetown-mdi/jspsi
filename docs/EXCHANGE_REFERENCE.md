...
- `single-pass` -- batches every key into one exchange and has the receiver reconstruct the cascade locally. The number of round trips stays the same no matter how many keys there are, which makes a multi-key linkage practical over a high-latency channel (such as `filedrop` or `sftp`), where a chain of dependent rounds is prohibitively slow. It has a tighter scale ceiling than `cascade` -- set by the receiver's peak memory, which the exchanged record counts and key count bound. An exchange above that ceiling aborts cleanly on both parties with a clear error that explains how to fit it (reduce the number of linkage keys or records, or split the dataset into smaller batches); it does not recommend switching to `cascade`, since the strategy is a mandatory-consistency agreed term that cannot change mid-exchange.
