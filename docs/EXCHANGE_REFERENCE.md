...
Under `certificate` signing mode this otherwise-free-text field is effectively pinned: it must match the identity bound into the party's signing certificate. See [Signing](#signing).
...
Four fields govern payload. `payload.send` and `payload.receive` are the exchanged data dictionary -- what this party will disclose, and what it expects to receive -- and are cross-checked as a mirror: one party's `send` against the other's `receive`. `expected_payload_columns` and `disclosed_payload_columns` are per-party local lock-ins that are not exchanged (each is deliberately kept out of the mirror), holding the actual runtime disclosure to what was promised at setup:

- **`payload.send`** -- the columns this party discloses, exchanged with the partner as a data dictionary. It must agree exactly with what this party's [input metadata](#input-metadata) actually transmits (`is_payload: true` and `role` not `ignored`, the single source of truth for disclosure); a `send` that over- or under-declares that set is refused before the exchange runs, so the dictionary shown for consent and written into the [exchange record](spec/EXCHANGE_RECORD.md) matches the bytes that flow. An absent or empty `send` is exempt (the guided and default paths author none while metadata still transmits).
...
- **`disclosed_payload_columns`** (local, send-side) -- the disclosed set this party promised, the send-side mirror of `expected_payload_columns`. It is checked before connecting: if the current metadata can no longer produce exactly that set the run fails fast, naming the offending column(s), rather than under- or over-delivering and having the partner abort mid-exchange. Every invite path (re)records it, so re-inviting after editing your data keeps the promise in step and it cannot go stale.

The runtime lock-in mechanism, the wire field an invitation carries, and how the observe-then-persist paths reach the same lock-in are specified in [FILE_SYNC.md](spec/FILE_SYNC.md#disclosed-columns-subset-on-the-token).
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `connection_per_poll` | boolean | false | When `true`, a fresh SFTP session is opened at the start of each poll cycle and released before the loop goes idle again, instead of holding one session for the whole exchange. Because each cycle's session then needs only survive that cycle's seconds, the poll loop ordinarily does not reach a server's maximum-session-duration or idle cap. **Two things still hold a session across an idle stretch.** The rendezvous that runs first -- this side waiting for the partner to join the exchange -- holds one across its wait, so a server cap can cut a long wait there. And a boundary reached while an operation of this side's is still in flight keeps its session rather than cutting that operation off the wire, so the session spans that gap and every later boundary the operation is still running at; a transfer the partner's server never finishes answering carries no deadline of its own, and a session held for one stays up for the rest of the exchange. A cap that cuts either is transparently re-dialed, so the exchange completes in every case. Holding a boundary is ordinary -- a reply written while the loop is cycling produces one -- so no occurrence draws a warning, and the run's total is stated once in the end-of-run summary. What a boundary is held for, and what ends a hold, are in [CHANNEL_SECURITY.md](spec/CHANNEL_SECURITY.md). Use it when the partner's SFTP server enforces a session-lifetime limit you cannot change and a single exchange spans many idle poll gaps (a slow peer that reconciles the directory only occasionally, so an exchange lasts hours). **Pair it with a long `poll_interval_ms`** (minutes-scale): the mode pays a full SSH handshake every cycle, negligible at a long interval but wasteful at a short one, so combining it with a sub-minute interval draws a CLI warning (the `--connection-per-poll` flag with no long `--polling-frequency` is flagged the same way). **Local, not bilateral**: how one party dials is invisible to the peer and to the shared directory, so it is not advertised in the hello and cannot cause a rendezvous mismatch -- one party may cycle its session while the other holds one, and each sets this independently. A drop within a cycle and one that cuts the rendezvous are both transparently re-dialed underneath the mode (see `max_reconnect_attempts`, whose cumulative cap this mode is exempt from). The mode's own per-cycle dialing is not reported as a reconnection: the reconnect count and its warning are reserved for unexpected session losses, so a run that suffers none reports zero at any poll interval. A rendezvous the partner's cap does cut is a real session loss and is reported, so a healthy run against such a server can report a non-zero reconnect count. This side cannot tell the two causes apart, so the re-dial warning names both: a drop inside a poll cycle, which points at the link or the partner's server, or a rendezvous wait the partner's cap cut, which needs nothing from you. A *different* warning -- that the partner's server did not close the connection, so this side closed it -- names no drop at all, and against some servers a healthy exchange draws it routinely: a server that accepts the disconnect and then leaves the connection half-open never completes the release, so the mode closes it from this side and dials a fresh session next cycle. It is paced (the first, then every tenth) and totalled once in the end-of-run summary. |
...
**Recovering a contaminated or reused directory (CLI-only).** When a prior run crashed, or a bilateral-mode mismatch left protocol files behind, the entry guard rejects the next run, naming the offending files. Re-run with `--sweep-exchange-files` -- from one party only, never both -- to delete every protocol file (this party's and the peer's) and start a fresh exchange; foreign files are left untouched, and the interrupted run is not resumed. The sweep refuses if the directory shows a retain-mode signal -- it will not silently destroy a durable audit transcript -- so clearing such a directory requires the additional `--force-retain-sweep`, which permanently discards the prior transcript. These two flags are **CLI-only and invocation-scoped**: they are not `connection.options` keys and cannot be set in `psilink.yaml` ("always sweep" is not a state to persist). `--force-retain-sweep` requires `--sweep-exchange-files` and is rejected on its own. What the sweep deletes and what it refuses is specified in [FILE_SYNC.md](spec/FILE_SYNC.md#the-five-enforcement-sites).

**The sweep is a single-operator assertion.** Passing `--sweep-exchange-files` asserts that the directory is yours to clear and that no other session is live in it. The protocol cannot check that for you: the directory carries no entry-time liveness signal, so a live partner's files are indistinguishable from a crashed run's leftovers, and sweeping them breaks the exchange that is still using them (the residual this leaves is stated in [FILE_SYNC.md](spec/FILE_SYNC.md#invariants)). Both parties reaching for the flag is the natural response -- the entry guard names it identically to whoever hits it -- but a shared directory swept from both sides is not a supported recovery. A sweep that lands after the other party has written its rendezvous files deletes them, and neither run then completes: each either waits out its peer timeout for a partner whose rendezvous files are gone, or reports the files the other sweep already deleted.

- **Sequence it out of band.** The sweeping party enters the exchange and waits there; the other party starts afterwards, with no sweep flag. Nothing the command prints marks the moment that second start is safe -- the `sweeping ...` line goes out before the deletions do, and a directory with nothing to delete draws no line at all -- so the two operators agree the order between themselves, checking the directory's own contents (the named leftovers gone, a single hello present) rather than a log line.
- **If both parties swept and both runs failed**, re-run the exchange from both sides with no sweep flag: the failed runs leave the protocol files gone, which is the clean directory the ordinary path needs. If a sweep instead reported that it could not delete every file, resolve that error first -- that directory may be only partly cleared.
