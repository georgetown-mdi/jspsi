...
>
> **Authored per party, not carried on an invitation:** each value comes from that party's own configuration file, and both parties then run `psilink exchange`. An invitation carries no deduplicating term at all -- `psilink invite` and the web inviter refuse to mint one -- because accepting adopts the inviting party's `deduplicate` rather than mirroring it, which would reach the run as the refused `true`/`true` pair. An invitation that carries the term regardless is shown to the accepting party with that refusal stated, and is refused before matching begins.
