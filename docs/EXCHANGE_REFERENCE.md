...
- The two directories must differ. This is checked byte-for-byte at config validation and again, after path normalization, when the connection opens; for SFTP the normalization is textual (repeated and trailing slashes, a leading `./`), so two paths that resolve to the same server directory only through the login home -- a relative path and the absolute path it expands to -- cannot be detected client-side and are the operator's responsibility to keep distinct.
