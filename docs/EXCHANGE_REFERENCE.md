...
**Regex patterns are screened for catastrophic backtracking.** The four regex functions (`replace_regex`, `extract_regex`, `filter_regex`, and `split_on`) take a `pattern` (a `delimiter` for `split_on`) that is compiled and run per row over the full dataset. Because a partner authors these patterns and they execute on your data, terms validation screens every one for catastrophic-backtracking (ReDoS) risk and rejects a dangerous pattern, aborting the exchange before any row is processed -- a runaway pattern would otherwise hang the application on its single thread. The screen is best-effort and errs toward caution: a legitimate but unusually complex pattern can be rejected. If one of yours is, avoid nested quantifiers (`(a+)+`) and ambiguous alternations (`(a|a)*`), keep the pattern anchored and simple, or split the work across steps. `parse_date` is screened too: its `input_format` is expanded into a regular expression internally, so a format packed with many adjacent variable-width tokens (e.g. `MMDDMMDD...`) is rejected as the same risk, while ordinary layouts (`MM/DD/YYYY`, `YYYY-MM-DD`, `YYYYMMDD`) are unaffected.
