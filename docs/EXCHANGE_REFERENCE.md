...
> **One combination not yet enabled:** `true` on BOTH parties resolves to many-to-many, whose pairing and [entity closure](spec/PROTOCOL.md#the-many-to-many-entity-closure) are specified but which no exchange runs. It is refused before matching begins, on both parties, rather than silently matched one-to-one: no single party's terms can carry the combination -- it takes both parties' `true` to resolve many-to-many -- so the refusal comes after the terms exchange and before any PSI round, on both parties alike. The terms exchange has run by then, so what it carries (each party's declared record count among it) has already been disclosed; no linkage-key or payload data has moved. A many-to-one match runs under either [`linkage_strategy`](#linkage_termslinkage_strategy), with `deduplicate: true` on one party and `false` on the other.
>
