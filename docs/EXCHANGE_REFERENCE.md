...
| `server_connect_timeout_ms` | integer | 30000 | Milliseconds to wait during each connection attempt to the primary exchange server. Must be a positive integer (zero is not a meaningful "no timeout"). On `sftp` an attempt has two phases and this value bounds each of them: the handshake and authentication, and then the request that opens the SFTP subsystem, which begins the moment authentication succeeds. So a stalled attempt ends within roughly twice this value rather than at it. An attempt whose subsystem request goes unanswered for the whole window ends the connection terminally, without a further attempt, reporting the phase that timed out -- distinct from a rejected credential and from a handshake that never completed. |
