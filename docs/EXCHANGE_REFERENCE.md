...
- `single-pass` -- batches every key into one exchange and has the receiver reconstruct the cascade locally. The number of round trips stays the same no matter how many keys there are, which makes a multi-key linkage practical over a high-latency channel (such as `filedrop` or `sftp`), where a chain of dependent rounds is prohibitively slow. It has a tighter scale ceiling than `cascade`. An exchange above that ceiling aborts cleanly on both parties with a clear error that explains how to fit it (reduce the number of linkage keys or records, or split the dataset into smaller batches). The ceiling and how it is derived are specified in [PROTOCOL.md](spec/PROTOCOL.md#the-single-pass-dataset-ceiling-receiver-memory-and-masking-compute).

`single-pass` is not a free optimization: to reconstruct the cascade in one pass, the sender sends the receiver the full record-by-record picture of which records share a value under each key, so the receiver sees matches on less precise keys that the step-by-step cascade would have discarded before they were ever shared. The emitted result is the same either way; the difference is what the receiver learns along the way. In the reverse direction, `single-pass` keeps a helper blind where it can: when a party set to receive no result also discloses no payload columns, it needs nothing back from the match, so it never receives -- and so never learns -- which of its own records matched. A helper that does disclose payload still learns its matched records, because it needs them to build what it sends. Choose `single-pass` when the round-trip saving matters and that additional disclosure to the receiver is acceptable. Both parties must agree on the value or the exchange aborts. The wire shape and disclosure are specified in [PROTOCOL.md](spec/PROTOCOL.md#linkage-strategies-cascade-and-single-pass).
...
A party that declares it receives no output (`output.expects_output: false`) may not list `payload.receive` columns: it cannot receive payload for matched records it never gets, so the combination is rejected at parse and compatibility time. The same rule is enforced at runtime: a non-receiving party is sent no payload (the exchange transmits payload only to a partner entitled to the result), and its receive side fails closed if it is sent any regardless -- so a non-receiving helper never receives the partner's disclosed columns. A non-receiving party may still `send` payload (to a partner that does receive output).

**Cross-party reconciliation (lazy `receive`).** The two parties' payload declarations are checked as a mirror -- one party's `send` against the other's `receive` -- but each direction is checked only when the *receiving* party declares a `receive`. A party that omits `payload.receive` accepts whatever the partner sends ("take whatever I am given"), so that direction is not cross-checked; a party that declares `payload.receive` requires the partner's `send` to match it exactly, or the exchange aborts before any data moves. A *declared* `receive` is strict whether or not it lists columns: an explicit empty `receive: []` asserts the partner sends nothing -- any disclosed column aborts the exchange -- distinct from omitting `receive` entirely, which is lazy. This is the same empty-is-strict, absent-is-lazy convention the runtime lock-in below enforces, so the cross-party declaration check and the runtime check agree; only a hand-authored config or crafted token produces an explicit `[]`, since the guided paths leave `receive` unauthored. This is what lets an invitation be authored without the inviter knowing the partner's columns: the inviter declares only its `send` and leaves `receive` open, the accepting party adopts the inviter's `send` as its own `receive` (so it validates exactly what it will get) and discloses its own `send` from its [input metadata](#input-metadata), and the inviter takes whatever that turns out to be. A zero-setup or direct `exchange` that authors no payload is lazy on both sides; a recurring exchange whose loaded configs declare `receive` on both sides is checked strictly in both directions. Laziness relaxes only this declaration check -- never what is disclosed: each party's own metadata still governs what it sends, so a lazy receiver only ever receives what the partner's own consented metadata transmits, and receiving is not a disclosure.

Beyond this terms-level mirror, a party that locked in an expected received-column set has the *actually-received* columns checked against it at runtime, after the payload is exchanged and before any result or [record](spec/EXCHANGE_RECORD.md) is written: a divergence aborts the exchange (the partner promised one disclosure and delivered another). An invitation carries the inviter's disclosed-columns subset directly, so the accepting party's consent screen shows the columns it will receive -- and locks them in for that runtime check -- even when the inviter authored no `payload.send`. Both an online and an offline accept persist it to the config's local `expected_payload_columns` field (distinct from `payload.receive`, to avoid the mirror) so a later `exchange` enforces it, and an online accept additionally enforces it in memory for its own single run; a recurring party locks in that same field, falling back to its persisted `payload.receive`. A party that learns its received set only by observing a first exchange -- the online inviter, or a zero-setup party run with `--save` -- reaches the same lock-in from the other direction: after that first exchange it records the columns it observed into the saved config's `expected_payload_columns`, so its later `exchange` fails closed on a divergence too. That observe-then-persist writer records only a non-empty observation: an empty received payload is an ambiguous zero-match run, so it is left absent (lazy) rather than frozen to a strict "receive nothing" that would false-abort a later matching run. An empty locked-in set is enforced strictly ("receive nothing"); only an absent one is lazy. A party with nothing locked in -- a first-contact or no-save exchange -- reconciles lazily and takes what it is given. Separately, a party entitled to no output is sent no payload at all and fails closed if it is (see the no-output rule above). See [FILE_SYNC.md](spec/FILE_SYNC.md#disclosed-columns-subset-on-the-token) for the wire field and the lock-in mechanism.

The committing side has the symmetric safeguard, checked before it connects: a party that persisted the disclosed set it promised -- the config's local `disclosed_payload_columns` field (its own column names, the send-side mirror of `expected_payload_columns`) -- fails fast if its current metadata can no longer produce exactly that set, naming the offending column(s), rather than silently sending a different set and letting the partner abort mid-exchange (which would surface as the *partner's* failure, after data had already moved). This is the send-side, self-attributed counterpart of the receive-side lock-in above. Every invite path records the promised set here as it mints the invitation -- a fresh invite writes it with the config, and an invite that reuses an existing config refreshes it in place (preserving your comments) -- so re-inviting after you edit your data keeps the recorded promise in step with what your partner is told, and it can never go stale. (The accepting party instead carries its own send commitment as `payload.send`, covered by the data-dictionary check below.) The error offers two remedies -- restore the column so you keep your promise, or re-establish the exchange (re-invite) with the narrower disclosure -- so choosing to disclose less is a first-class option, never something the check pushes you to undo. An absent field means no commitment is on record (lazy, unchecked); an empty one strictly promises to disclose nothing.

`payload.send` is a data dictionary: it states what flows, but whether a column's values are actually transmitted is governed by that column's [input metadata](#input-metadata) (`is_payload` and `role`), which is the single source of truth for disclosure. The two must agree exactly. A present (non-empty) `payload.send` must name exactly the columns its metadata transmits -- `is_payload: true` with `role` not `ignored` -- no more and no less. A `send` entry whose column is `is_payload: false`, `role: ignored`, or absent from the metadata *over-declares*: the dictionary exchanged with the partner, shown for consent, and written into the [exchange record](spec/EXCHANGE_RECORD.md) would claim a column that never flows. A column the metadata transmits but `send` omits *under-declares*: the dictionary understates what flows, and -- because a recurring partner mirrors this dictionary into its received-column lock-in -- that partner would lock in too few columns and abort an otherwise-honest exchange when the omitted column arrives. Either way the dictionary must match the disclosed set, and a mismatch is rejected up front (exit 64). An absent or empty `payload.send` is exempt: the guided and default paths author none while metadata still transmits. When the column metadata is in hand as the invitation is generated -- a config with an explicit `metadata` block, or the web editor -- the rejection happens at mint, so the token and the partner's consent screen never carry a mismatched dictionary. When the metadata is instead left to be inferred from the exchange's input (so it is not known at mint), the rejection happens during data preparation, before any connection is made -- as it also does for the paths that mint no invitation (a zero-setup or direct `exchange` run, and the accepting party). The mismatch never transmits a column the operator's metadata did not authorize -- metadata governs transmission either way -- so the check guards consent, record, and lock-in accuracy, not confidentiality.
