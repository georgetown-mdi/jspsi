...
One feature requires `single-pass`: matching on the several values a `split_on` produces runs under it only, so terms declaring a fan-out under `cascade` are refused rather than matched on one value (see [Fan-out (multi-value fields)](#fan-out-multi-value-fields)). Once fuzzy expansion ships, a key declaring `swap` requires it for the same reason: matching the pair in either order doubles that key's declared width, so `cascade` refuses those terms rather than matching the one order the terms name.
...
>
> - **Accepting an invitation:** accepting derives the accepting party's own value as `false` rather than adopting the invitation's, so an accepted deduplicating invitation runs many-to-one with the inviting party as the "many" side. The consent display states what that discloses: that the accepting party's own records are not grouped, and that more of the accepting party's records can still match than a one-to-one run of the same two files would.
...
> - **An accepting party that wants its own records grouped** declares it in its own configuration file: each party's value comes from its own file, and both run `psilink exchange`. Where both files declare it, the exchange is the both-sided one above.
...
A column name that holds invisible control characters -- a tab, a NUL, an escape, or one of the text-direction characters among them -- loses them at every read (see [CSV header sanitation](spec/CHANNEL_SECURITY.md#csv-header-sanitation-at-ingestion)), and the seat that read the file states which column positions changed. Two consequences reach this block. A name made only of those characters comes back empty and meets the refusal above, which states the removal as the cause instead of the header-row causes. And a name written as the header was typed -- a `metadata` `name`, a `linkage_fields` `name`, or a `standardization` output -- matches no column of the file: rename it to the column without those characters, which is the name `psilink init` writes into a configuration it authors from that file. The linkage refusal states that cause where a linkage key depends on the declared name.

Where the declared name is a `linkage_fields` name, an element `field` or `swap` reference, a `payload` column name, the legal agreement's `reference`, or a rule set's `name`, the name rule above refuses the document outright: the configuration fails to load, naming the field by its path (`linkage_fields.0.name`) and printing none of the value, and no exchange runs. A `metadata` `name` is not one of those -- the block is this party's own, held to its length cap alone -- so a `metadata` name for a column that is only sent as payload satisfies every key, meets no refusal, and is listed to your partner in the invitation as declared. Rename that one yourself.
...
A step the function cannot be built from at all is refused where the terms are minted, for a related reason: a pipeline is built once, before the first row is read, so a `pad_left` with no `length` or a fill longer than one character, a `phonetic` naming an algorithm this build does not implement, or a `function` it does not recognize stops the exchange before it matches anything -- on both parties, and only once the invitation has been accepted and set up against. Both front ends check at the mint, before the token is disclosed: a config-source `psilink invite`, as it is checked for a standardization that contradicts the terms, and the browser editor when Generate creates the invitation. The command-line refusal names the offending step's function, where the name is one this build recognizes, and the two remedies: correct that step's parameters, or remove the step. In the browser editor an authored cleaning step is refused earlier still, at the step editor, which holds Generate shut until the step is corrected or removed: a malformed parameter is marked on the input that has to change, and a step naming a function the build does not recognize is marked on the step itself, since its parameters cannot be edited there and removing it is the only remedy. Separately, a document declaring more transform steps than the mint checks -- counted across every cleaning step and every linkage-key element transform -- is refused with the count and the limit stated; the limit is far above what any authored document declares.
