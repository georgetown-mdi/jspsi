...
- **Neither list anywhere in the file.** The short form applies only when neither `linkage_fields` nor `linkage_keys` is written anywhere in the file, so a list un-indented out of `linkage_terms` to the top level is refused rather than filled over, naming where it was found and where it belongs. A list written under a mis-spelled key is not seen: psilink cannot tell it from any other key it does not read, and the block it was meant for holds no rules at all.
