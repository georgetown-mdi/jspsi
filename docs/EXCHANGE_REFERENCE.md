...
**Recovering a contaminated or reused directory (CLI-only).** When a prior run crashed, or a bilateral-mode mismatch left protocol files behind, the entry guard refuses the next run (exit 64). The refusal names the directory, how many offending files it found, and as many of their names as it can show; the directory itself is the full list. Remove those files after confirming no other session is using the path, or re-run with `--sweep-exchange-files`.
