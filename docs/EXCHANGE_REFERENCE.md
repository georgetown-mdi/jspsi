...
> **Two combinations not yet implemented:** `true` on BOTH parties resolves to many-to-many, whose pairing is specified but whose transitive closure into entity clusters is not, and `deduplicate: true` under [`linkage_strategy: single-pass`](#linkage_termslinkage_strategy) has no matching path -- single-pass matches one-to-one only. Either is refused before matching begins, on both parties, rather than silently matched one-to-one. Where your own terms carry the combination the refusal comes before the exchange begins, at the local prepare step, so nothing is sent; where an invitation carries it, the refusal comes as you accept, before the terms are displayed and before any connection; and where it takes both parties' terms to see it -- the both-sided pair, or a partner's own single-pass deduplication authored in its own config -- it comes after the terms exchange and before any PSI round, on both parties alike. A many-to-one match runs under `cascade`, with `deduplicate: true` on one party and `false` on the other.
>
> **Each party's own value, however the exchange is set up:** an invitation carries the inviting party's `deduplicate` and nothing more. Accepting derives the accepting party's own value as `false` rather than adopting the invitation's, so an accepted deduplicating invitation runs many-to-one with the inviting party as the "many" side, and the consent display states what that discloses, that the accepting party's own records are not grouped, and that more of the accepting party's records can still match than a one-to-one run of the same two files would. An accepting party that wants its own records grouped sets the pair up the other way instead: each party's value comes from its own configuration file, and both run `psilink exchange`. An invitation pairing the term with `single-pass` is refused as you accept it -- the accept path reads the inviting party's own terms for that pair, since the derived value of `false` would otherwise hide it until the terms exchange -- so such an invitation reaches neither a consent display nor a connection.
...
A `parse_date` step whose `input_format` cannot supply a complete date -- for example `MM/DD`, which carries no year -- drops every record rather than reformatting it, so a linkage key whose element transform relies on it can never match. This holds for any data, so it is reported before the exchange runs rather than surfacing only as an empty result afterward: the CLI pre-flight warns while another key remains live and refuses the run when every key the input could satisfy is dead this way, and the web acceptor's confirm-columns step flags the key.
