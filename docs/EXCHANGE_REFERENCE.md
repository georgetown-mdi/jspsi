...
> **Two combinations not yet implemented:** `true` on BOTH parties resolves to many-to-many, whose pairing is specified but whose transitive closure into entity clusters is not, and `deduplicate: true` under [`linkage_strategy: single-pass`](#linkage_termslinkage_strategy) has no matching path -- single-pass matches one-to-one only. Either is refused before the exchange begins, on both parties, rather than silently matched one-to-one. A many-to-one match runs under `cascade`, with `deduplicate: true` on one party and `false` on the other.
>
> **Authored per party, not carried on an invitation:** each value comes from that party's own configuration file, and both parties then run `psilink exchange`. An invitation carries no deduplicating term at all -- `psilink invite` and the web inviter refuse to mint one -- because accepting one would set the inviting party's `deduplicate` on the accepting party's own records too, and an exchange in which both parties deduplicate is refused. An invitation that carries the term regardless is shown to the accepting party with that refusal stated, and is refused before matching begins.
