...
>
> **A candidate set runs under the cascade too:** terms that match a record on several values -- a [`split_on`](#fan-out-multi-value-fields) transform, a [`generate_fuzzy_comparisons`](#element-fields) expansion, or a key declaring [`swap`](#swapped-keys) -- change nothing about which strategy pairs the both-sided match. Under `cascade` the pair runs over those candidates, and the clusters it produces may join records that matched on different values of one key ([the entity closure](spec/PROTOCOL.md#the-many-to-many-entity-closure)); under `single-pass` the both-sided pair is refused whatever the keys declare, as above. [The built-in rules](#the-built-in-rules) declare a swapped key, so a both-sided match over the shipped default keys is one the cascade runs.
>
...
- **Set it yourself, before the first run.** Ask the partner for the value their `psilink fingerprint` prints, over a channel you trust, and write it here. The first exchange holds the certificate the partner presents to this value and refuses a partnership that does not match it. This is the stronger posture: the anchor is the channel you chose.
...
Only `psilink exchange` writes this field, only into the configuration file named on its own command line, and only when the file holds none: a run whose partner presents a different certificate is refused and leaves the file exactly as it stands. Changing a pin is a deliberate edit, as changing a host-key pin is.

No cross-field rule ties this value to `signing.mode` in the block's schema, so a partially-authored config -- no partner fingerprint yet -- still parses wherever the schema is used; `psilink fingerprint` needs only `identity_file`, which it reads from the raw config text rather than through the schema.
...
A `split_on` step fails closed under a combination that matches one value per record, whether it sits in a `standardization` or in a linkage key's element transform, and that combination is refused at every point the terms it reads are known. A count-only exchange is decided by one party's own terms, so it is refused at the same points as above -- when the terms are authored or minted into an invitation, and at data preparation -- and again once both parties' terms are agreed, rather than run with the narrower matching it would deliver (see [Fan-out (multi-value fields)](#fan-out-multi-value-fields)). The web app authors no `split_on` at all: its cleaning-step menu does not offer the function, and a terms document that declares one -- in a cleaning step or a linkage key's element transform -- blocks generation. A fan-out is therefore authored in a configuration file, as the example above does, and run with `psilink exchange` or minted with `psilink invite`.
