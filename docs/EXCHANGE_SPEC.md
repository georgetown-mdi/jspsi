...
A case-folding step (`to_upper_case`, `to_lower_case`) can emit a non-NFC sequence for a few code points even from NFC input -- for example the uppercase of the Greek `U+0390` is the decomposed `U+0399 U+0308 U+0301`. The steps that compare or match an intermediate value against your authored input (`null_if`, `filter_regex`, `extract_regex`) therefore normalize the value they inspect to NFC before matching, so an exclusion, filter, or extraction authored in NFC behaves correctly even when it follows a case-fold. `null_if` and `filter_regex` still pass the original value downstream unchanged; `extract_regex` returns its capture from the normalized value.
