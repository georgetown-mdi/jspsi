...
Credential and opaque string fields in `psilink.yaml` support `@`-file
references: a value beginning with `@` is read from the file at the given path
rather than used literally. For example:
...
Fields that support this convention are marked "`@`-file recommended" in their
descriptions. It does not apply to free-text or structured fields such as
`linkage_terms.identity`, where `@` may appear as a literal character.
...
this is asymmetric: the provisioning party (conventionally the inviter) calls
