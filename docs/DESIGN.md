...
For the rest of this section we describe use cases as in the command line application. The application provides five explicit subcommands - `init`, `invite`, `accept`, `exchange`, and `fingerprint` (the last shows this party's signing-certificate fingerprint) - alongside a zero-setup mode in which both parties run the same command against a shared server without specifying a subcommand. Web application versions implement the same functionality with an appropriate graphical user interface and use browser storage instead of the file system.

> **Implementation status:** The `exchange`, `invite`, `accept`, and `fingerprint` subcommands, the zero-setup mode, and the `--save` flag are all functional. Only the `init` subcommand is stubbed (it prints a not-yet-implemented message), targeted for the 1.0 release; see [CLI.md](CLI.md) and [ROADMAP.md](ROADMAP.md). The command table and flows below describe the intended behavior.
