...
The web application is the browser-based, install-free entry point for conducting an exchange. It opens on a lobby with two primary actions -- setting up an exchange and accepting an invitation -- and offers secondary links beneath them for starting with built-in synthetic sample data and verifying a receipt. Files are read and processed in the browser; nothing leaves the machine except the protocol traffic of an exchange the operator has consented to.
