...
The web application is the browser-based, install-free entry point for conducting an exchange. It opens on the operator's list of recurring (managed) exchanges saved in this browser -- the home surface -- from which one is run again without a new invitation; the first-run experience is a designed empty state that explains what a recurring exchange is and offers setting one up, accepting a recurring invitation, or restoring one from a backup file. Setting up or accepting a one-off exchange is the quick path reached from that home, and remains available when the browser cannot store recurring exchanges at all. Verifying a receipt and starting with built-in synthetic sample data are secondary entries on the quick path. Files are read and processed in the browser; nothing leaves the machine except the protocol traffic of an exchange the operator has consented to.
