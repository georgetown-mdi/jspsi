...
> **Implementation status:** The `init`, `exchange`, `invite`, `accept`, and `fingerprint` subcommands, the zero-setup mode, and the `--save` flag are all functional; see [CLI.md](CLI.md). The command table and flows below describe the intended behavior.
