...
| SC-8 | Transmission Confidentiality and Integrity | Recurring (authenticated) exchanges on the SFTP and filedrop channels are protected in transit by AES-GCM AEAD keyed from the P-256 key-exchange session key; zero-setup exchanges on those channels are not AEAD-wrapped at the application layer and rely on the channel's transport encryption (for SFTP, the SSH session). WebRTC channels use DTLS end-to-end. |
...
The cryptographic primitives PSI-Link uses are algorithms specified by NIST publications (P-256 ECDH, Ed25519, SHA-256, HMAC-SHA-256, HKDF, AES-GCM), but **the cryptographic modules in use are not FIPS 140-validated**:

- `@noble/curves`, which performs the receipt-signing Ed25519, is not FIPS-validated.
...
The statements above are about the published container image and the browser application. A second container image exists but is not published: it runs on Amazon Linux 2023 and embeds the CMVP-validated OpenSSL FIPS provider AWS publishes for that distribution, which the key-agreement, AEAD and key-schedule primitives -- P-256 ECDH, AES-256-GCM, SHA-256, HMAC-SHA-256 and HKDF -- are measured to dispatch into. That measurement is a probe the image runs at every container start, and that CI runs against the built image on every change that can affect it: the probe makes each of those five calls at PSI-Link's own parameter shapes and requires, in the same process, that the validated module be loaded and that two operations no FIPS provider serves both fail, so a call served by an uncertified provider cannot read as a dispatch into this one. It does not change any of the statements above -- the PSI masking's BoringSSL is inside a WebAssembly module that no OpenSSL provider can reach in principle; receipt signing runs in `@noble/curves` outside any module until its decided migration lands; and key establishment's measured ECDH is a claim about that primitive alone, the key schedule above it staying composed in application JavaScript whatever serves the calls beneath -- and it does not make PSI-Link validated: a product embedding another vendor's validated module may say it uses one and may not call itself validated. What that image supports a claim of, at each of the three host tiers, and what stops working inside it, are recorded in [fips-variant-image.md](notes/fips-variant-image.md).
