...
| SC-13 | Cryptographic Protection | All cryptographic operations use algorithms specified by NIST publications: P-256 ECDH for key establishment (SP 800-56A Rev. 3, curve per FIPS 186-5 / SP 800-186), ECDSA over P-256 with SHA-256 (FIPS 186-5) for receipt signing identities, SHA-256, HMAC-SHA-256, HKDF (SP 800-56C), and AES-GCM. Every one of them is a `crypto.subtle` call, so where a validated module is configured beneath the platform that module performs it. That is still a distinct thing from module-certificate approval, and what a scoped claim may and may not say is recorded per area in [receipt-signing-fips-boundary.md](notes/receipt-signing-fips-boundary.md) and [key-establishment-fips-boundary.md](notes/key-establishment-fips-boundary.md). The PSI masking is the remaining exception: it runs in BoringSSL inside a WebAssembly module no OpenSSL provider can reach ([fips-provider-surface.md](notes/fips-provider-surface.md)). |
...
The cryptographic primitives PSI-Link uses are algorithms specified by NIST publications (P-256 ECDH, ECDSA over P-256 with SHA-256, SHA-256, HMAC-SHA-256, HKDF, AES-GCM), but **the cryptographic modules in use are not FIPS 140-validated**:
...
Receipt signing runs ECDSA over P-256 with SHA-256, an algorithm on the approved list of the FIPS 140-3 certificate this project targets, and every signing and verification operation is a `crypto.subtle` call -- so where a validated module is configured beneath the platform it performs them. That does not make the receipt validated: the canonical encoding, the domain separation, the per-signer binding, and the pin-before-signature check are an application composition above the algorithm that no certificate attests, and the browser build carries the same signing path with no module beneath it at all. The claims this does and does not license, and the alternative weighed against the migration, are recorded in [receipt-signing-fips-boundary.md](notes/receipt-signing-fips-boundary.md).

The statements above are about the published container image and the browser application. A second container image exists but is not published: it runs on Amazon Linux 2023 and embeds the CMVP-validated OpenSSL FIPS provider AWS publishes for that distribution, which the key-agreement, AEAD and key-schedule primitives -- P-256 ECDH, AES-256-GCM, SHA-256, HMAC-SHA-256 and HKDF -- are measured to dispatch into. That measurement is a probe the image runs at every container start, and that CI runs against the built image on every change that can affect it: the probe makes each of those five calls at PSI-Link's own parameter shapes and requires, in the same process, that the validated module be loaded and that two operations no FIPS provider serves both fail, so a call served by an uncertified provider cannot read as a dispatch into this one. It does not change any of the statements above -- the PSI masking's BoringSSL is inside a WebAssembly module that no OpenSSL provider can reach in principle; receipt signing's P-256 ECDSA runs through `crypto.subtle` but sits outside the probe's five measured legs, so that image supports no measured dispatch claim about it; and key establishment's measured ECDH is a claim about that primitive alone, the key schedule above it staying composed in application JavaScript whatever serves the calls beneath -- and it does not make PSI-Link validated: a product embedding another vendor's validated module may say it uses one and may not call itself validated. What that image supports a claim of, at each of the three host tiers, and what stops working inside it, are recorded in [fips-variant-image.md](notes/fips-variant-image.md).
