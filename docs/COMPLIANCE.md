...
| SC-12 | Cryptographic Key Establishment and Management | Session keys are established via an X25519 authenticated key exchange -- an ephemeral X25519 Diffie-Hellman (RFC 7748) keyed with the pre-shared secret under the Noise NNpsk0 pattern, with an explicit key-confirmation round whose construction is modelled on the bilateral key confirmation of NIST SP 800-56A Rev. 3 section 5.9.2, though not conformant to it: that section specifies the feature for key-agreement schemes whose parties each hold a static key-establishment key pair, and this handshake holds none. The key schedule that turns that exchange into a session key is composed in application JavaScript above primitive calls and sits outside any validated cryptographic module boundary, and the Diffie-Hellman itself runs in a pure-JavaScript library, so key establishment claims no module validation (see [FIPS 140](#fips-140)). Key-file permissions, rotation, backup, and compromise-response procedures are documented in [SECURITY_DESIGN.md#key-file-security](SECURITY_DESIGN.md#key-file-security). |
| SC-13 | Cryptographic Protection | All cryptographic operations use algorithms specified by NIST publications: X25519 (FIPS 186-5, SP 800-186), Ed25519 (FIPS 186-5) for receipt signing identities, SHA-256, HMAC-SHA-256, HKDF (SP 800-56C), and AES-GCM. That is approval at the algorithm-standard level, which is a distinct thing from module-certificate approval: the Ed25519 used for receipt signing is on no OpenSSL Project certificate's approved-algorithm list, and under the FIPS 140-3 certificate this project targets it is Non-Approved and Not Allowed. Two of the forty active certificates do approve EdDSA, and neither yields a certified module a redistributable image can obtain ([fips-provider-surface.md](notes/fips-provider-surface.md)). Migrating receipt signing to ECDSA over P-256 through `crypto.subtle` is decided and recorded, with what a scoped FIPS claim may and may not say about it meanwhile, in [receipt-signing-fips-boundary.md](notes/receipt-signing-fips-boundary.md). |
...
The cryptographic primitives PSI-Link uses are algorithms specified by NIST publications (X25519, Ed25519, SHA-256, HMAC-SHA-256, HKDF, AES-GCM), but **the cryptographic modules in use are not FIPS 140-validated**:
...
Key establishment is a distinct case from the primitives above: its key schedule is composed in application JavaScript above primitive calls, so however the modules beneath it are certified it yields validated primitives and never a validated protocol -- what a scoped claim may and may not say about it is recorded in [key-establishment-fips-boundary.md](notes/key-establishment-fips-boundary.md).

Receipt signing is a distinct case in the other direction. Its Ed25519 signatures run outside any module, and no provider configuration puts them inside one: Ed25519 is on no OpenSSL Project certificate's approved-algorithm list, and under the FIPS 140-3 certificate this project targets it is Non-Approved and Not Allowed, so routing it through a validated module would take that module out of approved mode for the operation. Two of the forty active certificates approve EdDSA, but neither is reachable for a redistributable image, so neither changes this ([fips-provider-surface.md](notes/fips-provider-surface.md)). Migrating it to ECDSA over P-256 is decided and recorded, with what a scoped claim may and may not say about receipt signing meanwhile, in [receipt-signing-fips-boundary.md](notes/receipt-signing-fips-boundary.md).

The statements above are about the published container image and the browser application. A second container image exists but is not published: it runs on Amazon Linux 2023 and embeds the CMVP-validated OpenSSL FIPS provider AWS publishes for that distribution, which the AEAD and the key-schedule primitives -- AES-256-GCM, SHA-256, HMAC-SHA-256 and HKDF -- are measured to dispatch into. That measurement is a probe the image runs at every container start, and that CI runs against the built image on every change that can affect it: the probe makes each of those four calls at PSI-Link's own parameter shapes and requires, in the same process, that the validated module be loaded and that two operations no FIPS provider serves both fail, so a call served by an uncertified provider cannot read as a dispatch into this one. It does not change any of the statements above -- the PSI masking's BoringSSL is inside a WebAssembly module that no OpenSSL provider can reach in principle, and key establishment and receipt signing run in `@noble/curves` outside any module until their decided migrations land -- and it does not make PSI-Link validated: a product embedding another vendor's validated module may say it uses one and may not call itself validated. What that image supports a claim of, at each of the three host tiers, and what stops working inside it, are recorded in [fips-variant-image.md](notes/fips-variant-image.md).
