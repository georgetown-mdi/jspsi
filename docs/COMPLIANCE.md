...
Receipt signing is a distinct case in the other direction. Its Ed25519 signatures run outside any module, and no provider configuration puts them inside one: Ed25519 is on no module certificate's approved-algorithm list, and under the FIPS 140-3 certificate this project targets it is Non-Approved and Not Allowed, so routing it through a validated module would take that module out of approved mode for the operation. Migrating it to ECDSA over P-256 is decided and recorded, with what a scoped claim may and may not say about receipt signing meanwhile, in [receipt-signing-fips-boundary.md](notes/receipt-signing-fips-boundary.md).
