...
| SC-12 | Cryptographic Key Establishment and Management | Session keys are established via an X25519 authenticated key exchange -- an ephemeral X25519 Diffie-Hellman (RFC 7748) keyed with the pre-shared secret under the Noise NNpsk0 pattern, with an explicit key-confirmation round modelled on NIST SP 800-56A Rev. 3 (sections 5.9 / 6.2.1.5). The key schedule that turns that exchange into a session key is composed in application JavaScript above primitive calls and sits outside any validated cryptographic module boundary, and the Diffie-Hellman itself runs in a pure-JavaScript library, so key establishment claims no module validation (see [FIPS 140](#fips-140)). Key-file permissions, rotation, backup, and compromise-response procedures are documented in [SECURITY_DESIGN.md#key-file-security](SECURITY_DESIGN.md#key-file-security). |
