...
This document describes the PSI and PSI-C algorithms, how they are composed to produce a privacy-preserving record linkage, and the wire-level X25519 authenticated key exchange. It does not cover the exchange agreement format that parameterizes the protocol (see [EXCHANGE_SPEC.md](EXCHANGE_SPEC.md)), the threat model and authentication design (see [SECURITY_DESIGN.md](SECURITY_DESIGN.md)), or the network layer over which the protocol runs (see [COMMUNICATION.md](COMMUNICATION.md)). Intended readers are security auditors, external implementors, and developers.
