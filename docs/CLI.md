...
Every matching rule is shown here rather than behind a second command: under `psi`, what is matched decides which identifiers are disclosed, so a rule you are consenting to is never hidden. A term the inviting party declares that this version of the exchange does not yet apply -- an approximate-match setting -- is marked as proposed, so the display never states behavior the run does not perform. Duplicate matching is a term the run does honor, and it discloses more than a one-to-one match does, so the `duplicate matches` line carries a sentence stating what: your side learns, for each of your own matched records, how many of your partner's records group onto it and which rows they are. Accepting adopts that declaration onto your own records as well, and an exchange in which both parties deduplicate is refused before matching begins, so a deduplicating exchange is set up in each party's own config file rather than reached by accepting an invitation that carries it. A value transform this version does not recognize is marked as unrecognized rather than printed in the same shape as a rule psilink can explain.
