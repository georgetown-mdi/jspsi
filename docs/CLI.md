...
- **Applied, and it discloses more.** Duplicate matching is a term the run does honor, and it discloses more than a one-to-one match does, so the `duplicate matches` line carries a sentence stating what: your side learns, for each of your own matched records, how many of your partner's records group onto it and which rows they are.
- **Applied, but not from an invitation.** Beneath that sentence, at the same level, is the refusal that goes with it: accepting adopts the declaration onto your own records as well, and an exchange in which both parties deduplicate is refused before matching begins, so the invitation does not run as proposed. `psilink invite` refuses to mint one for that reason. A deduplicating exchange is set up in each party's own config file instead -- one party's `deduplicate: true` against the other's `false` -- with both parties running `psilink exchange`.
