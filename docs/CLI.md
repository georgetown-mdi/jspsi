...
`--identity IDENTITY` supplies this party's label in the linkage terms -- what your partner reads as your name there, in the invitation, and in the exchange record. It is required on every command that authors its own terms: `psilink accept`, the zero-setup form, and `psilink invite` when it infers the terms from an input file. A run whose terms come from a configuration file takes the label from that file's `linkage_terms.identity` instead: `psilink exchange`, where `--identity` replaces it for that one run, and `psilink invite` over a pre-existing configuration, where the flag is reported as having no effect.

psilink stands in no label of its own. A run that needs one and supplies none -- including one whose value is empty or blank, as a scripted `--identity "$ORG"` sends when `ORG` is unset -- stops with a usage error (exit 64) naming the flag, rather than putting a name you did not choose in front of your partner.

The value is trimmed before that check, which applies to the flag alone: a label read from a configuration file is passed to the schema as written -- non-empty is all it requires -- so leading or trailing whitespace there reaches the agreed terms verbatim.
...
Whichever source supplies the terms supplies the identity in them. Inferring the terms from an `INPUT_FILE` requires `--identity`; taking them from a pre-existing configuration file mints the invitation under that file's `linkage_terms.identity`, and an `--identity` given there is reported as having no effect (edit `linkage_terms.identity` in the configuration to change it).
...
- **A linkage key's own cleaning can never produce a value** -- a `parse_date` whose `input_format` omits a component, so it drops every record regardless of the data. The key is named even though its columns are present: it passes the column check yet would contribute nothing, so the fix is a corrected invitation from the partner, not a different CSV. A key left live beside it is warned about and acceptance proceeds; when every key the input could satisfy is dead this way, acceptance stops with an error just as the no-satisfiable-key case does, since the exchange could then only ever produce an empty result.
...
- **`DOES NOT MATCH`** -- they carry different binders: two artifacts from different runs, not one exchange. This fails the run (exit 1) and is reported on its own line, distinct from a signature, identity, or anchoring failure -- every one of those may still read as passing. A note points at the shared timestamp stamp the record and receipt of one exchange carry by default.
...
Every `psilink` command exits with one of the following codes. The failure classes 64, 69, 70, and 73 follow the BSD `sysexits` convention.
