...
The `excerpt` is bytes the peer chose, bounded by the same read budget and carried as a JSON string value. Two things are done to it before it is emitted, so that a caller can print the line it reads:

- **The line is printable ASCII throughout.** Every byte outside `0x20`-`0x7e` is written as a JSON escape -- DEL and the C1 range as `\uHHHH`, and the control bytes JSON already escapes, the ESC that drives a terminal control sequence among them, as the form JSON defines for each. So the line carries nothing that can drive a terminal or break itself in two.
- **Private-key material is redacted**, the same strip the human-readable failure applies, so a peer answering with a PEM block has it replaced on both routes rather than logged verbatim.

The escapes are the JSON encoding's, not a rewriting of the value: `JSON.parse` gives back the peer's own bytes. So a caller that renders the parsed `excerpt` to a human still escapes it at that point -- the guarantee is about the line, not about the field once it is parsed out of it.
