...
- **Why a fast poll is allowed.** A conservative interval stays within SFTP servers' anti-flood limits, and because the per-round encryption dominates an exchange's wall-clock time a multi-second poll adds negligible latency, so the flag exists mainly to let a demo opt into a fast poll against a controlled server.
...
- **The SSH handshake trace.** `--log-level trace -vv` -- both, since `-v`/`--verbose` is what raises the SFTP client's own logger to that level -- adds the SSH stack's diagnostics for an `sftp` connection to the diagnostic output, each line prefixed `ssh:`: the server's identification string, the algorithms each side offered, the ones negotiated, and the packet flow, so one run answers whether the handshake completed and on what. It is roughly what `ssh -vvv` reports, without installing a second client to get it, and it is emitted at no other level: it runs to dozens of lines per dial. The server's own text in those lines is escaped before it reaches your terminal or a `--log-file`, so a server cannot forge a log line with it.
