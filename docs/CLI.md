...
| 69 | `EX_UNAVAILABLE` | A transport or availability failure: the exchange server, peer, or shared storage was unreachable, rejected an operation, or went silent. Retrying once the transport recovers may succeed. From `psilink doctor`, it means something the checks themselves depend on was not available, so nothing was established either way. Also a completed exchange whose audit artifact could not be written, which is not retryable and must not be re-run (see below). |
...
One more case exits 69 with the exchange itself intact: a run that completed and wrote its result but could not produce an audit artifact it was asked for -- the self-attested record and its verification keys, or the dual-signed receipt. **Do not re-run it.** The data was exchanged, the result is on disk, and only the local artifact is missing; fix what blocked the write (a full disk, a read-only or unwritable path, a revoked mount) and the next exchange records normally. The non-zero exit exists for the unattended run, where the warning on stderr is discarded by a supervisor or suppressed by `--log-level error`. Under `--event-stream` the same failure is a `warning` event while the terminal event stays `result`, which is how a supervisor tells this case from a failed exchange.
