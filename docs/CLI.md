...
A `certificate`-mode configuration that pins no `signing.partner_fingerprint` is refused outright rather than warned about: the run exits 64 before any credential, terms, or data are sent. Unlike the divergence above, that run has no way to complete -- the two sides swap signatures only after their data has crossed, and the certificate the partner presents is rejected there when nothing is on file to check it against, ending the exchange with no result, no record, and no receipt on this side. Set `signing.partner_fingerprint` to the value your partner's [`psilink fingerprint`](#signing-identity-fingerprint) prints, or run `signing.mode: none` until you hold it.
