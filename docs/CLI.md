...
An [online acceptance](#online-acceptance) that takes its exchange directories from the invitation's endpoint says so above the outline as well, because your partner chose those paths: the run reads and writes the exchange files there, and the configuration it saves records them for every later `alcove exchange`. It names the inbound directory, where you read your partner's files, and the outbound directory, where you write yours, and states what confirming does with them and that `--outbound-path` uses your own directories instead. Under `--consent-to-terms` the same lines state what the recorded consent authorizes, in the log at the default `info` level, before the exchange opens either directory.
