...
Creation is announced rather than silent. The identity string bound into the certificate -- the party's name, organization, and contact -- comes from `--identity`, or from `linkage_terms.identity` in the configuration when the flag is absent; once an identity exists, `--identity` is ignored unless `--force` is also given. A label containing a control character or one of the nine Unicode text-direction formatting characters, or one past the length the linkage terms hold the same value to, is refused rather than bound: the value goes into a certificate a partner pins and reads back long after the run, and a text-direction character in it reorders the copy it is displayed beside. The implicit direction marks `U+200E`, `U+200F`, and `U+061C` are admitted, as they are in the linkage terms.
