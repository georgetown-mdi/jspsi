...
Each value anchors at most one certificate, so pinning the same fingerprint twice anchors one slot and leaves the other for something else. Pinning your own fingerprint is redundant with your signing identity, which already anchors your slot; on an ordinary record the pair anchors one slot between them, though a record repeating one certificate can read them as two anchors (see the spec's certificate-anchoring limit).
