...
This authoring-time check validates field coverage only, not cleaning liveness: it does not check whether a declared key's own cleaning can ever produce a value. A configuration whose key cleaning always drops every record therefore mints an invitation that later fails at run time, when `psilink exchange` refuses the same terms.
...
The no-effect notice an [`--identity`](#configuration) raises over a kept configuration is shown by the same pairing, at `warn` level: the log keeps it wherever `warn` survives, and the prompt is shown it wherever the log does not reach the terminal.
