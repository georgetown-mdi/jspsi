...
- **`columns you will send`** (`enforced`) -- the columns this party will send to your partner for matched records, one per line, or `(none)` when your file discloses none. Derived from your own input through the same rule that decides what psilink transmits, so for the configuration this acceptance writes, no column outside this list is sent. It bounds what leaves your machine rather than promising every column listed leaves it: a partner that receives no result is sent no payload at all, so a run may disclose less than the line lists. The bound is over the configuration acceptance writes, not over every later run: where acceptance keeps a configuration that already exists, the exchange runs on that file's stored metadata, which nothing here compares against this input. Where the set is not yet known -- no input file was given, or its columns cannot satisfy the invitation's keys -- the line says so and names what settles it: your input file, read when the exchange runs, with no second confirmation.
...
- **`the inviting party will receive the result`** (`your partner's word`) -- withholding a result from a partner rests on the agreed terms being honored, not on anything psilink can impose.
...
- **`columns you will receive`** (`enforced`) -- the columns the invitation declares your partner will send for matched records. A declared-but-empty set is the strict lock-in, shown with the party that aborts when it is violated. The abort stops the exchange from completing, not the columns from crossing: each side reconciles what it received only after the payloads have been exchanged, so a violated lock-in is caught on the far side of the wire.
- **`columns the inviting party requests from you`** (`your partner's word`) -- a request, not a declaration of what you send; what you actually send is the first line of the display.
