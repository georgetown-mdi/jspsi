...
Where the log would land somewhere else, it keeps its own copy as well: `--log-file` records the terms in the file, at `info` level, and shows them at the prompt. Where the log's destination is the terminal the question is asked on, there is no second copy, since the only thing a prefixed one would add is the multi-screen outline a second time.
