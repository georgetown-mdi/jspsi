...
The command returns promptly once its work is done. Every file it writes to a path -- the result, the exchange record, the receipt -- is written and waited for with no time limit. Two waits are bounded instead. The transport closing is one, at a ceiling set per channel and well above that channel's own teardown budgets. The other is a result sent to `stdout` rather than to an `OUTPUT_FILE` path: psilink waits up to 60 seconds for whatever is reading the pipe to take the last line, and a reader that has stopped reading ends the run with exit 73 and a result only partly delivered -- the code a result file that could not be written gets, and the same instruction with it, since the exchange already happened and re-running conducts a second one. After that the process returns within three seconds, so a scheduled run does not hold its slot, or its container, past the exchange. If something inside it is still holding the process at that point, psilink names the kinds of resource still open on stderr and exits with the status the run already reached -- the line reports a fault to pass on, and never changes the exit code a supervisor acts on.
