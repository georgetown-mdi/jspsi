...
- **`columns you will receive`** (`enforced`) -- the columns the invitation declares your partner will send for matched records, or `(none)` where it declares an empty set. Only a declared direction reaches this line: where the invitation declares nothing, the line is absent and what arrives is reconciled against your partner's own disclosure when the exchange runs. A declared empty set is a strict commitment that no column crosses in that direction, and your own side is what enforces it -- anything your partner sends is received and then rejected, aborting the exchange. The abort stops the exchange from completing, not the columns from crossing: each side reconciles what it received only after the payloads have been exchanged, so a violated commitment is caught on the far side of the wire.
