...
- `psilink init` expands `--config-file`; `psilink fingerprint` expands `--config-file`, `--identity-file`, and `--export-certificate`; `psilink verify-receipt` expands `RECORD`, `--keys`, and `--config-file`.
