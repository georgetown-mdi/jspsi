...
- **It is set.** The presented certificate must match it. A partner presenting anything else ends the run right there, with no result, no receipt, and nothing disclosed -- and your configuration file is left exactly as it stands. Re-pinning is an edit you make, after confirming the new value with the partner out of band.
- **It is not set.** This is your first exchange with this partner, so the run pins the fingerprint it was presented, records it in the configuration file you passed to `psilink exchange`, and prints it. The line names the value and the file it went into, and it also rides the [machine-readable event stream](#machine-readable-event-stream) under `partnerCertificatePinned`, so a scheduled run that discards stderr still reports it. That write needs the configuration file and its directory writable, which a read-only configuration mount does not give: such a run exits 64 before it connects, naming the file and telling you to set `signing.partner_fingerprint` yourself or mount the configuration writable for the first run (see [DEPLOYMENT.md](DEPLOYMENT.md#key-file-permissions-in-containers)).
