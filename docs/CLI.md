...
| 77 | `EX_NOPERM` | Authentication failed: the key exchange rejected the shared secret or the peer, the rotated shared secret could not be saved to the key file, or the SFTP server presented a host key other than the pinned one (or, with no terminal, no key is pinned). A retry reaches the same refusal. **Do not retry.** For the shared secret, re-invite (see [Out-of-sync tokens](#out-of-sync-tokens)); for the host key, verify the server's key out-of-band and re-pin (see [SFTP host-key trust](#sftp-host-key-trust)). Treat an unexpected one as a possible security event. |
