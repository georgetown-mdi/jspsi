...
- **The input discloses columns the invitation will not accept** -- the invitation declares the inviting party accepts no payload column, while the input discloses some (the columns listed on the display's `columns you will send` line). A warning names them, because that disagreement is one the exchange refuses to run on: it is caught before connecting, so the configuration this acceptance writes would stop at `psilink exchange` rather than send anything. The warning states both remedies -- set those columns not to transmit in the written configuration (`is_payload: false`, or the `ignored` role), or ask the partner for an invitation that accepts them -- and acceptance proceeds, since the first of the two is a local edit to the file it just wrote.
