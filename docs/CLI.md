...
| 69 | `EX_UNAVAILABLE` | Two conditions share this code, and the code alone does not separate them. Usually a transport or availability failure: the exchange server, peer, or shared storage was unreachable, rejected an operation, or went silent, and retrying once the transport recovers may succeed. From `psilink doctor`, it means something the checks themselves depend on was not available, so nothing was established either way. An online `invite`/`accept` also exits 69 when the exchange itself completed but the configuration could not be written -- that run must not be retried, and the terminal `result` event, not the code, is what identifies it. See below. |
...
An online `psilink invite` or `psilink accept` whose exchange completed but whose configuration could not be written exits 69 rather than 0. The exchange itself succeeded and any result was written, so it must not be re-run; what is missing is the configuration a later recurring `psilink exchange` reads. The rotated key is on disk, so recovery is to recreate the configuration -- not to re-invite -- and the error naming the cause was logged when the write failed.

So 69 carries two meanings: "the transport failed, retrying may succeed" and "the exchange completed, do not run it again". A supervisor that retries every 69 unconditionally re-runs a completed setup, which then fails against the key file already on disk rather than doing damage -- but the run it retried needed a configuration, not a repeat. What separates the two is the outcome, not the code: with `--event-stream` the completed case emits a `warning` event for the failed write and its terminal event is a `result`, where a transport failure emits a classified `error` instead (see [Terminal-event guarantees](spec/CLI_EVENTS.md#terminal-event-guarantees)). Without the event stream, the logged error at the end of an otherwise complete run is the same signal.
