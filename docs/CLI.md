...
- **What the interval costs.** Every wait for the partner's next file costs up to one interval, and an exchange makes many, so at the `5s` default the polling, not the PSI masking, is most of a small exchange's wall-clock time. The masking runs at about 12 microseconds per distinct key value on one core of the CLI's native backend, parallelized across cores, so it outweighs the polling only at hundreds of thousands of records (see [PROTOCOL.md](spec/PROTOCOL.md), "Masking compute is a wall-clock expectation"). The conservative default is what stays within SFTP servers' anti-flood limits, so lower it where you control the directory or the server -- a local mount, CI, or a demo -- and leave it as it is against a partner's shared server.
