...
- **The URL names the location and nothing else.** A `ws://`/`wss://` URL carrying a user, a password, a query, or a fragment is a usage error (exit 64) rather than a silently dropped component. For a coordination server that needs an API key, author `channel: webrtc` with `server.key` in `psilink.yaml` and use [`psilink exchange`](#webrtc-exchanges) instead.
...
- **The file-sync flags do not apply.** `--retain-files`, `--lockless-rendezvous`, `--polling-frequency`, and `--connection-per-poll` are reported ignored on this channel, and `--outbound-path` is a usage error: there is no exchange directory to retain, poll, or split.
- **`--server-port` and `--server-username` do not apply.** Both are reported ignored: the coordination server's port is part of the URL, and a webrtc invitation reaches it by location alone. A coordination server that needs a user takes it from `server.username` in `psilink.yaml`, as its API key does.
...
Every `psilink` command exits with one of the following codes. The failure classes 64, 69, and 73 follow the BSD `sysexits` convention.
...
- **A zero-setup `--save` run could not write the configuration, or the key file that accompanies it when both parties saved.** The exchange itself completed and its result is written; what is missing is the recurring-exchange setup. Fix what blocked the write, or point `--config-file` / `--key-file` elsewhere, and establish the recurring exchange with `psilink invite` -- re-running the zero-setup command would conduct a second exchange. A configuration file that appeared at the target path after the run's up-front check is reported the same way: the run refuses to overwrite it and reports the save as lost.
