...
Such a run has no way to finish, and refusing it up front is what spares you the disclosure. The two sides swap signatures only after their data has crossed, and psilink refuses your own certificate there rather than presenting one your partner would reject: whichever leg you drew, you end with no result and no receipt, keeping only the [exchange record](#when-the-receipt-swap-fails) of the disclosure you had already made. Your partner's run fails on the frame that never arrives, and your data has already reached them.
