...
- **Applied to your partner's side, not yours.** Beneath that sentence, at the same level, is whose records the setting groups. Accepting turns it on for the inviting party alone: your own records are not grouped, so several of your own rows naming one individual stay separate. Grouping your records instead is set up in each party's own config file -- one party's `deduplicate: true` against the other's `false` -- with both parties running `psilink exchange`.
