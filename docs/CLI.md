...
- **`allowed-character patterns`** (`your partner's word`) -- shown when a field declares one. Each pattern is a partner-supplied regular expression psilink has not verified; the raw pattern is shown so a reader who knows regular expressions can inspect what it actually admits.
...
- **`columns you will receive`** (`enforced`) -- the columns the invitation declares your partner will send for matched records. A declared-but-empty set is the strict lock-in, shown with the party that aborts when it is violated.
