...
An online `psilink invite` or `psilink accept` whose exchange completed but whose configuration could not be written exits 69 rather than 0. The exchange itself succeeded and any result was written, so it must not be re-run; what is missing is the configuration a later recurring `psilink exchange` reads. The rotated key is on disk, so recovery is to recreate the configuration -- not to re-invite -- and the error naming the cause was logged when the write failed. With `--event-stream` the same condition also emits a `warning` event, for a supervisor that reads fd 3 rather than stderr.
