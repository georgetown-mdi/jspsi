...
`--log-level <level>` selects the verbosity: `silent`, `error`, `warn`, `info` (the default), `debug`, or `trace`. `silent` suppresses all log output. The level governs every diagnostic the run emits once logging is set up, including the low-level warnings from data cleaning and file handling, so `debug` and `trace` turn up their detail and `silent` keeps them out of the terminal and out of a `--log-file` alike - except a usage error in the command line itself, which is reported on `stderr` before either flag takes effect and so is never routed to a `--log-file`. It does not suppress the linkage terms `accept` shows when it stops to ask you to confirm them: those accompany the question rather than the log, and reach the terminal whatever this is set to (see [Offline acceptance](#offline-acceptance)).
