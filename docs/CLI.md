...
An `INPUT_FILE` argument may be given as `-` to read the CSV from standard input instead of a file on disk -- for example, `cat data.csv | psilink exchange - results.csv` -- so a pipeline need not stage a temporary file. This applies to `psilink exchange`, the zero-setup form (`psilink URL INPUT_FILE`), and `psilink invite`. It does not apply to `psilink accept`, which reads its interactive confirmation from standard input and so cannot also take the CSV there; give `accept` a file path.
