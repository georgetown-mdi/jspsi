...
- **`columns you will send`** (`enforced`) -- the columns this party will send to your partner for matched records, one per line. It reads `(none)` in the two cases where nothing leaves: your file discloses no columns, or the invitation gives the inviting party no result, in which case the payload step transmits nothing at all and no column set is listed -- your input file cannot change that answer. Where columns are listed they are derived from your own input through the same rule that decides what psilink transmits, so for the configuration this acceptance writes, no column outside that list is sent. The bound is over the configuration acceptance writes, not over every later run: where acceptance keeps a configuration that already exists, the exchange runs on that file's stored metadata, which nothing here compares against this input. Where the set is not yet known -- no input file was given, or its columns cannot satisfy the invitation's keys -- the line says so and names what settles it: your input file, read when the exchange runs, with no second confirmation.
...
- **The input can satisfy only some of the keys** -- a warning names the unsatisfied fields (the keys that depend on them will be inactive) and the configuration is written without the metadata or the standardizing transformations, so the user can modify their data and retry. Where every field the keys need is satisfiable instead, the columns are inspected to infer metadata and data standardizing transformations, and both are written to the configuration file together with the linkage terms.
...
- **The input discloses columns the invitation will not accept** -- the invitation declares the inviting party accepts no payload column *and* is entitled to the matched result, while the input discloses some (usually the columns listed on the display's `columns you will send` line; where that line reads the not-yet-known case instead, the warning names the set inferred from your file's header). A warning names them, one per line, because that disagreement is one the exchange refuses to run on. The two remedies are to set those columns not to transmit in the written configuration (`is_payload: false`, or the `ignored` role), or to ask the partner for an invitation that accepts them. What follows the warning differs by path, and the warning says which:
...
Neither mode changes anything on the share beyond its own working files: the probe removes every file it creates except the marker, and the mount check consumes the marker and removes the rest.
