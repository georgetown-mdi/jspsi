...
- **Where the connection comes from.** Accepting an invitation minted by the web application seeds the whole block -- coordination server and `role` both -- from the invitation itself, so `psilink accept` writes a ready configuration. Otherwise author it by hand: the CLI mints no webrtc endpoint of its own, so a CLI invitation carries none, and a `ws://` or `wss://` URL on the invitation and zero-setup paths is refused (exit 64) rather than half-building one.
...
| 69 | `EX_UNAVAILABLE` | Two kinds of condition share this code, and the code alone does not separate them. Usually a transport or availability failure: the exchange server, peer, or shared storage was unreachable, rejected an operation, or went silent, and retrying once the transport recovers may succeed. From `psilink doctor`, it means something the checks themselves depend on was not available, so nothing was established either way. The other kind is a completed exchange that could not persist something it was asked to -- an audit artifact, or the configuration an online `invite`/`accept` writes -- which must not be re-run; the terminal `result` event, not the code, is what identifies it. See below. |
...
Two conditions exit 69 with the exchange itself intact. **Do not re-run either.** The data was exchanged and any result is on disk; what is missing is something the run was asked to persist afterwards.
...
So 69 carries two meanings: "the transport failed, retrying may succeed" and "the exchange completed, do not run it again". A supervisor that retries every 69 unconditionally re-runs a completed exchange or setup, and no repeat writes the artifact the first run was missing. What the repeat costs depends on the command: an online `invite` or `accept` fails against the key file already on disk rather than doing damage, while a recurring `psilink exchange` has no such guard and simply conducts another exchange with the rotated token, re-sending this party's data and waiting on the partner at the rendezvous again -- as does a retried zero-setup run. What separates the two is the outcome, not the code: with `--event-stream` a completed run emits a `warning` event for what it could not persist and its terminal event is a `result`, where a transport failure emits a classified `error` instead (see [Terminal-event guarantees](spec/CLI_EVENTS.md#terminal-event-guarantees)). Without the event stream, the logged failure at the end of an otherwise complete run is the same signal. The non-zero exit exists for the unattended run, where that line on stderr is discarded by a supervisor or suppressed by `--log-level error`.
