...
One exception: a few low-level warnings (from data cleaning and file handling) are not redirected and may still appear on the terminal (on `stderr`) even with `--log-file` set. If a run must leave no diagnostics on the terminal, also redirect the terminal output (for example with a shell redirect) alongside `--log-file`.
