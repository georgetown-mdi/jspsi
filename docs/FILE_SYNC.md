...
|------|---------|-----------|-------|-------------|------------------|----------------------|----------------------|--------------------|--------------------------------|
| hello | `<id>-hello.json` | each party | `hello` | **yes (envelope; fields planned: bilateral flags via 193901017)** | no (I5a read gate instead) | yes | yes (not in retain mode) | no (non-numeric) | no (non-numeric) |
...
- **I4a -- delete-mode crash safety.** In delete mode, every committed file a party creates is added to `responsibleFiles` *before* it is written, so a crash mid-write still leaves it swept by `cleanup()` at the next `close()`. The hello is tracked unconditionally; the ack, wave, and message are tracked here only (the `add` is guarded by `!retainFiles`).
