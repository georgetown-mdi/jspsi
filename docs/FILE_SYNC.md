...
|------|---------|-----------|-------|-------------|------------------|----------------------|----------------------|--------------------|--------------------------------|
| hello | `<id>-hello.json` | each party | `hello` | no today / **planned: bilateral flags** | no | yes | yes (not in retain mode) | no (non-numeric) | no (non-numeric) |
| wave | `<id1>-<id2>.wave` | starter (wave path) | n/a (`.wave`) | no | n/a | yes | yes | no (extension) | no (extension) |
| hello-ack | `<id>-hello-ack.json` (**planned: `<peerId>-hello-ack.json`**) | each party (lockless) | `ack` | no today / **planned: writer id** | no | yes | yes (not in retain mode) | no (non-numeric) | no (non-numeric) |
...
- **I5 -- partial-sync safety for every payload-bearing file.** A file whose body is read for content must not be read before the sync tool has finished writing it. Messages carry a body gated by a byte count *in the filename*; receipts the same. Control files that gain a body (hello mode flags; any ack field) have no byte-count segment in their names, so they need a read gate that retries on a short/unparseable read until the peer timeout. That gate is introduced once by the payload-envelope precursor; see [Payload-bearing control files](#payload-bearing-control-files-i5).
...
The payload-envelope precursor `194332289` introduces the body plumbing and the read gate **once**, so `193901017` and `194304738` add a field to an existing envelope rather than each re-introducing the mechanism. This is the resolution of the earlier "introduce vs extend" ambiguity, and it supersedes `194002643`'s recorded decision that the hello body stays empty with no downstream payload. (The alternative considered and rejected was encoding the flags in the hello filename; a body was chosen as the more extensible home.)
...
| 194332289 -- hello/ack payload envelope + read gate | The shared control-file body plumbing and the I5 partial-sync read gate | *Introduces* the payload once; 193901017 and 194304738 extend it. Supersedes 194002643's "no payload" decision |
