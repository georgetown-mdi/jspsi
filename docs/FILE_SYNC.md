...
1. **Preexisting-file guard** (`synchronize()` start, ~[:630](../packages/core/src/connection/fileSyncConnection.ts#L630)). One mode-agnostic rule: at entry the directory must be empty except for at most one **peer** hello (a `-hello.json` whose id is not this party's own). That is the only file kind that can legitimately predate this party's entry -- a party writes its own hello/wave/ack only after observing the peer's hello, and messages and receipts exist only once rendezvous has completed. Anything else is a `UsageError`: a second peer hello, a self-hello (a same-id crash leftover), a wave, a `-hello-ack.json`, a message, a receipt, an in-flight `temp-*.tmp`, or any foreign file. The rule is strict-empty by design (the directory is the state machine), so it subsumes the former retain-specific guard, closes a gap where stale delete-mode messages slipped through, and gives retain mode its fresh-directory precondition for free (a stale message would be mis-consumed against `recvSeq`; a stale receipt could prematurely release the sender's gate). The two file kinds that will later legitimately pre-exist -- an orphaned `temp-*.tmp` swept by `193792285`, and a directory snapshot -- are accommodated by adding their names to the guard's `ignored` set; until then they are rejected. A new file kind that can be left behind by a crash belongs in `ignored` only if it is recoverable, otherwise it stays rejected here.
...
3. **`poll()` message scan** (~[:1106](../packages/core/src/connection/fileSyncConnection.ts#L1106)). Reads the peer's messages: `<peerId>-` prefix, `.json`, numeric terminal, and `size >= declaredSize`. Non-numeric terminals are ignored, not errors. In retain mode the scan additionally selects only the message whose `<NNN>` equals the in-memory `recvSeq` counter, so an already-consumed message still on disk (retain mode never removes it) is skipped rather than re-delivered. A message that has reached its declared byte count but then fails `JSON.parse` or `Message` validation is a terminal `UsageError` that stops the poller -- re-reading the same fully-synced bytes cannot help -- not a retryable transport error; this mirrors the control-file gate (I5) and, in retain mode, is what stops a corrupt never-deleted message from being re-read every cycle until the peer timeout. Retryable errors (a downstream `emit` handler throwing, a receipt-write failure, a transient `list`/`get` hiccup) reschedule instead.
...
Legal contents at entry: nothing this protocol owns, except at most one **peer** hello (the other party may have arrived first). The preexisting-file guard aborts -- in both modes, with one `UsageError` -- on anything else: a second peer hello, a self-hello, a wave, an ack (and, planned, `joining`), a message, a receipt, an in-flight `temp-*.tmp`, or a foreign file. This single rule makes retain mode's fresh-directory precondition code-enforced rather than merely documented (a stale message would be mis-consumed against the `recvSeq` counter, and a stale receipt could prematurely release the sender's gate) and is what lets the second party to start -- which always sees the first party's hello -- proceed instead of failing.
