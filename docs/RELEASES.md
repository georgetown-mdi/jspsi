...
1. Set `EXCHANGE_RECORD_VERSION` to `psilink-exchange-record/v1` in `packages/core/src/records/exchangeRecord.ts`.
...
This covers the npm tree this repository declares. The image as built -- its OS packages, and the npm CLI's bundled dependencies its base bundles -- is gated separately, in CI, by the [image vulnerability scan](#image-vulnerability-scan).
...
- One BOM covers both published images: they resolve the same committed lockfile through the same production install, so their npm trees are identical and only their OS packages and base-bundled tooling differ -- which no `npm sbom` run reaches on either image, and which the [image vulnerability scan](#image-vulnerability-scan) covers separately.
