...
An SBOM in CycloneDX format is generated as part of the release checklist (step 9) and attached to each GitHub Release. It covers the npm dependency tree and no other layer: each image's OS packages are covered by the attribution lists beside `NOTICE`, per [CONTAINER_IMAGES.md](spec/CONTAINER_IMAGES.md#the-os-layer-attribution-lists).
