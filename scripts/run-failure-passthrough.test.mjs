...
// walking the exchange tree, so a seat nobody thought to list is covered the day it
...
const SELF = "scripts/run-failure-passthrough.test.mjs";
...
const SEAT_DIR = "apps/web/src/exchange";
const WEB_SOURCE_DIR = "apps/web/src";
const COMPOSER_MODULE = "apps/web/src/exchange/useInviterExchange.ts";
...
/**
 * The modules read: every source in the exchange tree, plus any other source in the
...
function modulesToRead() {
  ...
  const seats = sourceModules(SEAT_DIR);
  const elsewhere = sourceModules(WEB_SOURCE_DIR).filter(
    (file) =>
      !file.startsWith(`${SEAT_DIR}/`) && written.test(readSource(file)),
  );
  return [...seats, ...elsewhere].sort();
}
...
describe("no seat decorates the failure it composed", () => {
  it("finds the call sites and the composer they are read against", () => {
    // A rot guard: a directory move, a rename of the composer, or an extraction of
  ...
  it("reports a decorating call site beside the tree's own idiom", () => {
    ...
    // seat in the app's idiom that raises one failure the way every seat does
  ...
  it("reports the shapes it cannot read rather than passing them", () => {
    ...
    const sources = {
      "unread/boundSeat.ts": `import { failureFor as composeFailure } from "@exchange/useInviterExchange";
